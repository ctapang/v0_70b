...
#pragma config FPLLIDIV = DIV_2, FWDTEN = OFF, FCKSM = CSECME, FPBDIV = DIV_1
...
/*****************************************************************************
   Scratch RAM, that will be used by media driver to backup the flash block,
   while doing sector write operation.
  ...
  Remarks:
    None.
*/
uint8_t scratchRam[ERASE_BLOCK_SIZE];
...
extern const uint8_t MBR_ATTRIBUTES MasterBootRecord[MEDIA_SECTOR_SIZE] ;
...
USB_DEVICE_MDD_INTFLASH_INIT mddFlashInitData =
{
    MEDIA_SECTOR_SIZE,
    MDD_INTERNAL_FLASH_TOTAL_DISK_SIZE,
    (uint8_t*)MASTER_BOOT_RECORD_ADDRESS,
    ERASE_BLOCK_SIZE,
    scratchRam,
    USB_DEVICE_MDD_INTFLASH_WP_STATE_UNPROTECTED
};
