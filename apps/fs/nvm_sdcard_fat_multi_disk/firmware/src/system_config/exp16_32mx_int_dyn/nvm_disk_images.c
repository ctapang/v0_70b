...
const uint8_t MBR_ATTRIBUTES masterBootRecord[MEDIA_SECTOR_SIZE] =
{
    ...
    (uint8_t)MDD_INTERNAL_FLASH_PARTITION_SIZE,
    (uint8_t)(MDD_INTERNAL_FLASH_PARTITION_SIZE >> 8),
    (uint8_t)(MDD_INTERNAL_FLASH_PARTITION_SIZE >> 16),
    (uint8_t)(MDD_INTERNAL_FLASH_PARTITION_SIZE >> 24), /* Length of partition in sectors (MBR sits at LBA = 0, and is not in the partition.) */
...
const uint8_t PARTITION_ATTRIBUTES(BOOT_SECTOR_ADDRESS) bootSector[MEDIA_SECTOR_SIZE]  =
{
    ...
    (uint8_t)(MDD_INTERNAL_FLASH_PARTITION_SIZE >> 8),
    (uint8_t)(MDD_INTERNAL_FLASH_PARTITION_SIZE >> 16),
    (uint8_t)(MDD_INTERNAL_FLASH_PARTITION_SIZE >> 24),	    /*  Total sectors (when WORD value at offset 20 is 0x0000) */
...
const uint8_t PARTITION_ATTRIBUTES(FAT0_ADDRESS)  fileAllocationTable[MEDIA_SECTOR_SIZE] =
{
...
const uint8_t PARTITION_ATTRIBUTES(FATx_ADDRESS) FATx[MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_NUM_FAT_SECTORS - 1)];
#endif


const uint8_t PARTITION_ATTRIBUTES(ROOTDIRECTORY0_ADDRESS) rootDirectory[MEDIA_SECTOR_SIZE] =
{
    //Root
    ...
    0x1B, 0x00, 0x00, 0x00,                         /* File Size (number of bytes) */
        ...
        const uint8_t PARTITION_ATTRIBUTES(ROOTDIRECTORY1_ADDRESS) RootDirectory1[MEDIA_SECTOR_SIZE] =
        {0};
#endif

#if (MDD_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT>32)
    const uint8_t PARTITION_ATTRIBUTES(ROOTDIRECTORY2_ADDRESS) RootDirectory2[MEDIA_SECTOR_SIZE] =
    {0};
#endif

#if (MDD_INTERNAL_FLASH_MAX_NUM_FILES_IN_ROOT>48)
    const uint8_t PARTITION_ATTRIBUTES(ROOTDIRECTORY3_ADDRESS) RootDirectory3[MEDIA_SECTOR_SIZE] =
    {0};
#endif
 ...
 * the MDD_INTERNAL_FLASH_DRIVE_CAPACITY definition.  We will initialize 
...
const uint8_t PARTITION_ATTRIBUTES(SLACK0_ADDRESS) slack0_0[MEDIA_SECTOR_SIZE] =
{
 ...
 *           [MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_DRIVE_CAPACITY - 1u)] = {0};
 ...
 ********************************************************************************/
        ...
        const uint8_t PARTITION_ATTRIBUTES(SLACK1_ADDRESS) slack1[MEDIA_SECTOR_SIZE*63] = {0};
    #else
        ...
        const uint8_t PARTITION_ATTRIBUTES(SLACK2_ADDRESS) slack2[MEDIA_SECTOR_SIZE*63] = {0};
    #else
        const uint8_t PARTITION_ATTRIBUTES(SLACK2_ADDRESS) slack2[MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_DRIVE_CAPACITY - 64u)] = {0};
    #endif
#endif

#if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 127)
    #if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 190)
        const uint8_t PARTITION_ATTRIBUTES(SLACK3_ADDRESS) slack3[MEDIA_SECTOR_SIZE*63] = {0};
    #else
        const uint8_t PARTITION_ATTRIBUTES(SLACK3_ADDRESS) slack3[MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_DRIVE_CAPACITY - 127u)] = {0};
    #endif
#endif

#if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 190)
    #if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 253)
        const uint8_t PARTITION_ATTRIBUTES(SLACK4_ADDRESS) slack4[MEDIA_SECTOR_SIZE*63] = {0};
    #else
        const uint8_t PARTITION_ATTRIBUTES(SLACK4_ADDRESS) slack4[MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_DRIVE_CAPACITY - 190u)] = {0};
    #endif
#endif

#if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 253)
    #if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 316)
        const uint8_t PARTITION_ATTRIBUTES(SLACK5_ADDRESS) slack5[MEDIA_SECTOR_SIZE*63] = {0};
    #else
        const uint8_t PARTITION_ATTRIBUTES(SLACK5_ADDRESS) slack5[MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_DRIVE_CAPACITY - 253u)] = {0};
    #endif
#endif

#if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 316)
    #if(MDD_INTERNAL_FLASH_DRIVE_CAPACITY >= 379)
        const uint8_t PARTITION_ATTRIBUTES(SLACK6_ADDRESS) slack6[MEDIA_SECTOR_SIZE*63] = {0};
    #else
        const uint8_t PARTITION_ATTRIBUTES(SLACK6_ADDRESS) slack6[MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_DRIVE_CAPACITY - 316u)] = {0};
    #endif
#endif
        ...
        /**********************************************************************
         * If your MDD_INTERNAL_FLASH_DRIVE_CAPACITY is > 442 sectors, then you 
         ...
         * arrays as needed to meet your MDD_INTERNAL_FLASH_DRIVE_CAPACITY 
         * size requirements.
         *********************************************************************/
    #else
        const uint8_t PARTITION_ATTRIBUTES(SLACK7_ADDRESS) slack7[MEDIA_SECTOR_SIZE*(MDD_INTERNAL_FLASH_DRIVE_CAPACITY - 379u)] = {0};
    #endif
#endif
