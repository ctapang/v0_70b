...
static SNMP_NON_MIB_RECD_INFO gSnmpNonMibRecInfo[SNMP_MAX_NON_REC_ID_OID] =
{
#ifdef TCPIP_STACK_USE_SNMPV3_SERVER
...
static SNMP_TRAP_INFO trapInfo={2};
#ifdef TCPIP_STACK_USE_IPV6
static IPV6_SNMP_TRAP_INFO ipv6TrapInfo={2};
	...
	switch(smState)
	{
		...
		case SM_NOTIFY_WAIT:
			...
			if ( TCPIP_SNMP_NotifyIsReady(&ipv6Address,IPV6_SNMP_TRAP) )
			{
				...
				{
                                    retVal = TCPIP_SNMPv3_Notify(var, val, 0,targetIndex);
				}
				...
				{
                                    retVal = TCPIP_SNMP_Notify(var, val, 0);
				}
...
void SNMPV2TrapDemo(void)
{
    ...
    */
    if(netStartForTrap)
    {
        ...
        */
        snmpStkDcptMemStubPtr->SNMPNotifyInfo.trapIDVar = SNMP_DEMO_TRAP;
         ...
         snmpStkDcptMemStubPtr->SNMPNotifyInfo.snmpTrapInf = (TCPIP_NET_HANDLE*)TCPIP_SNMP_ClientGetNet();

        if(!TCPIP_STACK_NetIsUp(snmpStkDcptMemStubPtr->SNMPNotifyInfo.snmpTrapInf))
        {
            return;
        }
    ...
    for(;trapIndex<TRAP_TABLE_SIZE;trapIndex++)
    {
        ...
        {
#ifdef __PIC32MZ__
            analogPotVal.word= (uint16_t)AD1DATA0;
#else
            ...
            */
            ...
            for(targetIndex=userIndex;targetIndex<SNMPV3_USM_MAX_USER;targetIndex++)
            {
...
#endif
                    if((SYS_TICK_Get() - TCPIP_SNMP_TrapTimeGet()> 1*SYS_TICK_TicksPerSecondGet()))
                    {
                        trapIndex++;
                ...
                if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV3_MSG_PROCESSING_MODEL)
                    && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV3_USM_SECURITY_MODEL))
                {
                    TCPIP_SNMPv3_Notify(PUSH_BUTTON,analogPotVal,0,targetIndex);
                }
                else if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV2C_MSG_PROCESSING_MODEL)
                    && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV2C_SECURITY_MODEL))
                {
...
#if defined(SNMP_V1_V2_TRAP_WITH_SNMPV3)
                //TCPIP_SNMPv3_Notify(LED_D5,analogPotVal,0);
                if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV3_MSG_PROCESSING_MODEL)
                    && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV3_USM_SECURITY_MODEL))
                {
                    TCPIP_SNMPv3_Notify(LED_D5,analogPotVal,0,targetIndex);
                }
                else if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV2C_MSG_PROCESSING_MODEL)
                    && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV2C_SECURITY_MODEL))
                {
                    TCPIP_SNMP_Notify(LED_D5,analogPotVal,0);
                }
                else if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV1_MSG_PROCESSING_MODEL)
                    && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV1_SECURITY_MODEL))
                {
...
// then we should disable gSetTrapSendFlag to false
                ...
                //  Fourth varbind.
                {
                    ...
                    if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV3_MSG_PROCESSING_MODEL)
                        && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV3_USM_SECURITY_MODEL))
                    {
                        TCPIP_SNMPv3_Notify(TRAP_COMMUNITY,analogPotVal,0,targetIndex);
                    }
                    else if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV2C_MSG_PROCESSING_MODEL)
                        && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV2C_SECURITY_MODEL))
                    {
                        TCPIP_SNMP_Notify(TRAP_COMMUNITY,analogPotVal,0);
                    }
                    else if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV1_MSG_PROCESSING_MODEL)
                        && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV1_SECURITY_MODEL))
                    {
    ...
    {
        TCPIP_UDP_Discard(snmpStkDcptMemStubPtr->SNMPNotifyInfo.socket);
        timeLock = false;
            ...
            if(analogPotVal.word >12u)
            {
                ...
                for(targetIndex=userIndex;targetIndex<SNMPV3_USM_MAX_USER;targetIndex++)
                {
...
#endif
                    {
                        ...
                        userIndex = targetIndex;
#endif
                        if((SYS_TICK_Get() - TCPIP_SNMP_TrapTimeGet()> 2*SYS_TICK_TicksPerSecondGet()))
                        {
                            anaPotNotfyCntr++;
                            gtrapSMStateUpdate = true;
                            if(snmpStkDcptMemStubPtr->SNMPNotifyInfo.socket != INVALID_UDP_SOCKET)
        ...
        case SM_NOTIFY_WAIT:
            if(TCPIP_SNMP_NotifyIsReady(remHostIpAddrPtr,IPV4_SNMP_TRAP))
            {
                ...
                for(targetIndex=userIndex;targetIndex<SNMPV3_USM_MAX_USER;targetIndex++)
                {
                    if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV3_MSG_PROCESSING_MODEL)
                        && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV3_USM_SECURITY_MODEL))
                        TCPIP_SNMPv3_Notify(snmpStkDcptMemStubPtr->gOIDCorrespondingSnmpMibID, val, 0,targetIndex);
                    else if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV2C_MSG_PROCESSING_MODEL)
                        && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV2C_SECURITY_MODEL))
                        TCPIP_SNMP_Notify(snmpStkDcptMemStubPtr->gOIDCorrespondingSnmpMibID, val, 0);
                    else if((snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].messageProcessingModelType == SNMPV1_MSG_PROCESSING_MODEL)
                        && (snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[targetIndex].securityModelType == SNMPV1_SECURITY_MODEL))
                        TCPIP_SNMP_Notify(snmpStkDcptMemStubPtr->gOIDCorrespondingSnmpMibID, val, 0);
                }
...
uint8_t SNMPValidateCommunity(uint8_t * community)
{
    ...
    */
    ...
    if(strlen((char*)community) <= SNMP_COMMUNITY_MAX_LEN)
    {
        ...
        {
            ptr = TCPIP_SNMP_WriteCommunityGet(i);
            if(ptr == NULL)
                continue;
            if(*ptr == 0x00u)
                continue;
        ...
        for(i = 0; i < SNMP_MAX_COMMUNITY_SUPPORT; i++)
        {
            ptr = TCPIP_SNMP_ReadCommunityGet(i);
            if(ptr == NULL)
                continue;
            if(*ptr == 0x00u)
                continue;
...
bool SNMPSetVar(SNMP_ID var, SNMP_INDEX index, uint8_t ref, SNMP_VAL val)
{
    ...
    case SNMP_TARGET_SECURITY_LEVEL :           // 43.6.1.4.1.17095.5.1.1.5: READWRITE  uint8_t.
        {
            if(TCPIP_SNMPv3_CmprTrapSecNameAndSecLvlWithUSMDb(index,strlen((char*)snmpv3EngnDcptMemoryStubPtr->Snmpv3TrapConfigData[index].userSecurityName),
