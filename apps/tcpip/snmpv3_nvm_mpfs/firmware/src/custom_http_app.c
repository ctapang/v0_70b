    ...
    switch(TCPIP_HTTP_CurrentConnectionPostSmGet(connHandle))
    {
        ...
        case SM_CFG_SNMP_READ_VALUE:
            ...
            if(memcmp((void*)httpDataBuff, (const void*)"rcm", 3) == 0)
                        ...
                        {
                            dest = TCPIP_SNMP_WriteCommunityGet
(vCommunityIndex);
                        }
            else
                        {
...
void HTTPPrint_drive(HTTP_CONN_HANDLE connHandle)
{
 TCPIP_TCP_StringPut(TCPIP_HTTP_CurrentConnectionSocketGet(connHandle), (const void*)SYS_FS_DRIVE);
}

void HTTPPrint_fstype(HTTP_CONN_HANDLE connHandle)
{
...
void HTTPPrint_cookiefav(HTTP_CONN_HANDLE connHandle)
{
    const uint8_t *ptr;
    TCP_SOCKET sktHTTP;
...
void HTTPPrint_write_comm(HTTP_CONN_HANDLE connHandle, uint16_t num)
{
    ...
    // Send proper string
    TCPIP_TCP_StringPut(sktHTTP, TCPIP_SNMP_WriteCommunityGet
(num));
    #endif
}
