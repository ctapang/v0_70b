...
{
    pNetIf = (NET_CONFIG *)TCPIPStackNetHandle(interface);
 ...
 switch ( APPCTX.mProtocol )
 {
      case kIperfProtoTCP:	// TCP
            ...
            if ( (APPCTX.tcpServerSock == INVALID_SOCKET) &&
                 (APPCTX.tcpServerSock = TCPIP_TCP_ServerOpen(IP_ADDRESS_TYPE_IPV4, APPCTX.mServerPort, 0)) == INVALID_SOCKET )
            {
               /* error case */
            ...
            TCPIP_TCP_FifoSizeAdjust(APPCTX.tcpServerSock, 1, 0,TCP_ADJUST_PRESERVE_RX | TCP_ADJUST_GIVE_REST_TO_RX);
      ...
      case kIperfProtoUDP:	// UDP
           ...
           if ( (APPCTX.udpSock = TCPIP_UDP_ServerOpen(IP_ADDRESS_TYPE_IPV4, APPCTX.mServerPort, 0)) == INVALID_UDP_SOCKET )
           {
               /* error case */
           ...
           TCPIP_UDP_SocketNetSet(APPCTX.udpSock, pNetIf); // Bind test socket to dedicated interface for UDP Ack diagram
...
StateMachineTcpRx(void)
{
    ...
    if( (length = TCPIP_TCP_GetIsReady(APPCTX.tcpServerSock)) == 0 )
    {

      if ( TCPIP_TCP_WasReset(APPCTX.tcpServerSock)  )
      {
    ...
    else
    {
       ...
       while ( length > 0 )
       {
          ...
          TCPIP_TCP_ArrayGet( APPCTX.tcpServerSock, (BYTE*)g_bfr, chunk);
          length -= chunk;
       }
...
StateMachineUdpRx(void)
{
    ...
    if( (length = TCPIP_UDP_GetIsReady(APPCTX.udpSock)) >= (uint16_t)(sizeof(tIperfPktInfo)) )
    {

       /* The GetArray should not fail... */
       if ( TCPIP_UDP_ArrayGet(APPCTX.udpSock, (BYTE*)g_bfr, sizeof(tIperfPktInfo)) != sizeof(tIperfPktInfo) )
       {
       ...
       if ( (APPCTX.pktCount == (uint32_t)0) && (APPCTX.pktId < (long)0) )
       {
          ...
          TCPIP_UDP_Discard(APPCTX.udpSock);
          return;
       }
       ...
       if (APPCTX.pktCount == (uint32_t)1 )
       {
          ...
          TCPIP_UDP_SocketInfoGet(APPCTX.udpSock, &sktInfo);
          sprintf((char *) g_ConsoleContext.txBuf,"    - Remote %u.%u.%u.%u port %u",      
      ...
      while ( length > 0 )
      {
         ...
         TCPIP_UDP_ArrayGet(APPCTX.udpSock, (BYTE*)g_bfr, chunk);
         length -= chunk;
      }
...
{
    if( TCPIP_UDP_GetIsReady(APPCTX.udpSock) > (uint8_t)0 )
    {
         TCPIP_UDP_Discard(APPCTX.udpSock);
         return;
    }
...
StateMachineUdpRxDone(void)
{
    ...
    if (  TCPIP_UDP_GetIsReady(APPCTX.udpSock)  )
    {
        TCPIP_UDP_Discard(APPCTX.udpSock);
        return;
    }
    ...
    if ( UDPIsPutReady(APPCTX.udpSock ) > 0u )
    {
      ...
      TCPIP_UDP_ArrayPut(APPCTX.udpSock, (BYTE*)g_bfr, MAX_BUFFER);

      for ( tmp=0; tmp < (128 - MAX_BUFFER); tmp++)
        TCPIP_UDP_Put(APPCTX.udpSock, 0x00);

      TCPIP_UDP_Flush(APPCTX.udpSock );
...
StateMachineRxDone(void)
{
   ...
   if ( APPCTX.mProtocol == kIperfProtoUDP )
   {
     TCPIP_UDP_Close(  APPCTX.udpSock );
   }
   else
   {

     /* 2 disconnects on purpose not mistake */
     TCPIP_TCP_Close( APPCTX.tcpServerSock );
   }
...
StateMachineTxStart(void)
{
   ...
   TCPIP_ARP_Resolve(pNetIf, &APPCTX.remoteSide.remoteIP.remoteIPv4);
...
StateMachineTxArpResolve(void)
{
  ...
  if(!TCPIP_ARP_IsResolved(pNetIf, &APPCTX.remoteSide.remoteIP.remoteIPv4, &APPCTX.remoteSide.remoteMACAddr))
  {
...
StateMachineUDPTxOpen(void)
{
    ...
    if ( (APPCTX.udpSock = TCPIP_UDP_ClientOpen(IP_ADDRESS_TYPE_IPV4, APPCTX.mServerPort, (IP_MULTI_ADDRESS*)&APPCTX.remoteSide.remoteIP.remoteIPv4)) == INVALID_UDP_SOCKET )
    {
        /* error case */
        WFConsolePrintStr("Create UDP socket failed", true);
        APPCTX.state = kStateStandby;
...
StateMachineTCPTxOpen(void)
{
   ...
   if  ( (APPCTX.tcpClientSock = TCPIP_TCP_ClientOpen(IP_ADDRESS_TYPE_IPV4, APPCTX.mServerPort, &APPCTX.remoteSide.remoteIPaddress)) == INVALID_SOCKET )
   {
       /* error case */
        WFConsolePrintStr("Create TCP socket failed", true);
        APPCTX.state = kStateStandby;
    ...
    TCPIP_TCP_FifoSizeAdjust(APPCTX.tcpClientSock, 10, 1,TCP_ADJUST_GIVE_REST_TO_TX);
...
StateMachineTCPTxConnect(void)
{
 ...
 if ( IperfAppKillRequested() == true )
 {
  ...
  if( !TCPIP_TCP_IsConnected(APPCTX.tcpClientSock) )
  {

    // Time out if too much time is spent in this state
    if(SYS_TICK_Get()- APPCTX.timer > 5*SYS_TICK_TicksPerSecondGet())
    {
       TCPIP_TCP_Close(APPCTX.tcpClientSock);
       APPCTX.tcpClientSock = INVALID_SOCKET;
  ...
  TCPIP_TCP_WasReset(APPCTX.tcpClientSock);
...
GenericTxStart(void)
{
    ...
    {
       /* Manage socket */
       if( TCPIP_TCP_GetIsReady(APPCTX.tcpClientSock) > 0u )
       {
          TCPIP_TCP_Discard(APPCTX.tcpClientSock);
          return false;
       }

       if ( TCPIP_TCP_WasReset(APPCTX.tcpClientSock) )
       {
       ...
       if  (( TCPIP_TCP_PutIsReady(APPCTX.tcpClientSock) <= APPCTX.mMSS ) && (!iperfKilled))
          return false;
    ...
    {
       /* Manage socket */
       if( TCPIP_UDP_GetIsReady(APPCTX.udpSock) > 0u )
       {
          TCPIP_UDP_Discard(APPCTX.udpSock);
          return false;
       }
    ...
    if ( APPCTX.mProtocol == kIperfProtoUDP )
    {
        ...
        if ( TCPIP_UDP_ArrayPut(APPCTX.udpSock, g_bfr, MAX_BUFFER) != MAX_BUFFER )
        {
    ...
    else
    {
        ...
        if (( TCPIP_TCP_ArrayPut(APPCTX.tcpClientSock, (BYTE*) g_bfr, MAX_BUFFER) != MAX_BUFFER ) && (!iperfKilled))
        {
            WFConsolePrintStr("Socket send failed", true);
            APPCTX.errorCount++;
            return false;
...
TcpTxFillSegment(void)
{
  ...
  while( APPCTX.remainingTxData > 0u )
  {
    ...
    if ( TCPIP_TCP_ArrayPut( APPCTX.tcpClientSock, (BYTE *) g_bfr, chunk) != chunk )
      return;
...
UdpTxFillDatagram(void)
{
  ...
  while( APPCTX.remainingTxData > 0u )
  {
    ...
    if (  TCPIP_UDP_ArrayPut(APPCTX.udpSock, (BYTE *) g_bfr, chunk) != chunk )
     return;
...
StateMachineTcpTxSegment(void)
{
    ...
    {
       TcpTxFillSegment();
       TCPIP_TCP_Flush(APPCTX.tcpClientSock);
       GenericTxEnd();
    }
}
