...
// RECOMENDED - network_config.h ipaddress should be 192.168.1.25 if DHCP server ip address range starts
// from 192.168.1.100.


#define DHCPS_IP_ADDRESS_RANGE_START		"192.168.1.100"     // Address range is starting from 100, because the from 1 to 100 is reserved. Start address(192.168.100.1)
															// should be used as server address
#define DHCP_SERVER_IP_ADDRESS			"192.168.1.25"      // First three octet should match to the TP address range for Server address
...
#define DHCP_SERVER_GATEWAY_ADDRESS		"192.168.1.25"      // as we are in same network, Gateway address is same as server IP address.

#if defined(TCPIP_STACK_USE_DNS)
#define DHCP_SERVER_PRIMARY_DNS_ADDRESS		"192.168.1.25"      // DNS Primary address
#define DHCP_SERVER_SECONDARY_DNS_ADDRESS	"192.168.1.25"      // DNS Secondary address
#endif 
