...
void GenericTCPClient(void)
{
...
#ifndef TEST_WITH_NO_DNS
            dnsRes = TCPIP_DNS_UsageBegin(netH);
    ...
    serverIP.Val= SERVER_ADDRESS;
        ...
        case SM_SOCKET_OBTAINED:
            ...
            clientTimer = SYS_TICK_Get();
#ifdef TCPIP_STACK_USE_SSL_CLIENT
            if(!TCPIP_TCPSSL_ClientStart(MySocket,(uint8_t *)"MCHPCLIENT"))
                break;
#endif
        ...
        case SM_PROCESS_DATA:
            ...
            TCPIP_TCP_StringPut(MySocket, (const uint8_t*)"GET ");
            TCPIP_TCP_StringPut(MySocket, RemoteURL);
            TCPIP_TCP_StringPut(MySocket, (const uint8_t*)" HTTP/1.0\r\nHost: ");
            TCPIP_TCP_StringPut(MySocket, (const uint8_t*)ServerName);
