...
#if defined(TCPIP_STACK_USE_TCP) && defined(APP_USE_GENERIC_TCP_SERVER_DEMO) && defined(TCPIP_STACK_USE_IPV6)
...
// How to test: in win7 pc, type in command telnet xx:xx:xx:xx:xx:xx:xx 9765   ,use IPv6 address, it will work on IPv6 telnet
...
void GenericTCPServer_IPv6(void)
{
	...
	switch(TCPServerState)
	{
		case SM_HOME:
			...
			MySocket = TCPIP_TCP_ServerOpen(IP_ADDRESS_TYPE_IPV6, SERVER_PORT, 0);
...
#endif  // defined(TCPIP_STACK_USE_TCP) && defined(APP_USE_GENERIC_TCP_CLIENT_DEMO)
