/*******************************************************************************
  ...
  Summary:
     Application to demonstrate SPI driver

  Description:
    This application implements EEPROM driver using SPI driver interface
    and demonstrates write and read fucntionalities.
 *******************************************************************************/
...
void APP_Initialize( void )
{
    /* Set the direction for LED ports */
    SYS_PORTS_PinDirectionSelect( PORTS_ID_0, SYS_PORTS_DIRECTION_OUTPUT, PORT_CHANNEL_D, PORTS_BIT_POS_12 );
    SYS_PORTS_PinDirectionSelect( PORTS_ID_0, SYS_PORTS_DIRECTION_OUTPUT, PORT_CHANNEL_A, PORTS_BIT_POS_0 );
    SYS_PORTS_PinDirectionSelect( PORTS_ID_0, SYS_PORTS_DIRECTION_OUTPUT, PORT_CHANNEL_A, PORTS_BIT_POS_1 );
    SYS_PORTS_PinDirectionSelect( PORTS_ID_0, SYS_PORTS_DIRECTION_INPUT, PORT_CHANNEL_G, PORTS_BIT_POS_9 );

    /* Clear the LED PORT lines*/
    SYS_PORTS_PinClear(PORTS_ID_0,PORT_CHANNEL_A, PORTS_BIT_POS_0 );
    SYS_PORTS_PinClear(PORTS_ID_0,PORT_CHANNEL_A, PORTS_BIT_POS_1 );
    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_EEPROM_VERIFY:
        {
            ...
            for(count =0; count < MAX_NUM_OF_BYTES; )
            {
                ...
                if(count%2)
                {
                    /* Check if the data is 0xAA*/
                    if(!(getEEPROMdata[count++] == 0x55))
                    {
                        break;
                    }
                }
                else /* Odd address locations*/
                {
            ...
            {
                /* Test Passed*/
                SYS_PORTS_PinSet(PORTS_ID_0,PORT_CHANNEL_A, PORTS_BIT_POS_1 );
            }
...
void APP_EEPROM_Read( SPI_DATA_TYPE *rxbuffer, uint32_t num_of_bytes )
{
    ...
    appData.drvSPIRDBUFHandle = DRV_SPI_BufferAddRead( appData.drvSPIHandle, (SPI_DATA_TYPE *)&appData.drvSPIRXbuffer[0], num_of_bytes );
}
...
uint8_t APP_EEPROM_Read_Tasks(void)
{
    ...
    switch(readstate)
    {
        ...
        {           
            /* Add read command op code & 16-bit begining address to buffer */
        ...
        {
            /* Add to the write buffer and transmit*/
            APP_EEPROM_Read(&appData.drvSPIRXbuffer[0], (MAX_NUM_OF_BYTES + 1));
...
void APP_EEPROM_Write( SPI_DATA_TYPE * txbuffer, uint32_t num_of_bytes)
{
    ...
    appData.drvSPIWRBUFHandle = DRV_SPI_BufferAddWrite(appData.drvSPIHandle,(SPI_DATA_TYPE *)&appData.drvSPITXbuffer[0], num_of_bytes);
}
