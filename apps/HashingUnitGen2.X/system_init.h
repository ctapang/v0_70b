 ...
 * Author: CarlosT
 *
 * Created on December 1, 2013, 2:28 AM
 */
...
// Put definitions of interrupt sources here for PIC32MZ since this part
// family is not supported in legacy libraries.
#if defined (__PIC32MZ__)
typedef bool    INT_EN_DIS;
typedef enum
{
    INT_CT      = 0,    // core timer
...
#define INT_DISABLED    false
#define INT_ENABLED     true
...
typedef unsigned int INT_PRIORITY;
...
typedef unsigned int INT_SUB_PRIORITY_LEVEL;

#define    INT_CORE_TIMER_VECTOR    0       // internal core timer

#define    INT_EXTERNAL_1_VECTOR    8       // _EXTERNAL_1_VECTOR

#define    INT_EXTERNAL_3_VECTOR    18      // _EXTERNAL_3_VECTOR

#define    INT_CHANGE_NOTICE_VECTOR 118    // _CHANGE_NOTICE_A_VECTOR

#define    INT_ETH_VECTOR           153    // _ETHERNET_VECTOR

typedef unsigned int    INT_VECTOR;
...
extern  SYS_TICK __attribute__((always_inline)) SYS_TICK_Get(void)
{
...
extern uint32_t __attribute__((always_inline)) SYS_TICK_ResolutionGet(void)
{
...
extern uint32_t __attribute__((always_inline)) SYS_TICK_TicksPerSecondGet(void)
{
    return _SystemTickResolution;
}
...
extern uint32_t __attribute__((always_inline)) SYS_TICK_HwResolutionGet(void)
{
    return 10; // SYS_CLK_ClockGet()/2;  // TODO: fix this
}
...
extern SYS_TICK __attribute__((always_inline)) SYS_TICK_HwCountGet(void)
{
#if defined (__C32__)
    return 2; //_CP0_GET_COUNT();
...
extern SYS_INT_SOURCE __attribute__((always_inline)) SYS_INT_Source(SYS_MODULE_ID modId)
{
    if(modId == SYS_MODULE_ETH_1)
    {
        return PLIB_INT_SOURCE_ETH_1;
    }
    else if(modId == SYS_MODULE_ETH_PHY)
    {
        return PLIB_INT_SOURCE_EXTERNAL_3;
    }
