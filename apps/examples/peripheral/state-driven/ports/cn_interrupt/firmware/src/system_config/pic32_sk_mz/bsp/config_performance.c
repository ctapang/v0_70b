/*******************************************************************************
  ...
  Summary:
    Contains a non-MPLAB Harmony implementation of legacy SYSTEMConfigPerformance 
    function

  Description:
    This file contains a non-MPLAB Harmony implementation of the legacy 
...
#define PB_BUS_MAX_FREQ_HZ      200000000
...
unsigned int SYSTEMConfigPerformance(unsigned int sys_clock)
{
    ...
    SYSKEY = 0xAA996655;
    SYSKEY = 0x556699AA;
    PB3DIV = 0x00008004;        // Set PBCLK = SYSCLK/5 = 200MHz/5 = 40MHz
    SPLLCON = 0x01310081;       // PLL/2, PLL*50, PLLIDIV = 1, FRC = input for PLL, Range = 5_10MHz (SYSCLK = SPLL = 200MHz)
    //SPLLCON = 0x03630201;
    OSCCON |= 0x00000100;       // Set SYSPLL as SYSCLK
    OSCCON |= 0x00000001;       // wait untill SYSPLL is selected
    while (OSCCON & 0x00000001);
    SYSKEY = 0x33333333;
