...
extern void IspSpiTxRx(unsigned char   *p_txBuf,  int  txLen, unsigned char   *p_rxBuf,int  rxLen);
extern bool IspSpi_Tx(unsigned char *buf,int size);
extern bool IspSpi_Rx(unsigned char *buf,int size);
...
uint8_t Read8BitWFRegister(uint8_t regId)
{
    ...
    bool intDisabled = WF_EintIsDisabled();
    WF_EintDisable();
...
#if defined(ISP_SPIDMA_TESTING)
    IspSpiDMA_Tx(p_Buf,	length);
#else  //!(ISP_SPIDMA_TESTING)
    IspSpi_Tx(p_Buf,	length);
#endif //(ISP_SPIDMA_TESTING)
...
#if defined(ISP_SPIDMA_TESTING)
    IspSpiDMA_Rx(p_Buf,	length);
#else // !defined(ISP_SPIDMA_TESTING)
    IspSpi_Rx(p_Buf,	length);
#endif //defined(ISP_SPIDMA_TESTING)
