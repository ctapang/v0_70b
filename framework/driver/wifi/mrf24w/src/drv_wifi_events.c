...
// TODO: the following structures should be part of MRF24W_MAC_DCPT
// so that each instance of the MRF24W driver supports the same data
// However, this implies lots of changes to the low level WiFi driver implemented
// by the RFPD.
// For now a simple, one MAC based based approach implementation is given
// since there's no real need to support multiple MRF24W WiFi MACs.
