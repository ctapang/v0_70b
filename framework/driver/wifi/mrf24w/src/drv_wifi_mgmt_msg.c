...
extern void WaitMgmtResponse(void);
...
PA6_IO = 0;
    ...
    if (freeAction == FREE_MGMT_BUFFER)
    {
        ...
        if (hdr.result != DRV_WIFI_SUCCESS)
        {
            ...
            #else
                DRV_WIFI_ProcessEvent(hdr.result, 0);
            #endif
        }
    }
}
...

{
    ...
    SYS_ASSERT((hdr.result  == DRV_WIFI_SUCCESS), "");
    SYS_ASSERT((hdr.subtype == expectedSubtype), "");
...
/*****************************************************************************
  Function:
    void SignalManagementFrame(void)

  Summary:
    Signals MRF24WG that a management frame has been copied and is ready to process.

  Description:
    The management message has already been copied to the MRF24WG scrach memory.
...
void SignalManagementFrame(void)
{
    // Notify MRF24WG that management message is ready to be processed
    Write16BitWFRegister(WF_HOST_MAIL_BOX_0_MSW_REG, 0x8000);
    Write16BitWFRegister(WF_HOST_MAIL_BOX_0_LSW_REG, 0x0000);
}
