...
typedef struct rawMoveWaitState
{
    ...
    SYS_TICK startTickCount;    // start time of raw move
    SYS_TICK maxAllowedTicks;   // max allowed wait time for a raw move
} t_rawMoveWaitState;
...
t_rawMoveWaitState g_rawMoveState;
                    ...
                    uint16_t   size)
{
    ...
    if (rawId <= RAW_ID_1)
    {
        ...
        ////Write8BitWFRegister(WF_HOST_INTR_REG, regValue);  // ORIG may need to put back
    }
    ...
    else
    {
        ...
        ////Write16BitWFRegister(WF_HOST_INTR2_REG, regValue);  // ORIG may need to put back
    }
#endif
    ...
    if (rawId <= RAW_ID_1)
    {
        ...
        g_rawMoveState.rawIntMask = (rawId == RAW_ID_0)?WF_HOST_INT_MASK_RAW_0_INT_0:WF_HOST_INT_MASK_RAW_1_INT_0;
    }
    else
    {
        /* will be INTR2 bit in host register, signifying RAW2, RAW3, or RAW4 */
        g_rawMoveState.rawIntMask = WF_HOST_INT_MASK_INT2;
    }
...
bool isRawMoveComplete(int rawId, int *p_status, uint16_t *p_byteCount)
{
    ...
    *p_status    = RM_WAITING;
    ...
    if(RawMoveState[rawId].rawInterrupt & g_rawMoveState.rawIntMask)
    {

        RawMoveState[rawId].waitingForRawMoveCompleteInterrupt = false; 

        // stop raw move timer
        StopRawMoveTimer(g_rawMoveState.rawId);

        /* read the byte count and save it */
        regId = g_RawCtrl1Reg[g_rawMoveState.rawId];
...
#if 0
    // else if timed out waiting for raw move complete
    else
    {
        /* If timed out waiting for RAW Move complete than lock up */
        if (isRawMoveTimeout(g_rawMoveState.rawId))
        {
           *p_status = RM_TIMEOUT;
            retCode = true;
        }
