...
extern TCPIP_MAC_MODULE_CTRL * GetStackData();
...
int WF_InitStateMachine(void)
{
    ...
    switch (g_initState)
    {
       ...
       //-----------------------------------------
           ...
           // TODO: can't call this here because the stack init needs to complete and
           // fill in data that GetNetworkConfig needs.  See where hMac is used
           // and fix so we can call it here
#if (WF_DEFAULT_NETWORK_TYPE == DRV_WIFI_NETWORK_TYPE_SOFT_AP)
           {
...
{
    TCPIP_NET_IF* pNetIf;
    TCPIP_MAC_MODULE_CTRL *p_Setup = GetStackData();
...
#error "No security defined"
    ...
    #if 0   // TODO jw :  remove it or reserver it?

        TCPIP_NET_IF* ppNetIf = (TCPIP_NET_IF*)pNetIf;
        if(ppNetIf !=0)
        {
            ppNetIf->Flags.bIsDHCPEnabled = p_wifi_ConfigData->Flags.bIsDHCPEnabled;
            ppNetIf->DefaultIPAddr.Val        = p_wifi_ConfigData->netIPAddr.Val;
            ppNetIf->DefaultMask.Val          = p_wifi_ConfigData->netMask.Val;
            ppNetIf->netGateway.Val       = p_wifi_ConfigData->netGateway.Val;
            ppNetIf->PrimaryDNSServer.Val = p_wifi_ConfigData->PrimaryDNSServer.Val;

            memcpy(ppNetIf->NetBIOSName, p_wifi_ConfigData->NetBIOSName, sizeof(ppNetIf->NetBIOSName));
        }
    #endif
