...
#define MAX_NONBUFFERED_BYTE_COUNT 4 //After this number the PMP transfer should be polled to gurantee data trasnfer
...
typedef enum
{
    /* First PMP instance */
    DRV_PMP_INDEX_0,         
    /* Second PMP instance (not avaialble for now) */
    DRV_PMP_INDEX_1         
...
typedef struct _DRV_PMP_QUEUE_ELEMENT_OBJ
{
	...
	/* number of times the data set has to be transferred repetedly */
	int32_t 												nRepeats;
...
/* Function:
  ...
  Description:
    This function initializes the PMP driver, making it ready for clients to
    open and use it.
  ...
  Returns:
    If successful, it returns a valid handle to a driver object.  Otherwise, it
    returns SYS_MODULE_OBJ_INVALID. The returned object must be passed as 
    argument to DRV_PMP_Reinitialize, DRV_PMP_Deinitialize, DRV_PMP_Tasks and 
    DRV_PMP_Status routines. 
  ...
  Remarks:
    ...
    unless DRV_PMP_Deinitialize is called to deinitialize the driver instance.
    ...
    DRV_PMP_Status operation. The system must use DRV_PMP_Status to find out 
...
/* Function:
  ...
  Description:
    ...
    settings using the specified initialization data, but it will not interrupt any
    ongoing operations.
...
/* Function:
  ...
  Description:
    This function is used to maintain the queue and execute the tasks stored in 
    the queue. It resides in the ISR of the PMP for interrupt-driven implementations.
...
/* Function:
  ...
  Description:
    This function gets the client-specfic status of the PMP driver associated
...
/* Function:
  ...
  Description:
    ...
    the external device to the MCU buffer through the selected PMP instance. 
    This function should be used for all the master and slave modes. Proper 
    configuration should be done using DRV_PMP_ModeConfig before calling this 
    function.
	
  Precondition:
    The DRV_PMP_Initialize routine must have been called. DRV_PMP_Open must have 
    been called to obtain a valid opened device handle. DRV_PMP_ModeConfig must 
    have been called to configure the desired mode
  ...
  Parameters:
    ...
    address      - Starting address of the slave device from where data has to be read.
                   ...
                   In PMP enhanced slave mode i.e. addressable buffer slave mode, this 
                   parameter should be the buffer number to be used.
    buffer       - Pointer to the buffer into which the data read through the PMP instance
...
*/
   ...
   QUEUE_ELEMENT_OBJECT* DRV_PMP_Read ( DRV_HANDLE hClient,
                        uint32_t address,
                        uint16_t* buffer,
                        uint32_t nBytes);
...
/* Function:
  ...
  Description:
    ...
    PMP instance. It repeats the operation n (=repeatCount) number of times as well. 
    This function should be used for all the master and slave modes. Proper 
    configuration should be done using DRV_PMP_ModeConfig before calling this 
    function.
  ...
  Parameters:
    ...
    buffer       - Pointer to MCU Buffer from which the data will be written through
                   the PMP instance. even if only one word has to be tranferred, pointer 
                   should be used.
    ...
    repeatCount  - Number of times the data set (nBytes of data) to be repetedly written.
                   This value should be 0 if user does not want any repetation. If 
                   repeatCount is greater than 0, then after writing every nBytes of data, 
                   the buffer starts pointing to its first element. Ideally, PMP Address 
                   should be in auto increment/decrement mode for repeatCount greater 
                   than 0.
...
*/				   
QUEUE_ELEMENT_OBJECT*       DRV_PMP_Write ( DRV_HANDLE* hClient,
                            bool address,
                            uint32_t *buffer,
                            uint32_t nBytes,
                            uint32_t repeatCount);
...
DRV_PMP_TRANSFER_STATUS DRV_PMP_TransferStatus( QUEUE_ELEMENT_OBJECT* queueObject);
...
// *****************************************************************************
// *****************************************************************************
/* These functions return the version information of the PMP driver */


// *****************************************************************************
/* Function:
    unsigned int DRV_PMP_VersionGet( const SYS_MODULE_INDEX drvIndex )

  Summary:
    Gets the PMP driver version in numerical format.

  Description:
    This function gets the PMP driver version. The version is encoded as
    major * 10000 + minor * 100 + patch. The stringed version can be obtained
    using DRV_PMP_VersionStrGet().
...
// *****************************************************************************
/* Function:
    char * DRV_PMP_VersionStrGet( const SYS_MODULE_INDEX drvIndex )

  Summary:
    Gets the PMP driver version in string format.

  Description:
    This function gets the PMP driver version. The version is returned as
    major.minor.path[type], where type is optional. The numerical version can
    be obtained using DRV_PMP_VersionGet().
