...
void __attribute__((always_inline)) _DRV_PMP_Master_Read_Task(void);
void __attribute__((always_inline)) _DRV_PMP_Master_Write_Task(void);
void _DRV_PMP_HardwareConfig (DRV_HANDLE);

PMP_MODULE_ID  _pmpID; //global module ID for task machine use
QUEUE_ELEMENT_OBJECT  PMPcommandBuffer[DRV_PMP_QUEUE_SIZE];                   //This contains the commands to send to the graphics driver
QUEUE_ELEMENT_OBJECT* PMPqueueIndex = &PMPcommandBuffer[0];                   //pointer to where the command Buffer is currently 
QUEUE_ELEMENT_OBJECT* PMPtaskIndex =  &PMPcommandBuffer[0];                   //pointer to where the command Buffer is currently
...
void __attribute__((always_inline)) _DRV_PMP_Master_Write_Task (void)
{
    ...
    switch(PMPtaskIndex->eTransferStatus)
    {
         ...
         case MASTER_8BIT_TRANSFER_IN_PROGRESS:                 
                PLIB_PMP_MasterSend(_pmpID, *((uint8_t*)&PMPtaskIndex->data.buffer + (3 - PMPtaskIndex->nTransfersDone)));
                break;
         ...
         case MASTER_16BIT_BUFFER_IN_PROGRESS:                 
                ...
                (uint16_t*)PMPtaskIndex->data.addressBuffer++;			
 				PMPtaskIndex->nTransfersDone++; 
                break;    
        default:
                break;
									...
									uint32_t nBytes)
{
   ...
   QUEUE_ELEMENT_OBJECT* index = PMPqueueIndex;
...
/* Function:
  ...
  Summary:
    Gets PMP driver version in numerical format.
...
unsigned int DRV_PMP_VersionGet( const SYS_MODULE_INDEX drvIndex )
{
    return( ( _DRV_PMP_VERSION_MAJOR * 10000 ) +
            ( _DRV_PMP_VERSION_MINOR * 100 ) +
            ( _DRV_PMP_VERSION_PATCH ) );
...
/* Function:
  ...
  Summary:
    Gets PMP driver version in string format.
...
char * DRV_PMP_VersionStrGet( const SYS_MODULE_INDEX drvIndex )
{
    return _DRV_PMP_VERSION_STR;

} /* DRV_PMP_VersionStrGet */
