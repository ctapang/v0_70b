/*******************************************************************************
  ...
  File Name:
    drv_spi_static_multi.c

  Summary:
    SPI Driver static multiple client implementation.
...
DRV_SPI_CLIENT_OBJ                  gDrvSPIClientObj[DRV_SPI_CLIENTS_NUMBER] ;
...
DRV_SPI_CLIENT_OBJ_HANDLE _DRV_SPI_ClientObjectAllocate( SYS_MODULE_INDEX drvIndex )
{
    /* Local Variables */
    DRV_SPI_CLIENT_OBJ_HANDLE hClientObj = ( DRV_SPI_CLIENT_OBJ_HANDLE ) 0;
    uint8_t i = 0;
    ...
    for (i = 0; i < DRV_SPI_CLIENTS_NUMBER ; i++)
    {
        ...
        if ( !( ( gDrvSPIClientObj[hClientObj].inUse == true ) ) )
        {
            gDrvSPIClientObj[hClientObj].driverObject = drvIndex;
            return hClientObj;
        }
    }
...
/* Function:
  ...
  Summary:
    Initializes hardware and data for the specified instance of the SPI module.
...
void _DRV_SPI_MAKE_NAME(Initialize)( const SYS_MODULE_INIT * const init )
{
    ...
    _DRV_SPI_OBJ( DRV_SPI_INDEX, numClients ) = 0;
...
/* Function:
  ...
  Description:
    ...
    hardware for driver modules).  Resets all of he internal data structures and
...
DRV_HANDLE _DRV_SPI_MAKE_NAME( Open )( const DRV_IO_INTENT ioIntent )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;

    /* Check for exclusive access */
    if( ( _DRV_SPI_OBJ( DRV_SPI_INDEX , IsExclusive == true )) ||
        ( _DRV_SPI_OBJ( DRV_SPI_INDEX , numClients > 0) && DRV_IO_ISEXCLUSIVE( ioIntent ) ) )
    {
    ...
    if( _DRV_SPI_OBJ( DRV_SPI_INDEX, numClients > DRV_SPI_CLIENTS_NUMBER ) )
    {
    ...
    clientObj = _DRV_SPI_ClientObjectAllocate( DRV_SPI_INDEX );

    /* Check for the client object */
    SYS_ASSERT( clientObj < DRV_SPI_CLIENTS_NUMBER, "Invalid Client Object" );

    gDrvSPIClientObj[clientObj].inUse = true;
    gDrvSPIClientObj[clientObj].driverObject = DRV_SPI_INDEX;

    /* Increment the client in case of Multi client support, otherwise remove
       the below statement */
    _DRV_SPI_OBJ(DRV_SPI_INDEX, numClients)++;
    ...
    gDrvSPIClientObj[clientObj].status = DRV_SPI_CLIENT_STATUS_READY;
...
/* Function:
  ...
  Description:
    This function closes an opened instance of a driver, invalidating the given
    handle.
...
void _DRV_SPI_MAKE_NAME( Close )( DRV_HANDLE handle )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj;
    ...
    gDrvSPIClientObj[clientObj].inUse = false ;

    /* To Do: OSAL - unlock Mutex */

    /* Update the Client Status */
    gDrvSPIClientObj[clientObj].status = DRV_SPI_CLIENT_STATUS_INVALID;
...
DRV_SPI_CLIENT_STATUS _DRV_SPI_MAKE_NAME( ClientStatus )( DRV_HANDLE handle )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;
    ...
    return( gDrvSPIClientObj[clientObj].status );
...
void _DRV_SPI_MAKE_NAME( CommunicationSetup )( DRV_HANDLE handle,
                                               const DRV_SPI_COMM_CONFIG * const config )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;
...
SPI_DATA_TYPE _DRV_SPI_MAKE_NAME( Get )( DRV_HANDLE handle )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;
    /* Get the Client object from the handle passed */
...
/* Function:
  ...
  Summary:
    Writes byte or word data to the SPI.

  Description:
    This function writes byte or word data to the SPI.
...
void _DRV_SPI_MAKE_NAME( Put )( DRV_HANDLE handle, const SPI_DATA_TYPE buffer )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;
...
unsigned int _DRV_SPI_MAKE_NAME( GetBuffer ) ( DRV_HANDLE handle, SPI_DATA_TYPE *buffer,
                                               const unsigned int numbytes )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;
...
/* Function:
  ...
  Summary:
    Writes buffered data to the SPI.

  Description:
    This function writes buffered data to the SPI.
...
unsigned int _DRV_SPI_MAKE_NAME( PutBuffer ) ( DRV_HANDLE handle, const SPI_DATA_TYPE *buffer,
                                               const unsigned int numbytes )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;
...
DRV_SPI_TRANSFER_STATUS _DRV_SPI_MAKE_NAME( TransferStatus )( DRV_HANDLE handle )
{
    /* Multi client variables are removed from single client builds. */
    DRV_SPI_CLIENT_OBJ_HANDLE clientObj = (DRV_SPI_CLIENT_OBJ_HANDLE) 0;
...
/* Function:
  ...
  Summary:
    Gets the PI driver version in string format.
