 ...
 * Prototypes
 ******************************************************************************/
static bool     _LinkReconfigure(TCPIP_MAC_HANDLE hMac);
...
bool                    DRV_ETHMAC_PIC32MACEventAcknowledge(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_EVENT macEvents);
TCPIP_MAC_EVENT         DRV_ETHMAC_PIC32MACEventPendingGet(TCPIP_MAC_HANDLE hMac);

static TCPIP_MAC_RES    DRV_ETHMAC_PIC32MACEventInit(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_EventF eventF, const void* eventParam, int intPri, int intSubPri);
static TCPIP_MAC_RES    DRV_ETHMAC_PIC32MACEventDeInit(TCPIP_MAC_HANDLE hMac);
...
static const TCPIP_MODULE_MAC_PIC32INT_CONFIG macPIC32INTConfigDefault = 
{
...
static __inline__ int __attribute__((always_inline)) _PIC32MacIdToIx(TCPIP_STACK_MODULE macId)
{
...
extern __inline__ void __attribute__((always_inline)) _DRV_ETHMAC_DataLineFlush(void* address)
...
void _DRV_ETHMAC_CacheInvalidate(DRV_ETHMAC_INSTANCE_DCPT* pMacD)
{
    _DRV_ETHMAC_DataLineFlush(&pMacD->mData._EnetTxFreeList);
    _DRV_ETHMAC_DataLineFlush(&pMacD->mData._EnetTxBusyList);
    _DRV_ETHMAC_DataLineFlush(&pMacD->mData._EnetRxFreeList);
    _DRV_ETHMAC_DataLineFlush(&pMacD->mData._EnetRxBusyList);
...
/****************************************************************************
 * Function:        DRV_ETHMAC_PIC32MACSetup
 *
 * PreCondition:    None
 *
 * Input:           None
...

{
    ...
    int		    ix;
	ETH_RESULT_CODE		ethRes, phyInitRes;
    ...
    uint16_t* pHdrSpace;
    SINGLE_LIST    _RxQueue;           // stores RX allocated packets 
    SYS_OBJ_HANDLE hPhySysObject;
    DRV_HANDLE     hPhyClient;
    DRV_ETHPHY_INIT sPhyInit;


    macIx = _PIC32MacIdToIx(stackData->moduleId);
    ...
    pMacD->mData._macFlags._linkPrev = ETH_LINK_ST_DOWN;
    ...
    pMacD->mData._callocF = (_TCPIP_HEAP_CALLOC_PTR)stackData->callocF;
    pMacD->mData._freeF = (_TCPIP_HEAP_FREE_PTR)stackData->freeF;


    pMacD->mData.pktAllocF = stackData->pktAllocF;
    pMacD->mData.pktFreeF = stackData->pktFreeF;
    pMacD->mData.pktAckF = stackData->pktAckF;

    // adjust size of the TX buffer
    pMacD->mData._nTxDescriptors = initData->nTxDescriptors;
    if(initData->rxBuffSize < ETH_PIC32_INT_MAC_MIN_RX_SIZE)
    {
        pMacD->mData._RxBuffSize = ETH_PIC32_INT_MAC_MIN_RX_SIZE;  
    }
    else
    {
        pMacD->mData._RxBuffSize = ((initData->rxBuffSize + 15) / 16) * 16;   // adjust RX size
    }
    pMacD->mData._nRxDescriptors = initData->nRxDescriptors;

#if (EMAC_RX_FRAGMENTS == 1)
    // there's only one descriptor
    pMacD->mData.rxPktDcpt[0].next = 0;
#endif  // (EMAC_RX_FRAGMENTS == 1)

    initRes = TCPIP_MAC_RES_OK;

    TCPIP_Helper_SingleListInitialize(&_RxQueue);
    
	while(1)
	{
		ETH_OPEN_FLAGS	oFlags, linkFlags;
		ETH_PAUSE_TYPE pauseType;
		ETHPHY_CONFIG_FLAGS cfgFlags;
        ...
        PLIB_ETH_MACSetMaxFrame(macIx, EMAC_RX_MAX_FRAME);


        sPhyInit.moduleInit.value = 0;
        sPhyInit.ethphyId = phyIx;
        sPhyInit.MyPHYMIIConfigure  = &DRV_EXTPHY_MIIConfigure;
        sPhyInit.MyPHYMDIXConfigure = &DRV_EXTPHY_MDIXConfigure;
        sPhyInit.MyPHYSMIAddressGet = &DRV_EXTPHY_SMIAddressGet;
        sPhyInit.MyPHYSMIClockGet   = &DRV_EXTPHY_SMIClockGet;


        hPhySysObject = DRV_ETHPHY_Initialize(phyIx, (SYS_MODULE_INIT *)&sPhyInit);
        if ( SYS_OBJ_HANDLE_INVALID == hPhySysObject )
        {
            return TCPIP_MAC_RES_PHY_INIT_FAIL;
        }

        pMacD->mData.hPhySysObject = hPhySysObject;
        hPhyClient = DRV_ETHPHY_Open(phyIx, DRV_IO_INTENT_READWRITE);
        ...
        phyInitRes = DRV_ETHPHY_Setup(hPhyClient, oFlags, cfgFlags, &linkFlags);
        if ( ETH_RES_OK != phyInitRes )
        {
            initRes = TCPIP_MAC_RES_PHY_INIT_FAIL;
            break;
        }
        ...
        pRxPkt = 0;
		for(ix=0; ix < pMacD->mData._nRxDescriptors; ix++)
        {   // the _RxBuffSize is viewed as total packet size, including the ETH frame
            // the ETH frame header is added by the packet allocation
            pRxPkt = (*pMacD->mData.pktAllocF)(sizeof(*pRxPkt), pMacD->mData._RxBuffSize - sizeof(TCPIP_MAC_ETHERNET_HEADER), 0);
            if(pRxPkt == 0)
            {   // failed
            ...
            *pHdrSpace = (uint8_t*)pRSeg->segLoad - (uint8_t*)pRxPkt;
            ...
            TCPIP_Helper_SingleListTailAdd(&_RxQueue, (SGL_LIST_NODE*)pRxPkt);
        ...
        pMacD->mData._RxBuffSize = ((pRxPkt->pDSeg->segSize) / 16) * 16;   // set actual RX size
		...
		// set the MAC address
        memcpy(alignMacAddress.addr, stackData->ifPhyAddress.v, sizeof(alignMacAddress.addr));
		...
		{	// use the factory programmed address existent in the MAC
            ...
            *pS++=EMACxSA1;
            *pS=EMACxSA0;
            memcpy(stackData->ifPhyAddress.v, alignMacAddress.addr, sizeof(alignMacAddress.addr));
		}
        else
		...
		if(DRV_ETHMAC_LibDescriptorsPoolAdd(pMacD, pMacD->mData._nTxDescriptors, DRV_ETHMAC_DCPT_TYPE_TX, _MacCallocCallback, (void*)pMacD)!=pMacD->mData._nTxDescriptors)
		{
            initRes = TCPIP_MAC_RES_ALLOC_ERR;
            break;
		}

		if(DRV_ETHMAC_LibDescriptorsPoolAdd(pMacD, pMacD->mData._nRxDescriptors, DRV_ETHMAC_DCPT_TYPE_RX, _MacCallocCallback, (void*)pMacD)!=pMacD->mData._nRxDescriptors)
		{
            initRes = TCPIP_MAC_RES_ALLOC_ERR;
            break;
		}

        PLIB_ETH_RxSetBufferSize(macIx, pMacD->mData._RxBuffSize);

		// set the RX buffers as permanent receive buffers
        while( (pRxPkt = (TCPIP_MAC_PACKET*)TCPIP_Helper_SingleListHeadRemove(&_RxQueue)) != 0)
        {   
            pRxPkt->next = 0;
            ethRes = DRV_ETHMAC_LibRxBuffersAppend(pMacD, (void*)&pRxPkt->pDSeg->segLoad, 1, DRV_ETHMAC_BUFF_FLAG_RX_STICKY);
        ...
        {	// we'll just wait for the negotiation to be done
            ...
            _LinkReconfigure(pMacD);	// start the line negotiation
        }
        else
        {	// no need of negotiation results; just update the MAC
            DRV_ETHMAC_LibMACOpen(pMacD, linkFlags, pauseType);
        }

        pMacD->mData._linkUpTick = SYS_TICK_Get();     // the initialization time
        pMacD->mData._linkWaitTick = ((SYS_TICK_ResolutionGet() * ETH_CFG_LINK_INIT_DELAY) + 999 ) / 1000;

#if defined(TCPIP_STACK_USE_EVENT_NOTIFICATION)
        if(DRV_ETHMAC_PIC32MACEventInit(pMacD, stackData->eventF, stackData->eventParam, TCPIP_EVENT_IPL, TCPIP_EVENT_SIPL) != TCPIP_MAC_RES_OK)
        {
        ...
        // WOL configuration  for PHY - 
        DRV_EXTPHY_WOLConfiguration(hPhyClient,alignMacAddress.addr);
...
/****************************************************************************
 * Function:        DRV_ETHMAC_PIC32MACTeardown
 *
 * PreCondition:    None
 *
...
size_t DRV_ETHMAC_PIC32MACGetConfig(TCPIP_STACK_MODULE modId, void* configBuff, size_t buffSize, size_t* pConfigSize)
{
    ...
    if(macIx >= 0)
    {
        ...
        {   // can copy the data
            ...
            DRV_ETHMAC_INSTANCE_DCPT* pMacD = _pic32_emb_mac_dcpt + macIx;
...
/****************************************************************************
 * Function:        DRV_ETHMAC_PIC32MACOpen
 *
 * PreCondition:    DRV_ETHMAC_PIC32MACSetup has been called
 *
...
/****************************************************************************
 * Function:        DRV_ETHMAC_PIC32MACClose
 *
 * PreCondition:    DRV_ETHMAC_PIC32MACSetup has been called
 *
...
TCPIP_MAC_RES DRV_ETHMAC_PIC32MACPacketTx(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_PACKET * ptrPacket)
{
    ...
    {
        pSeg = pPkt->pDSeg;
        if(pSeg == 0 || ( (pSeg != 0) && (pSeg->segLoadOffset < 2)))
    ...
    while(ptrPacket)
    {
        TCPIP_Helper_SingleListTailAdd(&pMacD->mData._TxQueue, (SGL_LIST_NODE*)ptrPacket);
        ptrPacket->pktFlags |= TCPIP_MAC_PKT_FLAG_QUEUED;
        ptrPacket = ptrPacket->next;
    }
...
TCPIP_MAC_PACKET* DRV_ETHMAC_PIC32MACPacketRx (TCPIP_MAC_HANDLE hMac, TCPIP_MAC_RES* pRes, const TCPIP_MAC_PACKET_RX_STAT** ppPktStat)
{
    ...
    eRes = DRV_ETHMAC_LibRxGetPacket (pMacD, pRootDcpt, &buffsPerRxPkt, &pRxPktStat );

    if(eRes == ETH_RES_PACKET_QUEUED || eRes == ETH_RES_NO_PACKET)
    {   // done, no more packets
        mRes = TCPIP_MAC_RES_PENDING;
    }
    else if(eRes == ETH_RES_OK)
    {   // available packet; minimum check
        ...
        {   // corrupted packet; discrd/re-insert it
            ...
            pMacD->mData.rxBadPkts++;
        }
        else
        {
            mRes = TCPIP_MAC_RES_OK;
            pMacD->mData.rxOkPkts++;
        }
    }
    else
    {   // ETH_RES_RX_PKT_SPLIT_ERR; too many fragments 
        mRes = TCPIP_MAC_RES_FRAGMENT_ERR;
        pMacD->mData.rxFragError++;
    }
...
#if (EMAC_RX_FRAGMENTS > 1)
        for(pPrevPkt = 0, pCurrDcpt = pRootDcpt; pCurrDcpt != 0 && pCurrDcpt->pBuff != 0; pCurrDcpt = pCurrDcpt->next)
        {
...
/****************************************************************************
 * Function:        DRV_ETHMAC_PIC32MACLinkCheck
 *
 * PreCondition:    None
 *
 * Input:           None
...
bool DRV_ETHMAC_PIC32MACLinkCheck(TCPIP_MAC_HANDLE hMac)
{
    ...
    hPhy = pMacD->mData.hPhyClient;
    
    linkCurrUp = (DRV_ETHPHY_LinkStatusGet(hPhy, false) & ETH_LINK_ST_UP) != 0;
    ...
    {   // PHY state changed 
        ...
        {   // possibly link up now
            ...
            {   // wait the init tmo before checking the link up
                SYS_TICK currTick = SYS_TICK_Get();
            ...
            {	// if the auto-negotiation is turned on we have to re-negotiate
                ...
                linkCurrUp = _LinkReconfigure(hMac) ? true : false;	
            }
        }
        // else link down, not much we can do about it

        // update the new stat
        pMacD->mData._macFlags._linkPrev = linkCurrUp;
    }
    // else same old state
    // 
    if(linkCurrUp == false)
    ...
    return linkCurrUp == true;
}

    
/******************************************************************************
 * Function:        TCPIP_MAC_RES DRV_ETHMAC_PIC32MACRxFilterHashTableEntrySet(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_ADDR* DestMACAddr)
 *
 * PreCondition:    TCPIP_MAC_Initialize() should have been called.
 *
...
TCPIP_MAC_RES DRV_ETHMAC_PIC32MACProcess(TCPIP_MAC_HANDLE hMac)
{
    ...
    // TODO aa: add MACReplenishRx functionality optimization here !!!
    //
...
static TCPIP_MAC_RES _MACTxPacket(DRV_ETHMAC_INSTANCE_DCPT* pMacD, TCPIP_MAC_PACKET * ptrPacket)
{
    ...
    eRes = DRV_ETHMAC_LibTxSendPacket(pMacD, (const DRV_ETHMAC_PKT_DCPT*)ptrPacket->pDSeg);
    ...
    else if(eRes == ETH_RES_NO_DESCRIPTORS)
    {
...
static TCPIP_MAC_RES _MacTxPendingPackets(DRV_ETHMAC_INSTANCE_DCPT* pMacD)
{
    ...
    while( (pPkt = (TCPIP_MAC_PACKET*)(pMacD->mData._TxQueue.head)) != 0)
    {
        ...
        TCPIP_Helper_SingleListHeadRemove(&pMacD->mData._TxQueue);
...
static void* _MacCallocCallback( size_t nitems, size_t size, void* param )
{
    ...
    return _TCPIP_HEAP_CALLOC_BY_PTR(pMacD->mData._callocF, pMacD->mData._AllocH, nitems, size);
}
...
{
    DRV_ETHMAC_INSTANCE_DCPT* pMacD = (DRV_ETHMAC_INSTANCE_DCPT*)param;
    _TCPIP_HEAP_FREE_BY_PTR(pMacD->mData._freeF, pMacD->mData._AllocH, ptr);
}
...
/*********************************************************************
* Function:        bool _LinkReconfigure(DRV_HANDLE handle)

  Precondition:    None.

  Input:           handle - A valid open-instance handle, returned from the
                            driver's open routine

  Output:          True if negotiation succeeded and MAC was updated; otherwise,
                   false.

  Side Effects:    None.

  Overview:        Performs reconfiguration after auto-negotiation was performed.
...
{
    DRV_HANDLE        hPhyClient;
    ETH_OPEN_FLAGS    linkFlags;
    ETH_LINK_STATUS   linkStat;
    ETH_PAUSE_TYPE    pauseType;
    ETH_RESULT_CODE   phyRes;
    bool              success = false;

    DRV_ETHMAC_INSTANCE_DCPT *pMacD = (DRV_ETHMAC_INSTANCE_DCPT*)hMac;
    hPhyClient = pMacD->mData.hPhyClient;
    phyRes = DRV_ETHPHY_NegotiationIsComplete(hPhyClient, false);    // see if negotiation complete

    if( phyRes == ETH_RES_OK )
    {
        linkStat = DRV_ETHPHY_NegotiationResultGet(hPhyClient, &linkFlags, &pauseType);
        if( linkStat & ETH_LINK_ST_UP )
        {   // negotiation succeeded; properly update the MAC
            linkFlags |= (DRV_ETHPHY_HWConfigFlagsGet(hPhyClient) & ETH_PHY_CFG_RMII) ? ETH_OPEN_RMII : ETH_OPEN_MII ;
            DRV_ETHMAC_LibMACOpen(pMacD, linkFlags, pauseType);
            success = true;
        }
    }
...
static void _MACDeinit(DRV_ETHMAC_INSTANCE_DCPT* pMacD )
{
    ...
    {
        SYS_INT_SourceDisable(pMacD->mData._macIntSrc);      // stop Eth ints
        DRV_ETHMAC_LibClose(pMacD, ETH_CLOSE_DEFAULT);
        SYS_INT_SourceStatusClear(pMacD->mData._macIntSrc); // clear any pending interrupt flag
#if defined(TCPIP_STACK_USE_EVENT_NOTIFICATION)
        DRV_ETHMAC_PIC32MACEventDeInit(pMacD);
#endif

        if(pMacD->mData.hPhyClient != DRV_HANDLE_INVALID)
        {
            DRV_ETHPHY_Close(pMacD->mData.hPhyClient);
        }
        if(pMacD->mData.hPhySysObject != SYS_OBJ_HANDLE_INVALID)
        {
            DRV_ETHPHY_Deinitialize(pMacD->mData.hPhySysObject);
        }
        _MACCleanup(pMacD);
...
#if defined(ETH_PIC32_INT_MAC_ISR_TX)
    while( (pPkt = (TCPIP_MAC_PACKET*)TCPIP_Helper_SingleListHeadRemove(&pMacD->mData._TxAckQueue)) != 0)
    {   // acknowledge the packet
        (*pMacD->mData.pktAckF)(pPkt, ackRes);
    }
#endif  // defined(ETH_PIC32_INT_MAC_ISR_TX)

    while( (pPkt = (TCPIP_MAC_PACKET*)TCPIP_Helper_SingleListHeadRemove(&pMacD->mData._TxQueue)) != 0)
    {   // acknowledge the packet
        (*pMacD->mData.pktAckF)(pPkt, ackRes);
    }
}
...
static void _MACTxAcknowledgeAckQueue(DRV_ETHMAC_INSTANCE_DCPT* pMacD)
{
    ...
    {   // acknowledge the packet; the ackRes should be set
        (*pMacD->mData.pktAckF)(pPkt, TCPIP_MAC_PKT_ACK_NONE);
    }
...
static void _MacRxPacketAck(TCPIP_MAC_PACKET* pRxPkt,  const void* param)
{
    ...
    for(pSeg = pRxPkt->pDSeg; pSeg != 0; pSeg = pSeg->next)
    {
        if(pMacD->mData._macFlags._init == 0)
        {   // we're dead and gone
            // late acknowledged packets
            // extract packet the segment belongs to
            uint16_t buffOffset = *((uint16_t*)pSeg->segLoad - 1);
            TCPIP_MAC_PACKET* pCurrPkt = (TCPIP_MAC_PACKET*)((uint8_t*)pSeg->segLoad - buffOffset);
            (*pMacD->mData.pktFreeF)(pCurrPkt);
        }
        else
        {   // return it to the ETHC
            // RX pool (sticky buffers)
            // No further acknowledge/processing needed from MAC
            DRV_ETHMAC_LibRxAcknowledgeBuffer(pMacD, pSeg->segLoad, 0, 0);
            // pRxPkt->pktFlags &= ~TCPIP_MAC_PKT_FLAG_QUEUED; Cleared by packet acknowledge function
        }
    }
}
 ...
 ******************************************************************************/
/*static __inline__*/static  ETH_LEGACY_EVENTS /*__attribute__((always_inline))*/ _XtlEventsTcp2Eth(TCPIP_MAC_EVENT tcpEv)
{
    ETH_LEGACY_EVENTS  eEvents;
 ...
 ******************************************************************************/
/*static __inline__*/static  TCPIP_MAC_EVENT /*__attribute__((always_inline))*/ _XtlEventsEth2Tcp(ETH_LEGACY_EVENTS eEvents)
{
...
/*******************************************************************************
  Function:
    TCPIP_MAC_RES    DRV_ETHMAC_PIC32MACEventInit(TCPIP_MAC_HANDLE hMac, int intPri, int intSubPri)
  ...
  Example:
    <code>
    DRV_ETHMAC_PIC32MACEventInit( hMac, 4, 3 );
    </code>
...
#if defined (HARMONY_INT)
    SYS_INT_VectorPrioritySet(pMacD->mData._macIntSrc, (INT_PRIORITY_LEVEL_1-1)+intPri);
    SYS_INT_VectorSubprioritySet(pMacD->mData._macIntSrc, INT_SUB_PRIORITY_LEVEL_0+intSubPri);
#else
...
/*******************************************************************************
  Function:
    TCPIP_MAC_RES    DRV_ETHMAC_PIC32MACEventDeInit(TCPIP_MAC_HANDLE hMac )
...
*****************************************************************************/
bool DRV_ETHMAC_PIC32MACEventMaskSet(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_EVENT macEvMask, bool enable)
{
    DRV_ETHMAC_INSTANCE_DCPT*     pMacD = (DRV_ETHMAC_INSTANCE_DCPT*)hMac;
    ...
    if(enable)
    {
        ETH_LEGACY_EVENTS  ethSetEvents;
    ...
    {   // disable some events
        ETH_LEGACY_EVENTS  ethClrEvents;
        int         ethILev = 0;
...
static void TcpIpEthIsr(void* p)
{
    ETH_LEGACY_EVENTS          currEthEvents, currGroupEvents;
    DRV_ETHMAC_EVENT_DCPT* pDcpt;
    ...
    {   // locally acknowledge TX done activity
        EthTxAcknowledgePacket(0, _MACTxLocalAckCallback, pMacD);
        // schedule the next packet
        TCPIP_MAC_PACKET* pPkt;
        while((pPkt = (TCPIP_MAC_PACKET*)TCPIP_Helper_SingleListHeadRemove(&pMacD->mData._TxQueue)) != 0)
        {
            eEthRes eRes = EthTxSendPacket((const DRV_ETHMAC_PKT_DCPT*)pPkt->pDSeg);
            if(eRes == ETH_RES_NO_DESCRIPTORS)
            {   // no more descriptors
                break;
            }
            else if(eRes != ETH_RES_OK) 
            {   // it must be an error, ETH_RES_NO_DESCRIPTORS shouldn't happen
                pPkt->ackRes = TCPIP_MAC_PKT_ACK_BUFFER_ERR; 
                TCPIP_Helper_SingleListTailAdd(&pMacD->mData._TxAckQueue, (SGL_LIST_NODE*)pPkt);
            }
...
static void	_MACTxLocalAckCallback(void* pBuff, int buffIx, void* fParam)
{
    if(buffIx == 0)
    {
        ...
        TCPIP_Helper_SingleListTailAdd(&pMacD->mData._TxAckQueue, (SGL_LIST_NODE*)ptrPacket);
    }
}
#endif  // defined(ETH_PIC32_INT_MAC_ISR_TX)
