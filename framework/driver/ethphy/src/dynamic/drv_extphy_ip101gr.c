...
#if defined (__PIC32MZ__) && !defined(_ETH)
// TODO: workaround for missing _ETH in the processor header file!
#define _ETH
#endif
...
ETH_RESULT_CODE DRV_EXTPHY_MIIConfigure(DRV_HANDLE hClientObj,ETHPHY_CONFIG_FLAGS cFlags)
{
	...
	DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_PAGE_SEL, PHY_ADDRESS, PAGENUM_16);
	
	DRV_ETHPHY_SMIReadStart(hClientObj,PHY_REG_UTP_CONTROL_MODE, PHY_ADDRESS);
	phyReg=DRV_ETHPHY_SMIReadResultGet(hClientObj);
	
	if(cFlags&ETH_PHY_CFG_RMII)
	{
		phyReg|= _UTP_MODECTRL_RMII_V12_MASK |_UTP_MODECTRL_REPEATER_MODE_MASK;		// use RMII 1.2 and set the repeater
	}
	
	DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_UTP_CONTROL_MODE, PHY_ADDRESS, phyReg);	// update the RMII and Bypass Register
	
	DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_PAGE_SEL, PHY_ADDRESS, PAGENUM_0);
	return ETH_RES_OK;	
...
ETH_RESULT_CODE DRV_EXTPHY_MDIXConfigure(DRV_HANDLE hClientObj, ETH_OPEN_FLAGS oFlags)
{
    ...
    {   // enable Auto-MDIX
        ...
        DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_UTP_CONTROL_MODE, PHY_ADDRESS, phyReg);   
    }
    else
    {	// no Auto-MDIX It is a Forced MDIX
        ...
        DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_PAGE_SEL, PHY_ADDRESS, PAGENUM_16);  
        DRV_ETHPHY_SMIReadStart(hClientObj,PHY_REG_MDIX_CNTL_SPFC_STATUS, PHY_ADDRESS);
        phyReg=DRV_ETHPHY_SMIReadResultGet(hClientObj);
        ...
        DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_MDIX_CNTL_SPFC_STATUS, PHY_ADDRESS, phyReg);   
    }
 ...
 * Note:            None
 *****************************************************************************/
    unsigned int DRV_EXTPHY_SMIAddressGet(DRV_HANDLE handle)
{
	return PHY_ADDRESS;
}
...
{   
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_PAGE_SEL, PHY_ADDRESS, PAGENUM_4);
    
    DRV_ETHPHY_SMIReadStart(hClientObj,PHY_REG_WOL_CNTRL, PHY_ADDRESS);
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_WOL_CNTRL, PHY_ADDRESS, wol);
...
static void set_magic_mac(DRV_HANDLE hClientObj,uint8_t *bAddr)
{
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_PAGE_SEL, PHY_ADDRESS, PAGENUM_5);

    // due to little endian we don't need to convert this again.
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_WOL_MAC_ADDR, PHY_ADDRESS, macAddr45Byte);    
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_WOL_MAC_ADDR, PHY_ADDRESS, macAddr23Byte);    
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_WOL_MAC_ADDR, PHY_ADDRESS, macAddr01Byte);
...
static bool set_wol_intr(DRV_HANDLE hClientObj,int en)
{
    ...
    if (!wol_enabled(hClientObj))
        return false;
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_PAGE_SEL, PHY_ADDRESS, PAGENUM_17);
    DRV_ETHPHY_SMIReadStart(hClientObj,PHY_REG_WOL_STATUS, PHY_ADDRESS);
    wol=DRV_ETHPHY_SMIReadResultGet(hClientObj);
    if (en)
    {
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_WOL_STATUS, PHY_ADDRESS, wol);
...
static bool wol_intr32_pin_enabled(DRV_HANDLE hClientObj)
{
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_PAGE_SEL, PHY_ADDRESS, PAGENUM_16);
    DRV_ETHPHY_SMIReadStart(hClientObj,PHY_REG_INTR_STATUS, PHY_ADDRESS);
    intr=DRV_ETHPHY_SMIReadResultGet(hClientObj);
...
static void set_wol_intr32_pin(DRV_HANDLE hClientObj,int en)
{
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_INTR_STATUS, PHY_ADDRESS, intr);
...
{   
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj,PHY_REG_WOL_CNTRL, PHY_ADDRESS, (wol|_WOL_PLUS_MANUAL_SET_MASK));
...
#else
void DRV_EXTPHY_IntInit(DRV_HANDLE hClientObj,SYS_INT_SOURCE src,int intPri, int intSubPri)
#endif
{
    ...
    SYS_INT_DynamicRegister(src, IP101GRWOLIsr,&hClientObj);
#endif
