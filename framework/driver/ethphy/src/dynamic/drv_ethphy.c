...
static inline unsigned short _PhyReadReg( DRV_HANDLE hClientObj,
                                          unsigned int rIx,
                                          unsigned int phyAdd )
{
    DRV_ETHPHY_SMIReadStart(hClientObj, rIx, phyAdd);
...
static int _PhyDetectReset(DRV_HANDLE hClientObj)
{
    ...
    bmcon.w = _PhyReadReg(hClientObj, PHY_REG_BMCON, _PhyAdd);        // read the BMCON register

    if(bmcon.RESET)
    {   // that is already suspicios...but give it a chance to clear itself
        tWaitReset = SYS_TICK_Get() + ((PHY_RESET_CLR_TMO * SYS_TICK_ResolutionGet()) + 999)/1000;
        do
        {
            bmcon.w = _PhyReadReg(hClientObj, PHY_REG_BMCON, _PhyAdd);
        }while(bmcon.RESET && SYS_TICK_Get() < tWaitReset); // wait reset self clear
    ...
    DRV_ETHPHY_SMIWriteStart(hClientObj, PHY_REG_BMCON, _PhyAdd, _BMCON_LOOPBACK_MASK|_BMCON_ISOLATE_MASK);    // write control bits
    bmcon.w = _PhyReadReg(hClientObj, PHY_REG_BMCON, _PhyAdd);        // read back
    if( (bmcon.LOOPBACK == 0) || (bmcon.ISOLATE == 0) )
    {   // failed to set
        return 0;
    }
    bmcon.w ^=_BMCON_LOOPBACK_MASK|_BMCON_ISOLATE_MASK;
    DRV_ETHPHY_SMIWriteStart(hClientObj, PHY_REG_BMCON, _PhyAdd, bmcon.w);     // clear bits and write
    bmcon.w = _PhyReadReg(hClientObj, PHY_REG_BMCON, _PhyAdd);        // read back
...
// *****************************************************************************
// *****************************************************************************
// Section: PHY Driver Functions - ISP vs. Unified Stack
// *****************************************************************************
// *****************************************************************************
/*
             ISP Function                  Unified Stack Function
    ===============================     ============================
    DRV_ETHPHY_ClientStatus             -none-
    DRV_ETHPHY_Close                    -none-
    DRV_ETHPHY_Deinitialize             -none-
    DRV_ETHPHY_HWConfigFlagsGet         EthPhyGetHwConfigFlags
    DRV_ETHPHY_Setup                    EthPhyInit
    DRV_ETHPHY_Initialize               -none-
    DRV_ETHPHY_LinkScanRead             EthPhyScanLinkRead
    DRV_ETHPHY_LinkScanStart            EthPhyScanLinkStart
    DRV_ETHPHY_LinkScanStop             EthPhyScanLinkStop
    DRV_ETHPHY_LinkStatusGet            EthPhyGetLinkStatus
    DRV_ETHPHY_NegotiationIsComplete    EthPhyNegotiationComplete
    DRV_ETHPHY_NegotiationResultGet     EthPhyGetNegotiationResult
    DRV_ETHPHY_Open                     -none-
    DRV_ETHPHY_Reinitialize             -none-
    DRV_ETHPHY_Reset                    EthPhyReset
    DRV_ETHPHY_RestartNegotiation       EthPhyRestartNegotiation
    DRV_ETHPHY_SMIReadResultGet         EthMIIMReadResult
    DRV_ETHPHY_SMIReadStart             EthMIIMReadStart
    DRV_ETHPHY_SMIScanDataGet           EthMIIMScanResult
    DRV_ETHPHY_SMIScanStart             EthMIIMScanStart
    DRV_ETHPHY_SMIScanStatusGet         -none-
    DRV_ETHPHY_SMIScanStop              -none-
    DRV_ETHPHY_SMIWriteStart            EthMIIMWriteStart
    DRV_ETHPHY_SMIisBusy                -none-
    DRV_ETHPHY_Status                   -none-
    DRV_ETHPHY_Tasks                    -none-
    DRV_ETHPHY_VersionGet               -none-
    DRV_ETHPHY_VersionStrGet            -none-
    DRV_ETHPHY_SMIClockSet              EthMIIMConfig

     Unified Stack Function                       ISP Function
    =========================           ================================
    EthMIIMConfig                       DRV_ETHPHY_SMIClockSet
    EthMIIMReadResult                   DRV_ETHPHY_SMIReadResultGet
    EthMIIMReadStart                    DRV_ETHPHY_SMIReadStart
    EthMIIMScanResult                   DRV_ETHPHY_SMIScanDataGet
    EthMIIMScanStart                    DRV_ETHPHY_SMIScanStart
    EthMIIMWriteStart                   DRV_ETHPHY_SMIWriteStart
    EthPhyGetHwConfigFlags              DRV_ETHPHY_HWConfigFlagsGet
    EthPhyGetLinkStatus                 DRV_ETHPHY_LinkStatusGet
    EthPhyGetNegotiationResult          DRV_ETHPHY_NegotiationResultGet
    EthPhyInit                          DRV_ETHPHY_Setup
    EthPhyNegotiationComplete           DRV_ETHPHY_NegotiationIsComplete
    EthPhyReset                         DRV_ETHPHY_Reset
    EthPhyRestartNegotiation            DRV_ETHPHY_RestartNegotiation
    EthPhyScanLinkRead                  DRV_ETHPHY_LinkScanRead
    EthPhyScanLinkStart                 DRV_ETHPHY_LinkScanStart
    EthPhyScanLinkStop                  DRV_ETHPHY_LinkScanStop
    -none-                              DRV_ETHPHY_ClientStatus
    -none-                              DRV_ETHPHY_Close
    -none-                              DRV_ETHPHY_Deinitialize
    -none-                              DRV_ETHPHY_Initialize
    -none-                              DRV_ETHPHY_Open
    -none-                              DRV_ETHPHY_Reinitialize
    -none-                              DRV_ETHPHY_SMIScanStatusGet
    -none-                              DRV_ETHPHY_SMIScanStop
    -none-                              DRV_ETHPHY_SMIisBusy
    -none-                              DRV_ETHPHY_Status
    -none-                              DRV_ETHPHY_Tasks
    -none-                              DRV_ETHPHY_VersionGet
    -none-                              DRV_ETHPHY_VersionStrGet
                                      ...
                                      const SYS_MODULE_INIT   * const init )
{
    ...
    hSysObj->sExtPHYFunctions.MyPHYSMIAddressGet = ethphyInit->MyPHYSMIAddressGet;
...
/* Function:
  ...
  Precondition:
    EthInit (from legacy PLIB) should have been called to initialize the Ethernet MAC
                                  ...
                                  ETH_OPEN_FLAGS*  pResFlags )
{
    ...
    // Get PHY SMI address
    MyPHYSMIAddressGet = hDriver->sExtPHYFunctions.MyPHYSMIAddressGet;
    if ( MyPHYSMIAddressGet == NULL )
    {
    ...
    else
    {
        ...
        // TODO: add specific PIC32MZ configuration
        _PhyAdd = 0;
#else
        _PhyAdd = (*MyPHYSMIAddressGet)(hDriverClient); // get the PHY address
#endif
    }
    ...
    {
        DRV_ETHPHY_SMIClockSet(hDriverClient, SYS_CLK_ClockGet(), (*MyPHYSMIClockGet)(hDriverClient) );
    }
    ...
    phyCpbl   = _PhyReadReg(hDriverClient, PHY_REG_BMSTAT, _PhyAdd);
    ...
    {   // ok, we can perform auto negotiation
        ...
        DRV_ETHPHY_SMIWriteStart(hDriverClient, PHY_REG_ANAD, _PhyAdd, anadReg);      // advertise our capabilities
    ...
    {   // ok, just don't use negotiation
        ...
        DRV_ETHPHY_SMIWriteStart(hDriverClient, PHY_REG_BMCON, _PhyAdd, ctrlReg); // update the configuration
    }
                              ...
                              unsigned int phyAdd )
{
    ...
    PLIB_ETH_PHYAddressSet(ethphyId,phyAdd);
...
SMI_SCAN_DATA_STATUS DRV_ETHPHY_SMIScanStatusGet( DRV_HANDLE handle )
{
    ...
    SYS_ASSERT( NULL != hClientObj, "Bad client handle!" );
    ethphyId = hClientObj->ethphyId;
    return ( PLIB_ETH_DataNotValid(ethphyId) ? SMI_SCAN_DATA_NOTVALID : SMI_SCAN_DATA_VALID );
...
void DRV_ETHPHY_RestartNegotiation( DRV_HANDLE hClient )
{
    ...
    phyCpbl.w = _PhyReadReg(hClient, PHY_REG_BMSTAT, _PhyAdd);
...
ETH_RESULT_CODE DRV_ETHPHY_NegotiationIsComplete( DRV_HANDLE hClient, bool waitComplete )
{
    ...
    phyBMCon.w = _PhyReadReg(hClient, PHY_REG_BMCON, _PhyAdd);
    if(phyBMCon.AN_ENABLE)
    {   // just protect from an accidental call
        phyBMCon.w = _PhyReadReg(hClient, PHY_REG_BMCON,  _PhyAdd);
        phyStat.w  = _PhyReadReg(hClient, PHY_REG_BMSTAT, _PhyAdd);
        ...
        if(waitComplete)
        {
            ...
            {   // not started yet
                tWait = SYS_TICK_Get() + ((PHY_NEG_INIT_TMO * SYS_TICK_ResolutionGet()) + 999)/1000;
                do
                {
                    phyBMCon.w=_PhyReadReg(hClient, PHY_REG_BMCON, _PhyAdd);
                }while(phyBMCon.AN_RESTART && SYS_TICK_Get() < tWait);      // wait auto negotiation start
            }

            if(!phyBMCon.AN_RESTART)
            {   // ok, started
                tWait = SYS_TICK_Get() + ((PHY_NEG_DONE_TMO * SYS_TICK_ResolutionGet()) + 999)/1000;
                do
                {
                    phyStat.w=_PhyReadReg(hClient,PHY_REG_BMSTAT, _PhyAdd);
                }while(phyStat.AN_COMPLETE==0 && SYS_TICK_Get() < tWait);   // wait auto negotiation done
                                                 ...
                                                 ETH_PAUSE_TYPE* pPauseType )
{
    ...
    {   // we're up and running
        ...
        phyExp.w = _PhyReadReg(hClient, PHY_REG_ANEXP, _PhyAdd);
        if(phyExp.LP_AN_ABLE)
        {   // ok,valid auto negotiation info

            lpAD.w = _PhyReadReg(hClient, PHY_REG_ANLPAD, _PhyAdd);
            if(lpAD.REM_FAULT)
            {
                linkStat |= ETH_LINK_ST_REMOTE_FAULT;
            }
        ...
        anadReg.w  = _PhyReadReg(hClient, PHY_REG_ANAD, _PhyAdd);       // get our advertised capabilities
...
{
    DRV_ETHPHY_SMIWriteStart(hClient, PHY_REG_BMCON, _PhyAdd, _BMCON_RESET_MASK);       // Soft Reset the PHY
    ...
    {   // wait reset self clear
        ...
        do
        {
            bmcon.w=_PhyReadReg(hClient, PHY_REG_BMCON, _PhyAdd);
        }while(bmcon.RESET && SYS_TICK_Get() < tWaitReset);
...
{
    DRV_ETHPHY_SMIScanStart(hClient, PHY_REG_BMSTAT, _PhyAdd);
