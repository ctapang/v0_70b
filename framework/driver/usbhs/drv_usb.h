...
/* USB Driver Module Index Numbers
  ...
  Remarks:
    ...
    routines to  identify  the  driver  instance  in  use.   These  may  not  be
...
// *****************************************************************************
/* USB Driver Client Status Enumeration

  Summary:
    Identifies the client-specific status of the USB driver

  Description:
    This enumeration identifies the client-specific status of the USB driver.

  Remarks:
    These values are returned by the DRV_USB_ClientStatusGet() function. 
*/

typedef enum
{
    /* Client is closed, and available for reuse*/
    DRV_USB_CLIENT_CLOSED       /*DOM-IGNORE-BEGIN*/ = 0 /*DOM-IGNORE-END*/,
    
    /* Client up and running, no operations running */
    DRV_USB_CLIENT_READY        /*DOM-IGNORE-BEGIN*/ = 1 /*DOM-IGNORE-END*/,
    
    /* A client operation is currently in progress */
    DRV_USB_CLIENT_BUSY         /*DOM-IGNORE-BEGIN*/ = 2 /*DOM-IGNORE-END*/,

} DRV_USB_CLIENT_STATUS;
...
typedef struct
{
    ...
    USBHS_MODULE_ID     usbID;          // Identifies peripheral (PLIB-level) ID
    ...
    /* Endpoint table pointer. This is for compatibility with
     PICMX USB Driver and should be set to NULL. */
     void * endpointTable;
...
/* Function:
  ...
  Parameters:
    ...
    init     - Pointer to a data structure containing any data necessary to
               initialize the driver. This should be a DRV_USB_INIT structure
               reference type casted to SYS_MODULE_INIT reference.
  ...
  Remarks:
    ...
    unless DRV_USB_Deinitialize is called to de-initialize the driver  instance.
    If the driver is  built  for  static  mode,  then  the  initialization  data
    structure  is  ignored.   The  static  overrides  provided  in  system_config.h
...
/* Function:
  ...
  Description:
    ...
    Anyother  setting  of  the  intent  flag  will  return  a   invalid   driver
    handle.  A driver instance can only support  one  client.   Trying  to  open
    a driver that  has  an  existing  client  will  result  in  an  unsuccessful
    function call.
  ...
  Returns:
    ...
    with the driver. If an error occurs, the return value is DRV_HANDLE_INVALID.
...
/* Function:
  ...
  Summary:
    This function sets up the event callback function that is invoked by the
    USB controller driver to notify the a client of USB bus events.

  Description:
    This function sets up the event callback function that is envoked by the USB
...
/* Function:
  ...
  Description:
    ...
    * The size parameter while receiving data from the host  must  be  an  exact
      ...
      case, the driver will return  a  USB_ERROR_IRP_SIZE_INVALID result. 
      If while processing the IRP, the driver receives less than maximum
      ...
      data received from the host. The IRP status is returned as 
      USB_DEVICE_IRP_STATUS_COMPLETED_SHORT.
    * If a ZLP needs to be sent to host, the IRP size should be specified as 0
      and flag should be set as USB_DEVICE_IRP_FLAG_DATA_COMPLETE.
    * If the IRP size is an exact multiple of endpoint size, the client can 
      request the driver to not send a ZLP, by specifying the 
      USB_DEVICE_IRP_FLAG_DATA_PENDING. This flag indicates that there is more
      data pending in this transfer.
    ...
    * If the size id greater than endpoint size, and the flag is specified as
      USB_DEVICE_IRP_FLAG_DATA_PENDING, the driver will send multiple of 
      endpoint size data.
...
void DRV_USB_HOST_PipeClose(DRV_USB_HOST_PIPE_HANDLE pipe);
void DRV_USB_HOST_BusResetControl(DRV_HANDLE hClient, bool control);
void DRV_USB_ResumeControl(DRV_HANDLE hClient, bool control);
void DRV_USB_BusSuspend(DRV_HANDLE hClient, bool control);
...
#include "driver/usb/drv_usb_mapping.h"
