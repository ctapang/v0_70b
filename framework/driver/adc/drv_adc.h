...
typedef enum {
    ...
    /* Begin sampling automaticaly after previous conversion */
...
typedef enum
{
    ...
    /* Client in an invalid (or un-opened) state */
...
/* Function:
  ...
  Description:
    This function initializes the ADC driver, making it ready for clients to
    open and use it.
  ...
  Remarks:
    ...
    unless DRV_ADC_Deinitialize is called to deinitialize the driver instance.
...
/* Function:
  ...
  Description:
    ...
    instance of the ADC module using the hardware initialization given data.
    It does not clear or reinitialize internal data structures (although it may
    change the value of a few appropriate data items necessary to manage the
    new hardware state).
...
/* Function:
  ...
  Precondition:
    The DRV_ADC_Initialize function must have been called for the specified ADC
    driver instance.
  ...
  Remarks:
    ...
    called by the system's Tasks function (SYS_Tasks) or by the apropriate raw
    ISR.
...
/* Function:
  ...
  Description:
    This function gets the client-specfic status of the ADC driver associated
...
/* Function:
  ...
  Description:
    This function reads only the most recenly converted sample data from the ADC 
    driver into the given buffer.  Only the data for a single set of samples 
    for the registerd inputs is copied to the caller's buffer.  If the buffer
    size is less than the size of a complete set of samples for the registerd
    inputs, no data is copied to the caller's buffer.  Also, no sample data is
    copied to the caller's buffer if no sample data is currently available.
  ...
  Remarks:
    ...
    This function does not remove any data from the driver's internal buffer 
