...
DRV_SDCARD_XFER_OBJECT      gDrvSDCARDTransferObj[ DRV_SDCARD_QUEUE_POOL_SIZE ];
...
DRV_SDCARD_QUEUE_OBJECT   	gDrvSDCARDQueueObj;
                                      ...
                                      const SYS_MODULE_INIT     * const init )
{
    ...
    dObj->queueHandle = _DRV_SDCARD_QueueInitialize ( );
...
void DRV_SDCARD_Tasks ( SYS_MODULE_OBJ object )
{
    ...
    switch ( dObj->taskState )
    {
        case DRV_SDCARD_TASK_OPEN_SPI:
            ...
            for SD card driver */
            dObj->spiClientHandle = DRV_SPI_Open ( dObj->spiIndex, DRV_IO_INTENT_EXCLUSIVE );
        ...
        case DRV_SDCARD_TASK_CHECK_DEVICE:
            ...
            {
                if ( !_DRV_SDCARD_IsQueueEmpty ( (DRV_SDCARD_QUEUE_HANDLE)&gDrvSDCARDQueueObj ) )
                {
    ...
    switch (dObj->cmdDetectState)
    {
        case DRV_SDCARD_INIT_START_INIT:
            ...
            PLIB_SPI_BaudRateSet( _DRV_SDCARD_PERIPHERAL_ID_GET( dObj->spiId ),
                            SYS_CLK_ClockFrequencyGet( CLK_PERIPHERAL ),
                                                    ...
                                                    uint8_t *buffer, uint32_t sectorCount )
{
    ...
    return _DRV_SDCARD_AddToQueue (  (DRV_SDCARD_QUEUE_HANDLE)&gDrvSDCARDQueueObj, 
            buffer, DRV_SDCARD_TRANSFER_WRITE, sectorCount, sector_addr );
}

SYS_FS_MEDIA_STATUS DRV_SDCARD_MediaStatusGet ( DRV_HANDLE handle )
{
	if ( gDrvSDCARDObj[0].isAttached == DRV_SDCARD_IS_ATTACHED )
	{
                                    ...
                                    uint32_t sector_addr, uint32_t sectorCount )
{
    ...
    return _DRV_SDCARD_AddToQueue ( (DRV_SDCARD_QUEUE_HANDLE)&gDrvSDCARDQueueObj, buffer, DRV_SDCARD_TRANSFER_READ,
...
// *****************************************************************************
/* Function:
    DRV_SDCARD_QUEUE_HANDLE _DRV_SDCARD_QueueInitialize ( void )
...
DRV_SDCARD_QUEUE_HANDLE _DRV_SDCARD_QueueInitialize ( void )
{
    gDrvSDCARDQueueObj.bufferPool =  &gDrvSDCARDTransferObj[0];

    gDrvSDCARDQueueObj.startIndex = 0;

    gDrvSDCARDQueueObj.endIndex = 0;

    gDrvSDCARDQueueObj.size = DRV_SDCARD_QUEUE_POOL_SIZE;

    return ( ( DRV_SDCARD_QUEUE_HANDLE ) &gDrvSDCARDQueueObj.startIndex );
}
