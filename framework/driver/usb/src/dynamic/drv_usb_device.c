...
void DRIVER _DRV_USB_DEVICE_Initialize(DRV_USB_OBJ * drvObj, SYS_MODULE_INDEX index)
{
		
    /* Enable the desired mode */

    PLIB_USB_OperatingModeSelect( drvObj->usbID, USB_OPMODE_DEVICE);

    /* Enable or disable the interrupts */
    PLIB_USB_AllInterruptEnable(drvObj->usbID, USB_INT_ALL,
                                    USB_ERR_INT_ALL, ~USB_OTG_INT_ALL);

}

void DRIVER DRV_USB_DEVICE_AddressSet(DRV_HANDLE handle, uint8_t address)
{
    /* Set the address */
    ...
    PLIB_USB_DeviceAddressSet( ((DRV_USB_CLIENT_OBJ *)handle)->usbID, address );

}

USB_SPEED DRIVER DRV_USB_DEVICE_CurrentSpeedGet(DRV_HANDLE handle)
{
    /* Return the speed */
    if(DRV_HANDLE_INVALID == handle)
    {
...
void DRIVER DRV_USB_DEVICE_Attach(DRV_HANDLE handle)
{
    ...
    PLIB_USB_Enable(((DRV_USB_CLIENT_OBJ *)handle)->usbID);
}

void DRIVER DRV_USB_DEVICE_Detach(DRV_HANDLE handle)
{
    /* Detach the device */
    if(DRV_HANDLE_INVALID == handle)
    {
    ...
    PLIB_USB_Disable(((DRV_USB_CLIENT_OBJ *)handle)->usbID);
...
USB_ERROR DRIVER DRV_USB_DEVICE_EndpointEnable
(
    DRV_HANDLE handle, 
...
)
{
    ...
    usbID        = ((DRV_USB_CLIENT_OBJ *)handle)->usbID;
...
USB_ERROR DRIVER DRV_USB_DEVICE_EndpointDisable
(
    DRV_HANDLE handle, 
    USB_ENDPOINT endpointAndDirection
)
...
bool DRIVER DRV_USB_DEVICE_EndpointIsEnabled(DRV_HANDLE client, 
                                        USB_ENDPOINT endpointAndDirection)
{
...
bool DRIVER DRV_USB_DEVICE_EndpointIsStalled(DRV_HANDLE client, 
                                        USB_ENDPOINT endpointAndDirection)
{
    /* Return the state of the endpoint */
...
)
{
    ...
    {
        /* This means the data is either less than
         * maxPacketSize or not a multiple of maxPacketSize */

        if(USB_DATA_DIRECTION_HOST_TO_DEVICE == direction)
        {
//            SYS_ASSERT(false, "IRP size in not correct");
//            return(USB_ERROR_IRP_SIZE_INVALID);
        }
        else
        {
        	/* Dont do anything if otherwise */
...
USB_ERROR DRIVER DRV_USB_DEVICE_IRPCancelAll
(
    DRV_HANDLE client,
    USB_ENDPOINT endpointAndDirection
)
{
    ...
    usbID        = ((DRV_USB_CLIENT_OBJ *)client)->usbID;
    hDriver      = ((DRV_USB_CLIENT_OBJ *)client)->hDriver;
...
USB_ERROR DRIVER DRV_USB_DEVICE_EndpointStall(DRV_HANDLE client,
                                    USB_ENDPOINT endpointAndDirection)
{
    int     direction;
    bool    interruptWasEnabled = false;
...
USB_ERROR DRIVER DRV_USB_DEVICE_EndpointStallClear(DRV_HANDLE client,
                                    USB_ENDPOINT endpointAndDirection)
{
    int     direction;
    uint8_t endpoint;
