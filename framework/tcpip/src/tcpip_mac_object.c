...
{
    return (*((const TCPIP_MAC_DCPT*)hMac)->pObj->TCPIP_MAC_PacketTx)(hMac, ptrPacket);
}

TCPIP_MAC_PACKET* TCPIP_MAC_PacketRx(TCPIP_MAC_HANDLE hMac, TCPIP_MAC_RES* pRes, const TCPIP_MAC_PACKET_RX_STAT** ppPktStat)
{
    return (*((const TCPIP_MAC_DCPT*)hMac)->pObj->TCPIP_MAC_PacketRx)(hMac, pRes, ppPktStat);
}

TCPIP_MAC_RES TCPIP_MAC_Process(TCPIP_MAC_HANDLE hMac)
{
    return (*((const TCPIP_MAC_DCPT*)hMac)->pObj->TCPIP_MAC_Process)(hMac);
}
