...
#if BIGINT_PROFILE
    SYS_TICK addBICounter = 0;
    SYS_TICK addBIROMCounter = 0;
    SYS_TICK subBICounter = 0;
    SYS_TICK subBIROMCounter = 0;
    SYS_TICK zeroBICounter = 0;
    SYS_TICK msbBICounter = 0;
    SYS_TICK mulBICounter = 0;
    SYS_TICK mulBIROMCounter = 0;
    SYS_TICK sqrBICounter = 0;
    SYS_TICK masBICounter = 0;
    SYS_TICK masBIROMCounter = 0;
    SYS_TICK copyBICounter = 0;
    
    #define    addBI()        {addBICounter -= SYS_TICK_Get(); _addBI(); addBICounter += SYS_TICK_Get();}
    #define    addBIROM()    {addBIROMCounter -= SYS_TICK_Get(); _addBIROM();addBIROMCounter += SYS_TICK_Get();}
    #define    subBI()        {subBICounter -= SYS_TICK_Get(); _subBI(); subBICounter += SYS_TICK_Get();}
    #define    subBIROM()    {subBIROMCounter -= SYS_TICK_Get(); _subBIROM(); subBIROMCounter += SYS_TICK_Get();}
    #define    zeroBI()    {zeroBICounter -= SYS_TICK_Get(); _zeroBI(); zeroBICounter += SYS_TICK_Get();}
    #define    msbBI()        {msbBICounter -= SYS_TICK_Get(); _msbBI(); msbBICounter += SYS_TICK_Get();}
    #define    mulBI()        {mulBICounter -= SYS_TICK_Get(); _mulBI(); mulBICounter += SYS_TICK_Get();}
    #define    mulBIROM()    {mulBIROMCounter -= SYS_TICK_Get(); _mulBIROM(); mulBIROMCounter += SYS_TICK_Get();}
    #define    sqrBI()        {sqrBICounter -= SYS_TICK_Get(); _sqrBI(); sqrBICounter += SYS_TICK_Get();}
    #define    masBI()        {masBICounter -= SYS_TICK_Get(); _masBI(); masBICounter += SYS_TICK_Get();}
    #define    masBIROM()    {masBIROMCounter -= SYS_TICK_Get(); _masBIROM(); masBIROMCounter += SYS_TICK_Get();}
    #define    copyBI()    {copyBICounter -= SYS_TICK_Get(); _copyBI(); copyBICounter += SYS_TICK_Get();}
#else
    #define    addBI()        _addBI()
