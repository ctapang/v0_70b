...
//	TCP_TIME_WAIT, state is not implemented
    ...
    TCP_CLIENT_WAIT_CONNECT,    // client socket waiting for conection
} TCP_STATE;
...
typedef struct
{
    ...
    TCPIP_MAC_DATA_SEGMENT  tcpSeg;     // for IPv6 we put the 
}TCP_V6_PACKET;
...
typedef struct
{
    ...
    TCP_PORT        	localSSLPort;			    // Local SSL port number (for listening sockets)
