...
void TCPIP_ANNOUNCE_Send(void)
{
    ...
    // create the socket
    announceSocket = TCPIP_UDP_ClientOpen(IP_ADDRESS_TYPE_IPV4, ANNOUNCE_PORT, 0);
    ...
    for(netIx = 0; netIx < announceIfs; netIx++)
    {
        ...
        if((announceRequestMask & (1 << netIx)) != 0)
        {
            ...
            if(TCPIP_STACK_NetworkIsUp(pNetIf) && TCPIP_STACK_NetworkIsLinked(pNetIf))
...
bool TCPIP_ANNOUNCE_Task(TCPIP_NET_IF * pNetIf)
{
	...
	switch(announceDcpt.sm)
	{
		case DISCOVERY_HOME:
			...
			s = TCPIP_UDP_ServerOpen(IP_ADDRESS_TYPE_IPV4, ANNOUNCE_PORT, 0);
            ...
            if(!TCPIP_UDP_RemoteBind(s, IP_ADDRESS_TYPE_IPV4, ANNOUNCE_PORT,  0))
            {
                TCPIP_UDP_Close(s);
                break;
            }
