...
static tcpipAsyncHandle     ndpDADAsyncHandle = 0;      // handle for the DAD task
static tcpipAsyncHandle     ndpRSAsyncHandle = 0;       // handle for the RS task
static tcpipAsyncHandle     ndpNUDAsyncHandle = 0;      // handle for the NUD task
                    ...
                    const void* ndpData)
{
    ...
    if(ndpDADAsyncHandle == 0)
    {
        ndpDADAsyncHandle = _TCPIPStackAsyncHandlerRegister(TCPIP_NDP_DupAddrDiscoveryTask, 0, NDP_TASK_TIMER_RATE);
    }
    if(ndpRSAsyncHandle == 0)
    {
        ndpRSAsyncHandle = _TCPIPStackAsyncHandlerRegister(TCPIP_NDP_RouterSolicitTask, 0, NDP_TASK_TIMER_RATE);
    }
    if(ndpNUDAsyncHandle == 0)
    {
        ndpNUDAsyncHandle = _TCPIPStackAsyncHandlerRegister(TCPIP_NDP_NborUnreachDetectTask, 0, NDP_TASK_TIMER_RATE);
    }
    
    if (ndpDADAsyncHandle == 0 || ndpRSAsyncHandle == 0 || ndpNUDAsyncHandle == 0)
    {
        _TCPIP_NDP_Cleanup();
        return false;
    }
...
/*****************************************************************************
  Function:
    void TCPIP_NDP_TimeoutHandler (SYS_TICK curSysTick)

  Summary:
    Timeout function for NDP tasks.

  Description:
    This function will be called by the system timer to indicate to the
    stack that NDP task functions should be called.
...
static void _TCPIP_NDP_Cleanup(void)
{
    if(ndpDADAsyncHandle)
    {
        _TCPIPStackAsyncHandlerDeRegister(ndpDADAsyncHandle);
        ndpDADAsyncHandle = 0;
    }
    if(ndpRSAsyncHandle)
    {
        _TCPIPStackAsyncHandlerDeRegister(ndpRSAsyncHandle);
        ndpRSAsyncHandle = 0;
    }
    if(ndpNUDAsyncHandle)
    {
        _TCPIPStackAsyncHandlerDeRegister(ndpNUDAsyncHandle);
        ndpNUDAsyncHandle = 0;
    }
...
/*****************************************************************************
  ...
  Remarks:
    None
  ***************************************************************************/
IPV6_HEAP_NDP_NC_ENTRY * TCPIP_NDP_NborEntryCreate (TCPIP_NET_IF * pNetIf, IPV6_ADDR * remoteIPAddr, TCPIP_MAC_ADDR * remoteMACAddr, unsigned char initialState, unsigned char routerFlag, IPV6_ADDR_STRUCT * preferredSource)
{
...
/*****************************************************************************
  ...
  Remarks:
    None
  ***************************************************************************/
IPV6_HEAP_NDP_DC_ENTRY * TCPIP_NDP_DestCacheEntryCreate (TCPIP_NET_IF * pNetIf, IPV6_ADDR * remoteIPAddress, unsigned long linkMTU, IPV6_HEAP_NDP_NC_ENTRY * neighbor)
{
                ...
                case DAD_STATE_TRANSMIT:
                    {
                        ...
                        if (pkt != NULL)
                        {
                            ...
                            gDuplicateAddrDetectState[i].timer = SYS_TICK_Get() + ((SYS_TICK_ResolutionGet() * TCPIP_IPV6_InterfaceConfigGet(gDuplicateAddrDetectState[i].netConfig)->retransmitTime) / 1000);
    ...
    for(netIx=0; netIx< nStackIfs; netIx++)
    {
        ...
        switch (pRs->state)
        {
            ...
            case RS_STATE_TRANSMIT:
                ...
                pRs->transmitAttempt++;
                pRs->timer = SYS_TICK_Get() + (SYS_TICK_ResolutionGet() * RTR_SOLICITATION_INTERVAL);
                pRs->state = RS_STATE_WAIT;
                break;
            case RS_STATE_WAIT:
                if ((long)(SYS_TICK_Get() - pRs->timer) > 0)
                {
                    if (pRs->transmitAttempt == MAX_RTR_SOLICITATIONS)
                    {
                        pRs->state = RS_STATE_INACTIVE;
                        ndpRSCount--;
                    }
...
/*****************************************************************************
  ...
  Remarks:
    None
  ***************************************************************************/
void * TCPIP_NDP_RemoteNodeFind (TCPIP_NET_IF * pNetIf, IPV6_ADDR * source, uint8_t type)
{
    void * nodePointer;
    IPV6_INTERFACE_CONFIG*  pIpv6Config;
...
void TCPIP_NDP_ReachabilitySet (TCPIP_NET_IF * pNetIf, IPV6_HEAP_NDP_NC_ENTRY * neighborPointer, NEIGHBOR_UNREACHABILITY_DETECT_STATE newState)
{
    ...
    {
        neighborPointer->nextNUDTime = SYS_TICK_Get() + (TCPIP_IPV6_InterfaceConfigGet(pNetIf)->reachableTime * SYS_TICK_ResolutionGet());
        neighborPointer->flags.bResolvingAddress = false;
    }
...
/*****************************************************************************
  ...
  Remarks:
    None
  ***************************************************************************/
IPV6_HEAP_NDP_NC_ENTRY * TCPIP_NDP_NextHopGet (TCPIP_NET_IF * pNetIf, IPV6_ADDR * address)
{
    IPV6_HEAP_NDP_NC_ENTRY * neighborPointer;
    IPV6_HEAP_NDP_DC_ENTRY * destinationPointer;
    IPV6_HEAP_NDP_DR_ENTRY * routerPointer;
        ...
        if (prefixInfo->dPreferredLifetime <= prefixInfo->dValidLifetime)
        {
            if ((localAddressPointer = TCPIP_NDP_PrefixFind (pNetIf, &prefixInfo->aPrefix, prefixInfo->vPrefixLen, false)) == NULL)
            {
                ...
                if (prefixInfo->dValidLifetime != 0)
                {
                    ...
                    localAddressPointer = TCPIP_IPV6_UnicastAddressAdd (pNetIf, &tempAddress, false);
...
/*****************************************************************************
  ...
  Remarks:
    None
  ***************************************************************************/
uint8_t TCPIP_NDP_PrefixOnLinkStatusGet (TCPIP_NET_IF * pNetIf, IPV6_ADDR * address)
{
    IPV6_HEAP_NDP_PL_ENTRY * prefixPointer;
                        ...
                        if (neighborPointer->unansweredProbes < MAX_UNICAST_SOLICIT)
                        {
                            ...
                            else
                            {
                                if (TCPIP_IPV6_TxIsPutReady(pkt, sizeof(NDP_OPTION_LLA)))
                                {
                                    ...
                                    neighborPointer->nextNUDTime = SYS_TICK_Get() + ((TCPIP_IPV6_InterfaceConfigGet(pNetIf)->retransmitTime * SYS_TICK_ResolutionGet()) / 1000);
                                }
                                else
                                {
                                    TCPIP_IPV6_PacketFree (pkt);
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (TCPIP_IPV6_TxIsPutReady(pkt, sizeof(NDP_OPTION_LLA)))
                                {
                                    ...
                                    neighborPointer->unansweredProbes++;
                                    neighborPointer->nextNUDTime = SYS_TICK_Get() + ((pIpv6Config->retransmitTime * SYS_TICK_ResolutionGet()) / 1000);
                                }
                                else
                                {
                                    TCPIP_IPV6_PacketFree (pkt);
