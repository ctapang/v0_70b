...
static tcpipAsyncHandle       ipv6InitTimerHandle = 0;      // Handle for the IPv6 initialization task timer
...
static tcpipAsyncHandle       ipv6TimerHandle = 0;          // Handle for the IP task timer
...
static SYS_TICK        ulaOperStartTick;
...
static void _TCPIP_IPV6_QueuedPacketTransmitTask (SINGLE_LIST* pList);

static void _TCPIP_IPV6_PacketEnqueue(IPV6_PACKET * pkt, SINGLE_LIST* pList, int queueLimit, int tmoSeconds);
...
static TCPIP_MAC_PACKET*    TCPIP_IPV6_MacPacketTxAllocate(uint16_t segLoadLen, TCPIP_MAC_PACKET_FLAGS flags);
...
static void                 TCPIP_IPV6_MacPacketTxAddSegments(IPV6_PACKET* ptrPacket, TCPIP_MAC_PACKET* pMacPkt);
...
bool TCPIP_IPV6_Initialize(const TCPIP_STACK_MODULE_CTRL* const pStackInit, const IPV6_MODULE_CONFIG* pIpv6Init)
{
    ...
    {   // 1st time we run
        ...
        TCPIP_Helper_SingleListInitialize (&mcastQueue);
        ipv6MemH = pStackInit->memH;
        TCPIP_Helper_SingleListInitialize (&ipv6QueuedPackets);
        ...
        {
            ipv6TimerHandle = _TCPIPStackAsyncHandlerRegister(TCPIP_IPV6_Task, 0, IPV6_TASK_PROCESS_RATE);
        }
        ...
        {
            ipv6InitTimerHandle = _TCPIPStackAsyncHandlerRegister(TCPIP_IPV6_InitializeTask, 0, IPV6_INIT_TASK_PROCESS_RATE);
        }
...
static void _TCPIP_IPV6_Cleanup(const void* memH)
{
    if(ipv6TimerHandle)
    {
        _TCPIPStackAsyncHandlerDeRegister(ipv6TimerHandle);
        ipv6TimerHandle = 0;
    }

    if(ipv6InitTimerHandle)
    {
        _TCPIPStackAsyncHandlerDeRegister(ipv6InitTimerHandle);
        ...
        if ((pNetIf->Flags.bIPv6InConfig == true) && (TCPIP_STACK_NetworkIsLinked(pNetIf)))
        {
            ...
            switch (pIpv6Config->initState)
            {
                case IPV6_INIT_STATE_INITIALIZE:
                    ...
                    localAddressPointer = TCPIP_IPV6_UnicastAddressAdd (pNetIf, &linkLocalAddress, false);
                    if (localAddressPointer == NULL)
                    {
                        pIpv6Config->initState = IPV6_INIT_STATE_FAIL;
                        break;
...
unsigned short TCPIP_IPV6_ArrayPutHelper (IPV6_PACKET * ptrPacket, const void * dataSource, uint8_t dataType, unsigned short len)
{
    ...
    while(len != 0)
    {
        ...
        { // allocate new segment
            allocSize = (len < IP_DEFAULT_ALLOCATION_BLOCK_SIZE) ? IP_DEFAULT_ALLOCATION_BLOCK_SIZE : len;
...
int TCPIP_IPV6_Flush (IPV6_PACKET * ptrPacket)
{
    ...
    else
    {
        ...
        switch (neighborPointer->reachabilityState)
        {
            ...
            case NDP_STATE_INCOMPLETE:
                    ...
                    _TCPIP_IPV6_PacketEnqueue(ptrPacket, &neighborPointer->queuedPackets, IPV6_QUEUE_NEIGHBOR_PACKET_LIMIT, 0);
                    return 0;
            default:
    ...
    {   // packet needs to be transmitted
        ...
        else
        {
            if(mcastPkt)
            {
                _TCPIP_IPV6_PacketEnqueue(ptrPacket, &mcastQueue, IPV6_QUEUE_MCAST_PACKET_LIMIT, IPV6_QUEUED_MCAST_PACKET_TIMEOUT);
            }
            else
            {
                _TCPIP_IPV6_PacketEnqueue(ptrPacket, &neighborPointer->queuedPackets, IPV6_QUEUE_NEIGHBOR_PACKET_LIMIT, 0);
...
bool TCPIP_IPV6_PacketTransmit (IPV6_PACKET * pkt)
{
    ...
    TCPIP_MAC_PACKET* pMacPkt = TCPIP_IPV6_MacPacketTxAllocate(ipv6PktLoad, 0); 
    if (pMacPkt == 0)
    {
        return false;
    }

    TCPIP_IPV6_MacPacketTxAddSegments(pkt, pMacPkt);

    // Write the Ethernet Header to the MAC packet
    TCPIP_IPV6_MacPacketTxPutHeader(pkt, pMacPkt, ETHERTYPE_IPV6);

    // Transmit the packet
    hMac = TCPIP_STACK_NetToMAC ((TCPIP_NET_IF*)pkt->netIfH);
    if(TCPIP_MAC_PacketTx(hMac, pMacPkt) < 0)
    {   // failed
...
bool TCPIP_IPV6_PacketTransmitInFragments (IPV6_PACKET * pkt, uint16_t mtu)
{
    ...
    do
    {
        ...
        TCPIP_MAC_PACKET* pMacPkt = TCPIP_IPV6_MacPacketTxAllocate(currentPayloadLen, 0); 
        if (pMacPkt == 0)
        {
...
// ipv6_manager.h
IPV6_ADDR_STRUCT * TCPIP_IPV6_AddressFind(TCPIP_NET_IF * pNetIf, IPV6_ADDR * addr, unsigned char listType)
{
...
/*****************************************************************************
  Function:
    void _TCPIP_IPV6_PacketEnqueue (IPV6_PACKET * pkt, SINGLE_LIST* pList, int queueLimit, int tmoSeconds)
...
static void _TCPIP_IPV6_PacketEnqueue(IPV6_PACKET * pkt, SINGLE_LIST* pList, int queueLimit, int tmoSeconds)
{
    ...
    if (ptrPacket == NULL)
    {
        ...
        {
            ptrPacket = (IPV6_PACKET *)TCPIP_Helper_SingleListHeadRemove (pList);
            if (ptrPacket->ackFnc)
            {
                (*ptrPacket->ackFnc)(ptrPacket, false, ptrPacket->ackParam);
            }
        }
        TCPIP_Helper_SingleListTailAdd (pList, (SGL_LIST_NODE *)pkt);
...
// ipv6_private.h
uint8_t TCPIP_IPV6_FragmentationHeaderProcess (TCPIP_NET_IF * pNetIf, IPV6_ADDR * remoteIP, IPV6_ADDR * localIP, uint8_t * nextHeader, uint16_t dataCount, uint16_t headerLen, TCPIP_MAC_PACKET* pRxPkt, uint16_t previousHeaderLen)
{
    ...
    if (ptrFragment == NULL)
    {
        ...
        MACGetArray(pRxPkt, ptrFragment->packet, headerLen);
    ...
    if (dataCount)
    {
        ...
        if (fragmentHeader.offsetM.bits.m)
        {
            ...
            MACGetArray(pRxPkt, ptrFragment->packet + headerLen + (fragmentHeader.offsetM.bits.fragmentOffset << 3), dataCount);
    ...
    if (ptrFragment->packetSize == ptrFragment->bytesInPacket)
    {
        ...
        tempReadPtr = MACSetReadPtr (pRxPkt, (TCPIP_MAC_PTR_TYPE)ptrFragment->packet);
        tempBaseReadPtr = MACSetBaseReadPtr (pRxPkt, (TCPIP_MAC_PTR_TYPE)ptrFragment->packet - sizeof (TCPIP_MAC_ETHERNET_HEADER));
            ...
            while (ptrRxFragment != NULL)
            {
                ...
                if ((char)(ptrRxFragment->secondsRemaining) == 0)
                {
                    if (ptrRxFragment->firstFragmentLength != 0)
                    {
                        ...
                        tempReadPtr = MACSetReadPtr (pRxPkt, (TCPIP_MAC_PTR_TYPE)ptrRxFragment->packet);
                        tempBaseReadPtr = MACSetBaseReadPtr (pRxPkt, (TCPIP_MAC_PTR_TYPE)ptrRxFragment->packet - sizeof (TCPIP_MAC_ETHERNET_HEADER));

                        // If we received the first fragment, send it and a Time Exceeded error message
                        TCPIP_IPV6_ErrorSend (pNetIf, pRxPkt, (IPV6_ADDR *)&ptrRxFragment->packet[IPV6_HEADER_OFFSET_DEST_ADDR], (IPV6_ADDR *)&ptrRxFragment->packet[IPV6_HEADER_OFFSET_SOURCE_ADDR], ICMPV6_ERR_TE_FRAG_ASSEMBLY_TIME_EXCEEDED, ICMPV6_ERROR_TIME_EXCEEDED, 0, ptrRxFragment->firstFragmentLength);
...
/*****************************************************************************
  Function:
    void _TCPIP_IPV6_QueuedPacketTransmitTask (SINGLE_LIST* pList)
...
static void _TCPIP_IPV6_QueuedPacketTransmitTask (SINGLE_LIST* pList)
{
    ...
    while (pList->nNodes != 0)
    {
        ...
        if ((long)(tick - queuedPacket->queuedPacketTimeout) > 0)
        {
            ...
            TCPIP_Helper_SingleListHeadRemove (pList);
...
// ipv6_private.h
unsigned char TCPIP_IPV6_ASCompareSourceAddresses(TCPIP_NET_IF * pNetIf, IPV6_ADDR_STRUCT * addressOne, IPV6_ADDR_STRUCT * addressTwo, IPV6_ADDR * dest, IPV6_ADDR_SEL_INDEX rule)
{
...
// ipv6_manager.h
void TCPIP_IPV6_Process (TCPIP_NET_IF * pNetIf, TCPIP_MAC_PACKET* pRxPkt)
{
    ...
    pRxPkt->pktFlags |= TCPIP_MAC_PKT_FLAG_IPV6;
    ...
    if (tempLocalIPv6Addr.v[0] == 0xFF)
    {
        ...
        if (TCPIP_IPV6_AddressIsSolicitedNodeMulticast (&tempLocalIPv6Addr))
        {
            ...
            if ((localAddressPointer = TCPIP_IPV6_SolicitedNodeMulticastAddressFind(pNetIf, &tempLocalIPv6Addr, IPV6_ADDR_TYPE_UNICAST)) != NULL)
            {
                addrType = IPV6_ADDR_TYPE_SOLICITED_NODE_MULTICAST;
            }
            else if ((localAddressPointer = TCPIP_IPV6_SolicitedNodeMulticastAddressFind(pNetIf, &tempLocalIPv6Addr, IPV6_ADDR_TYPE_UNICAST_TENTATIVE)) != NULL)
            {
        ...
        else
        {
            ...
            localAddressPointer = TCPIP_IPV6_AddressFind(pNetIf, &tempLocalIPv6Addr, IPV6_ADDR_TYPE_MULTICAST);
    ...
    else
    {
        ...
        localAddressPointer = TCPIP_IPV6_AddressFind (pNetIf, &tempLocalIPv6Addr, IPV6_ADDR_TYPE_UNICAST);
    ...
    if (localAddressPointer == NULL)
    {
        ...
        if ((localAddressPointer = TCPIP_IPV6_AddressFind (pNetIf, &tempLocalIPv6Addr, IPV6_ADDR_TYPE_UNICAST_TENTATIVE)) != NULL)
        {
            addrType = IPV6_ADDR_TYPE_UNICAST_TENTATIVE;
        }
        else
    ...
    while (cIPFrameType != IPV6_PROT_NONE)
    {
        switch (cIPFrameType)
        {
            ...
            case IPV6_PROT_FRAGMENTATION_HEADER:
                action = TCPIP_IPV6_FragmentationHeaderProcess (pNetIf, &tempRemoteIPv6Addr, &tempLocalIPv6Addr, &cIPFrameType, dataCount, extensionHeaderLen + sizeof (IPV6_HEADER), pRxPkt, headerLen);
            ...
            case IPV6_PROT_TCP:
                ...
                TCPIP_TCP_ProcessIPv6(pRxPkt, &tempLocalIPv6Addr, &tempRemoteIPv6Addr, dataCount, extensionHeaderLen);
#endif
                cIPFrameType = IPV6_PROT_NONE;
                action = 0;
                return;
            ...
            case IPV6_PROT_UDP:
                ...
                TCPIP_UDP_ProcessIPv6(pRxPkt, &tempLocalIPv6Addr, &tempRemoteIPv6Addr, dataCount, extensionHeaderLen);
                return;
#else
                action = 0;
                break;
            ...
            case IPV6_PROT_ICMPV6:
                ...
                TCPIP_ICMPV6_Process(pNetIf, pRxPkt, localAddressPointer, &tempLocalIPv6Addr, &tempRemoteIPv6Addr, dataCount, extensionHeaderLen, hopLimit, addrType);
                cIPFrameType = IPV6_PROT_NONE;
                action = 0;
                break;
            // Unknown header type
...
ParameterProblem:
                // Send ICMP Parameter Problem Code 1 and discard packet
                {
                    ...
                    TCPIP_IPV6_ErrorSend (pNetIf, pRxPkt, &tempLocalIPv6Addr, &tempRemoteIPv6Addr, ICMPV6_ERR_PP_UNRECOGNIZED_NEXT_HEADER, ICMPV6_ERROR_PARAMETER_PROBLEM, TCPIP_Helper_htonl((uint32_t)currentOffset), dataCount + extensionHeaderLen + sizeof (IPV6_HEADER));
                    cIPFrameType = IPV6_PROT_NONE;
                }
                break;
        }
        ...
        switch (action)
        {
            ...
            case IPV6_ACTION_DISCARD_PP_0:
                TCPIP_IPV6_ErrorSend (pNetIf, pRxPkt, &tempLocalIPv6Addr, &tempRemoteIPv6Addr, ICMPV6_ERR_PP_ERRONEOUS_HEADER, ICMPV6_ERROR_PARAMETER_PROBLEM, TCPIP_Helper_htonl((uint32_t)currentOffset), dataCount + extensionHeaderLen + sizeof (IPV6_HEADER));
                cIPFrameType = IPV6_PROT_NONE;
                break;
            ...
            case IPV6_ACTION_DISCARD_PP_2:
                TCPIP_IPV6_ErrorSend (pNetIf, pRxPkt, &tempLocalIPv6Addr, &tempRemoteIPv6Addr, ICMPV6_ERR_PP_UNRECOGNIZED_IPV6_OPTION, ICMPV6_ERROR_PARAMETER_PROBLEM, TCPIP_Helper_htonl((uint32_t)currentOffset), dataCount + extensionHeaderLen + sizeof (IPV6_HEADER));
                cIPFrameType = IPV6_PROT_NONE;
                break;
...
// ipv6_manager.h
void TCPIP_IPV6_ErrorSend (TCPIP_NET_IF * pNetIf, TCPIP_MAC_PACKET* pRxPkt, IPV6_ADDR * localIP, IPV6_ADDR * remoteIP, uint8_t code, uint8_t type, uint32_t additionalData, uint16_t packetLen)
{
    IPV6_PACKET * pkt;
...
{
    SNTP_RESULT sntpRes;
    SYS_TICK    lastUpdateTick, tStampWindow, currTick;
    HASH_SUM shaSum;
    ...
    switch(ulaState)
    {
        ...
        case TCPIP_IPV6_ULA_CONN_START:
            ...
            {
                sntpRes = TCPIP_SNTP_ConnectionParamSet(0, (ulaFlags & IPV6_ULA_FLAG_NTPV4) ? IP_ADDRESS_TYPE_IPV4 : IP_ADDRESS_TYPE_IPV6);
            }
            else
            {
            ...
            if(currTick - ulaOperStartTick < (SYS_TICK_ResolutionGet() * IPV6_ULA_NTP_ACCESS_TMO + 999)/1000)
            {   // can try again
                return;
            }
        ...
        case TCPIP_IPV6_ULA_TSTAMP_GET:
            ...
            if(sntpRes < 0 || tStampWindow > (SYS_TICK_ResolutionGet() * IPV6_ULA_NTP_VALID_WINDOW + 999)/1000)
            {   // failed
                if(currTick - ulaOperStartTick < (SYS_TICK_ResolutionGet() * IPV6_ULA_NTP_ACCESS_TMO + 999)/1000)
                {   // can try again
                    return;
    ...
    if(genRes == IPV6_ULA_RES_OK)
    {
        ...
        {
            TCPIP_IPV6_UnicastAddressAdd(ulaNetIf, &ulaAddress, (ulaFlags & IPV6_ULA_FLAG_SKIP_DAD) != 0);
        }
    }
    else
    {
...
static TCPIP_MAC_PACKET* TCPIP_IPV6_MacPacketTxAllocate(uint16_t segLoadLen, TCPIP_MAC_PACKET_FLAGS flags)
{
    ...
    if(pMacPkt != 0)
    {
        TCPIP_PKT_PacketAcknowledgeSet(pMacPkt, TCPIP_IPV6_MacPacketTxAck, 0);
    }
