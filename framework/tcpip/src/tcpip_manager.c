...
static void _TCPIP_STACK_TickHandler(SYS_TICK currSysTick);        // stack tick handler
...
static void* _NetConfigStringToBuffer(void** ppDstBuff, void* pSrcBuff, size_t* pDstSize, size_t* pCurrLen);
...
bool TCPIP_STACK_Initialize(const TCPIP_NETWORK_CONFIG* pUsrConfig, int nNets, const TCPIP_STACK_MODULE_CONFIG* pModConfig, int nModules)
{
    ...
    // (that don't need interface specific initialization)
        ...
        if(!initFail)
        {
            tcpip_stack_tickH = SYS_TICK_TimerCreate(_TCPIP_STACK_TickHandler);
            if(tcpip_stack_tickH)
            {
                sysRes = SYS_TICK_ResolutionGet();
                rateMs = ((sysRes * TCPIP_STACK_TICK_RATE) + 999 )/1000;    // round up
                stackTaskRate = (rateMs * 1000) / sysRes;
                SYS_TICK_TimerSetRate(tcpip_stack_tickH, rateMs);
            }
            else
            {
...
static void ProcessTCPIPMacRxEvents(TCPIP_NET_IF* pNetIf)
{
    ...
    {
        // get a new packet
        if((pRxPkt = TCPIP_MAC_PacketRx(pNetIf->hIfMac, 0, 0)) == 0)
        {   // no more RX packets, done
            return;
        }

        pMacHdr = (TCPIP_MAC_ETHERNET_HEADER*)pRxPkt->pMacLayer;

        // update the packet interface
        pRxPkt->pktIf = pNetIf;
        // get the packet type
...
/*******************************************************************************
  Function:
    void    _TCPIP_STACK_TickHandler(SYS_TICK currSysTick)
  ...
  Remarks:
    None
*****************************************************************************/
static void _TCPIP_STACK_TickHandler(SYS_TICK currSysTick)
{
        newTcpipTickAvlbl++;
}
...
/*********************************************************************
 * Function:        TCPIP_STACK_NetByAddress(IPV4_ADDR* pIpAddress)
 *
 * PreCondition:    TCPIP stack should have been initialized by
 *                    TCPIP_STACK_Initialize()
 *
...
TCPIP_STACK_MODULE  TCPIP_STACK_NetMACId(TCPIP_NET_HANDLE netH)
{
    ...
    if(pNetIf)
    {
        return TCPIP_STACK_NetMACIdGet(pNetIf);
    }
...
TCPIP_NETWORK_CONFIG*   TCPIP_STACK_NetConfigSet(void* configStoreBuff, void* netConfigBuff, size_t buffSize, size_t* pNeededSize)
{
    ...
    pNetConf->interface = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    strncpy(tempBuff, (char*)pNetStg->NetBIOSName, sizeof(tempBuff) - 1);
    pNetConf->hostName = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    TCPIP_Helper_MACAddressToString(&pNetStg->netMACAddr, tempBuff, sizeof(tempBuff) - 1);
    pNetConf->macAddr = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    TCPIP_Helper_IPAddressToString(&pNetStg->netIPAddr, tempBuff, sizeof(tempBuff) - 1);
    pNetConf->ipAddr = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    TCPIP_Helper_IPAddressToString(&pNetStg->netMask, tempBuff, sizeof(tempBuff) - 1);
    pNetConf->ipMask = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    TCPIP_Helper_IPAddressToString(&pNetStg->netGateway, tempBuff, sizeof(tempBuff) - 1);
    pNetConf->gateway = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    TCPIP_Helper_IPAddressToString(&pNetStg->PrimaryDNSServer, tempBuff, sizeof(tempBuff) - 1);
    pNetConf->priDNS = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    TCPIP_Helper_IPAddressToString(&pNetStg->SecondaryDNSServer, tempBuff, sizeof(tempBuff) - 1);
    pNetConf->secondDNS = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;

    strncpy(tempBuff, TCPIP_Helper_PowerModeToString(pNetStg->Flags.powerMode), sizeof(tempBuff) - 1);
    pNetConf->powerMode = _NetConfigStringToBuffer(&pDstBuff, tempBuff, &dstSize, &currLen);
    totLen += currLen;
...
static bool _InitNetConfig(const TCPIP_NETWORK_CONFIG* pUsrConfig, int nNets)
{
    ...
    //
    if(!TCPIP_STORAGE_Initialize(0) || (hStg = TCPIP_STORAGE_Open(0, true)) == TCPIP_STORAGE_HANDLE_INVALID)
    {
    ...
    for(ix =0, pNetConfig = tcpipNetIf; ix < nNets; ix++, pNetConfig++)
    {
        ...
        {   // the entry for this interface exists; load it
            TCPIP_STORAGE_EntryIndexRead(hStg, storageIx, pNetConfig);
...
static void TCPIP_STACK_StacktoMacCtrl(TCPIP_MAC_MODULE_CTRL* pMacCtrl, TCPIP_STACK_MODULE_CTRL* stackCtrlData)
{
    ...
    pMacCtrl->pktAllocF = TCPIP_PKT_PacketAlloc;
    pMacCtrl->pktFreeF = TCPIP_PKT_PacketFree;
    pMacCtrl->pktAckF = TCPIP_PKT_PacketAcknowledge;
