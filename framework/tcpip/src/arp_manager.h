...
/*****************************************************************************
  ...
  Remarks:
    ...
    ARP_MODULE_CONFIG initialization data) is not implemented for stack
    nit/deinit sequences.  To maintain the data after the stack is completely
    de-initialized would need a persistent heap that's not yet implemented.
    The selection cannot be changed by ifup since this operation does not
    carry ARP configuration parameters (arpDate == 0).
 */
bool  TCPIP_ARP_Initialize(const TCPIP_STACK_MODULE_CTRL* const stackData ,const ARP_MODULE_CONFIG* arpData);
...
/*****************************************************************************
  Function:
    bool  TCPIP_ARP_TaskIsPending(void)

  Summary:
    Returns true if an ARP task is pending, false otherwise.
  ...
  Returns:
    true  - ARP task is pending
    false - ARP task is not pending
...
/*****************************************************************************
  ...
  Summary:
    Performs ARP tasks from within TCPIP stack tasks.
...
/*****************************************************************************
  Function:
    TCPIP_ARP_RESULT TCPIP_ARP_Process(TCPIP_NET_IF* pIf)
