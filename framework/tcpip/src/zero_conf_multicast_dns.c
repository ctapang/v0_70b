          ...
          ,DNSDesc_t *pDNSdesc)
{
   ...
   TCPIP_UDP_Put(pDNSdesc->mDNS_socket, 0x00); 
   TCPIP_UDP_Put(pDNSdesc->mDNS_socket, record_type);      // Resource Record Type
   ...
   {
      TCPIP_UDP_Put(pDNSdesc->mDNS_socket, cFlush); 
      TCPIP_UDP_Put(pDNSdesc->mDNS_socket, 0x01);      // Class
   }
   else
   {
    ...
    TCPIP_UDP_Put(pDNSdesc->mDNS_socket, ttl.v[3]);      // Time To Live
    TCPIP_UDP_Put(pDNSdesc->mDNS_socket, ttl.v[2]);
    TCPIP_UDP_Put(pDNSdesc->mDNS_socket, ttl.v[1]);
    TCPIP_UDP_Put(pDNSdesc->mDNS_socket, ttl.v[0]);
   ...
   while (1)
   {
      ...
      if((substr_len & 0xC0) == 0xC0)   // b'11 at MSb indicates compression ptr
      {
         ...
         if (bFollowPtr)
         {
            ...
            len = _mDNSDeCompress(offset_in_ptr, pcString, bFollowPtr, cElement, cDepth,pDNSdesc);
      ...
      else
      {
         ...
         else
         {
            ...
            while (i--)
            {
               TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &tmp);
            }
         }
                      ...
                      ,DNSDesc_t       *pDNSdesc)
{
   ...
   // TYPE & CLASS
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.type.v[1]);
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.type.v[0]);
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.class.v[1]);
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.class.v[0]);
   ...
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ttl.v[3]);      // Time to live
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ttl.v[2]);
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ttl.v[1]);
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ttl.v[0]);
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.rdlength.v[1]);      // Response length
   TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.rdlength.v[0]);
   ...
   {
   case QTYPE_A:
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ip.v[0]); // Read out IP address
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ip.v[1]);
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ip.v[2]);
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.ip.v[3]);
   ...
   case QTYPE_SRV:
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.srv.priority.v[1]); // Put Priority, weight, port
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.srv.priority.v[0]);
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.srv.weight.v[1]);
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.srv.weight.v[0]);
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.srv.port.v[1]);
      TCPIP_UDP_Get(pDNSdesc->mDNS_socket, &res_rec.srv.port.v[0]);
...
/***************************************************************
  ...
  Description:
   This function is used as mDNS-Responder. On initialization of
    ...
    This function gets polled from TCPIP_MDNS_Process for every iteration.
                    ...
                    ,const DNS_CLIENT_MODULE_CONFIG* dnsData)
{
    ...
    TCPIP_MAC_RxFilterHashTableEntrySet(pNetIf->hIfMac, &mcast_addr);
...
bool TCPIP_MDNS_Process(TCPIP_NET_IF* pNetIf)
{
    DNSDesc_t *pDNSdesc;
