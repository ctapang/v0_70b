...
#include "tcpip/src/system/system_random_private.h"
 ...
 ********************************************************************/
#if defined(USE_ISP_DEBUG) && defined(__MPLAB_DEBUGGER_PIC32MXSK)
    #define SYS_ASSERT(test,message)   ((void)( (test) ? 0 : AssertToIDE(#test,message,__LINE__,__FILE__), 0))
#else
 ...
 ********************************************************************/
#if defined(USE_ISP_DEBUG) && defined(__MPLAB_DEBUGGER_PIC32MXSK)
    #define SYS_ERROR(level, message)   if( (level) < SYSTEM_CURRENT_ERROR_LEVEL) _SYS_ERROR(__LINE__, __FILE__, message);
#else
#if defined(SYS_DEBUG_ENABLE)
