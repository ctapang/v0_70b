 ...
 *
 ******************************************************************************
 * FileName:        drv_spi_sd.c
 * Dependencies:    drv_spi_sd.h
 *                  string.h
 *                  mdd_fs.h
 *                  sys_fs_config.h
...
#include "tcpip/src/system/fs/mdd_fs.h"
#include "tcpip/src/system/fs/fs_defs.h"
#include "tcpip/src/system/drivers/drv_spi.h"
#include "tcpip/src/system/drivers/drv_spi_sd.h"
...
static uint8_t MDD_SDSPI_ReadMedia(void);
static MMC_RESPONSE SendMMCCmd(uint8_t cmd, uint32_t address);
static uint8_t MDD_SDSPI_AsyncWriteTasks(ASYNC_IO* info);
static uint8_t MDD_SDSPI_AsyncReadTasks(ASYNC_IO* info);

#if defined __C30__
 static   void OpenSPIM ( unsigned int sync_mode);
 static   void CloseSPIM( void );
#endif
static void InitSPISlowMode(void);
...
/*********************************************************
  Function:
    uint16_t MDD_SDSPI_ReadSectorSize (void)
  Summary:
...
/*********************************************************
  Function:
    uint32_t MDD_SDSPI_ReadCapacity (void)
  Summary:
...
/*********************************************************
  Function:
    uint16_t MDD_SDSPI_InitIO (void)
  Summary:
...
/*********************************************************
  Function:
    uint8_t MDD_SDSPI_ShutdownMedia (void)
  Summary:
...
/*****************************************************************************
  Function:
    MMC_RESPONSE SendMMCCmd (uint8_t cmd, uint32_t address)
  Summary:
...
static MMC_RESPONSE SendMMCCmd(uint8_t cmd, uint32_t address)
{
    ...
    DRV_SPI_TxRx(SDMMC_SPI_CHN, &CmdPacket.cmd, sizeof(CmdPacket.cmd), NULL, 0);//Send Command
    DRV_SPI_TxRx(SDMMC_SPI_CHN, &CmdPacket.addr3, sizeof(CmdPacket.addr3), NULL, 0);//Most Significant Byte
    DRV_SPI_TxRx(SDMMC_SPI_CHN, &CmdPacket.addr2, sizeof(CmdPacket.addr2), NULL, 0);
    DRV_SPI_TxRx(SDMMC_SPI_CHN, &CmdPacket.addr1, sizeof(CmdPacket.addr1), NULL, 0);
    DRV_SPI_TxRx(SDMMC_SPI_CHN, &CmdPacket.addr0, sizeof(CmdPacket.addr0), NULL, 0); //Least Significant Byte
    DRV_SPI_TxRx(SDMMC_SPI_CHN, &CmdPacket.crc, sizeof(CmdPacket.crc), NULL, 0); //Send CRC
    ...
    {
        uint8_t tmp = 0xff;
        DRV_SPI_TxRx(SDMMC_SPI_CHN, &tmp, sizeof(tmp), NULL, 0); //Send 8 SPI clocks
    }
...
/*****************************************************************************
  Function:
    uint8_t MDD_SDSPI_SectorRead (uint32_t sector_addr, uint8_t * buffer)
  Summary:
...
/*****************************************************************************
  Function:
    uint8_t MDD_SDSPI_AsyncReadTasks(ASYNC_IO* info)
  Summary:
  ...
  Return Values:
    uint8_t - Returns a status byte indicating the current state of the read
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_READ_NEW_PACKET_READY:
            ...
            if(ioInfo.dwBytesRemaining != 0x00000000)
            {
                ...
                //processor type.
#if 1
                DRV_SPI_TxRx(SDMMC_SPI_CHN, NULL, 0, ioInfo.pBuffer, ioInfo.wNumBytes);
#else
                #if defined __C30__ || defined __C32__
                {
                    ...
                    uint8_t* localPointer = ioInfo.pBuffer;
                    uint16_t localCounter = ioInfo.wNumBytes;
            ...
            else
            {
                ...
                DRV_SPI_TxRx(SDMMC_SPI_CHN, &bData, 1, NULL, 0); //Send 8 SPI clocks
...
/*****************************************************************************
  Function:
    uint8_t MDD_SDSPI_AsyncWriteTasks(ASYNC_IO* info)
  Summary:
  ...
  Return Values:
    uint8_t - Returns a status byte indicating the current state of the write
            operation. The possible return values are:
...
static uint8_t MDD_SDSPI_AsyncWriteTasks(ASYNC_IO* info)
{
    ...
    static uint16_t blockCounter, crc = 0xffff;
    static uint32_t WriteTimeout;
    static uint8_t command, tmp = 0xff;
    ...
    switch(info->bStateVariable)
    {
        ...
        case ASYNC_WRITE_TRANSMIT_PACKET:
            ...
            if(blockCounter == MEDIA_BLOCK_SIZE)
            {
                ...
                {
                    tmp = DATA_START_MULTI_BLOCK_TOKEN;
                }
                else
                {
                    //Else must be a single block write
                    tmp = DATA_START_TOKEN;
                    DRV_SPI_TxRx(SDMMC_SPI_CHN, &tmp, sizeof(tmp), NULL, 0);
                }
            }
...
#if 1
            DRV_SPI_TxRx(SDMMC_SPI_CHN, ioInfo.pBuffer, ioInfo.wNumBytes, NULL, 0);
#else
            #if defined __C30__ || defined __C32__
            {
            ...
            if(blockCounter == 0)
            {
                ...
                DRV_SPI_TxRx(SDMMC_SPI_CHN, (uint8_t *)&crc, 2, NULL, 0);
        ...
        case ASYNC_WRITE_MEDIA_BUSY:
            ...
            {
                WriteTimeout--;
                DRV_SPI_TxRx(SDMMC_SPI_CHN, &tmp, 1, NULL, 0); //Send 8 SPI clocks  //Dummy read to gobble up a byte (ex: to ensure we meet NBR timing parameter)
                ...
                if(data_byte != 0x00)
                {
                    ...
                    if(ioInfo.dwBytesRemaining == 0)
                    {
                        ...
                        if(command == WRITE_MULTI_BLOCK)
                        {
                            ...
                            tmp = 0xff;
                            DRV_SPI_TxRx(SDMMC_SPI_CHN, &tmp, 1, NULL, 0); //Send 8 SPI clocks
        ...
        case ASYNC_STOP_TOKEN_SENT_WAIT_BUSY:
            ...
            if(WriteTimeout != 0)
            {
                ...
                if(data_byte != 0x00)
                {
                    ...
                    SD_CS = 1;          //De-select media
                    DRV_SPI_TxRx(SDMMC_SPI_CHN, &tmp, 1, NULL, 0); //Send 8 SPI clocks //NEC timing parameter clocking
                    info->bStateVariable = ASYNC_WRITE_COMPLETE;
                    return ASYNC_WRITE_COMPLETE;
                }
        ...
        case ASYNC_WRITE_ABORT:
            ...
            DRV_SPI_TxRx(SDMMC_SPI_CHN, &tmp, 1, NULL, 0); //Send 8 SPI clocks //After raising CS pin, media may not tri-state data out for 1 bit time.
            info->bStateVariable = ASYNC_WRITE_ERROR;
...
/*****************************************************************************
  Function:
    uint8_t MDD_SDSPI_SectorWrite (uint32_t sector_addr, uint8_t * buffer, uint8_t allowWriteToZero)
  Summary:
...
/*******************************************************************************
  Function:
    uint8_t MDD_SDSPI_WriteProtectState
  Summary:
...
uint8_t MDD_SDSPI_WriteProtectState(void)
{
    return(SD_WE);
}
...
/*****************************************************************************
  Function:
    uint8_t MDD_SDSPI_ReadMedia (void)
  Summary:
...
static uint8_t MDD_SDSPI_ReadMedia(void)
{
    uint8_t tmp = 0xff, data = 0;
    DRV_SPI_TxRx(SDMMC_SPI_CHN, &tmp, 1, &data, 1); // Note, 0xff seems to be mandatory to sent out
    return data;
}
...
static void OpenSPIM( unsigned int sync_mode)
{
    ...
    #ifdef __PIC32MX__
        #if 0 //(SYS_CLK_PeripheralClockGet() <= 20000000)
            SPIBRG = SPICalutateBRG(GetPeripheralClock(), 10000);
        #else
            SPIBRG = SPICalutateBRG(SYS_CLK_PeripheralClockGet(), SD_SPI_FREQUENCY);
        #endif
        SPICON1bits.CKP = 1;
        SPICON1bits.CKE = 0;
...
static void InitSPISlowMode(void)
{
    #if defined __C30__ || defined __C32__
        #ifdef __PIC32MX__
        // SPI Mode 0 - (clock idle at low, sample at idle to active change)
        DRV_SPI_Initialize(SDMMC_SPI_CHN, SPI_OPEN_MSTEN | SPI_OPEN_MODE8 | SPI_OPEN_SMP_END | SPI_OPEN_CKE_REV, SYS_CLK_PeripheralClockGet()/400000);
...
MEDIA_INFORMATION *  MDD_SDSPI_MediaInitialize(void)
{
    ...
    DRV_SPI_TxRx(SDMMC_SPI_CHN, CSDResponse, 10, NULL, 0);
    ...
    do
    {
        ...
        //due to possible application noise on the SCK line.
        SD_CS = 1;
        //Send some "extraneous" clock pulses.  If a previous
        //command was terminated before it completed normally,
        //the card might not have received the required clocking
        //following the transfer.
        DRV_SPI_TxRx(SDMMC_SPI_CHN, CSDResponse, 1, NULL, 0);  //Send 8 SPI clocks
    ...
    DRV_SPI_Initialize(SDMMC_SPI_CHN, SPI_OPEN_MSTEN | SPI_OPEN_MODE8 | SPI_OPEN_SMP_END | SPI_OPEN_CKE_REV, SYS_CLK_PeripheralClockGet()/SD_SPI_FREQUENCY);
    ...
    if(CSDResponse[0] & 0xC0)   //Check CSD_STRUCTURE field for v2+ struct device
    {
        ...
        MDD_SDSPI_finalLBA = ((uint32_t)(c_size + 1) * (uint16_t)(1024u)) - 1; //-1 on end is correction factor, since LBA = 0 is valid.
    }
    else //if(CSDResponse[0] & 0xC0)    //Check CSD_STRUCTURE field for v1 struct device
    {
        ...
        MDD_SDSPI_finalLBA = ((uint32_t)(c_size + 1) * (uint16_t)((uint16_t)1 << (c_size_mult + 2)) * block_len) - 1;   //-1 on end is correction factor, since LBA = 0 is valid.
    }
