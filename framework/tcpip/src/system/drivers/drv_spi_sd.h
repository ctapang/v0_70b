 ...
 *
 ******************************************************************************
 * FileName:        drv_spi_sd.h
 * Dependencies:    sys_fs_config.h
 *                  fs-defs.h
...
typedef union
{
    ...
    struct
    {
        ...
        #if defined __C30__
            uint8_t c30filler;     // Filler space (since bitwise declarations can't cross a WORD boundary)
...
#define DELAY_PRESCALER   (uint8_t)      8

// Description: An approximation of the number of cycles per delay loop of overhead
#define DELAY_OVERHEAD    (uint8_t)      5

// Desription: Media Response Delay Timeouts
#define NCR_TIMEOUT     (uint16_t)20        //Byte times before command response is expected (must be at least 8)
#define NAC_TIMEOUT     (uint32_t)0x40000  //SPI byte times we should wait when performing read operations (should be at least 100ms for SD cards)
#define WRITE_TIMEOUT   (uint32_t)0xA0000  //SPI byte times to wait before timing out when the media is performing a write operation (should be at least 250ms for SD cards).
...
typedef struct
{
    ...
    uint32_t dwBytesRemaining; //Should be initialized to the total number of bytes that you wish to read or write.  This value is allowed to be greater than a single block size of the media.
    uint8_t* pBuffer;          //Pointer to where the read/written bytes should be copied to/from.  May be updated between calls to the handler function.
...
uint8_t MDD_SDSPI_ShutdownMedia(void);
    ...
    extern uint8_t ReadByte( uint8_t* pBuffer, uint16_t index );
    extern uint16_t ReadWord( uint8_t* pBuffer, uint16_t index );
    extern uint32_t ReadDWord( uint8_t* pBuffer, uint16_t index );
#endif
