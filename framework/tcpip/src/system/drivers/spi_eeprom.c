...
/*********************************************************************
 * Function:        XEE_RESULT XEEInit(unsigned char speed)
 *
 * PreCondition:    None
 *
 * Input:           speed - not used (included for compatibility only)
 *
 * Output:          XEE_SUCCESS if success
 *
 * Side Effects:    None
 *
                        ...
                        uint32_t length)
{
    ...
    // Send address
    sUint32.Val = address;
    #if defined(MEDIA_EEPROM_TYPE_25LC1024)
    EEPROM_SSPBUF = sUint32.v[2];
    WaitForDataByte();
    Dummy = EEPROM_SSPBUF;
    #endif

    EEPROM_SSPBUF = sUint32.v[1];
    WaitForDataByte();
    Dummy = EEPROM_SSPBUF;

    EEPROM_SSPBUF = sUint32.v[0];
    WaitForDataByte();
    Dummy = EEPROM_SSPBUF;
...
#endif //#if defined(MEDIA_STORAGE_EEPROM)
