...
char * SYS_FS_mddfs_getcwd(char *path, int numbchars) { return FSgetcwd(path, numbchars);}
...
int32_t SYS_FS_IsLegalDirString(const char * buffer,int32_t len)
{
    ...
    for(i=0;i<len;i++)
    {
        ...
        // And .. is allowed.
        //if((*(buffer+i)=='.') && (*(buffer+i+1)!=SYS_FS_CHAR) && (*(buffer+i+1)!='.'))
...
int32_t SYS_FS_fgetname(int32_t filedes, uint8_t *buf, uint16_t len)
{
    ...
    if(strncmp(fs,"mdd",SYS_FS_MAX_DRIVE)==0)
    {
        ...
        desc=SYS_FS_Sys2LocDesc(filedes, "mdd");
        // TODO: Get file name with relative directory(based on CWD)
...
int32_t SYS_FS_chdir(const char *path)
{
    ...
    if (ret == RETURN_SUCCESS)
    {
        ...
        else            // Relative path
        {
            ...
            if (SYS_FS_sGcwd[pathlen-1] !=  SYS_FS_CHAR) {
