...
/*****************************************************************************/
    ...
    uint8_t    gFileSlotOpen[FS_MAX_FILES_OPEN];   // Array that indicates which elements of gFileArray are available for use
		...
		uint16_t lfnData[FS_MAX_FILES_OPEN][257];
	#endif
#endif
		...
		uint16_t recordSearchName[257];
		uint16_t recordFoundName[257];
		uint16_t recordSearchLength;
	#endif
	uint16_t fileFoundString[261];
#endif
...
typedef struct
{
   ...
   uint16_t LFN_Part2[6];    // File name part 2
   uint16_t LFN_Reserved2;	// Reserved for future use
   uint16_t LFN_Part3[2];     // File name part 3
}LFN_ENTRY;
...
#if defined (__C30__) || defined (__PIC32MX__)
    uint8_t ReadByte( uint8_t* pBuffer, uint16_t index );
    uint16_t ReadWord( uint8_t* pBuffer, uint16_t index );
    uint32_t ReadDWord( uint8_t* pBuffer, uint16_t index );
#endif
...
// Write functions
    ...
    uint8_t FILEallocate_new_cluster( FILEOBJ fo, uint8_t mode);
    uint8_t FAT_erase_cluster_chain (uint32_t cluster, DISK * dsk);
...
CETYPE FILEfind( FILEOBJ foDest, FILEOBJ foCompareTo, uint8_t cmd, uint8_t mode)
{
	...
	uint16_t *templfnPtr = (uint16_t *)foCompareTo -> utf16LFNptr;
...
/*********************************************************************************
  Function:
    int FSCreateMBR (uint32_t firstSector, uint32_t numSectors)
  Summary:
...
int FSCreateMBR (uint32_t firstSector, uint32_t numSectors)
{
    ...
    tempSector %= 63;
    CyHdSc |= tempSector + 1;
    gDataBuffer[447] = (uint8_t)((CyHdSc >> 16) & 0xFF);
    gDataBuffer[448] = (uint8_t)((CyHdSc >> 8) & 0xFF);
    gDataBuffer[449] = (uint8_t)((CyHdSc) & 0xFF);
    ...
    tempSector %= 63;
    CyHdSc |= tempSector + 1;
    gDataBuffer[451] = (uint8_t)((CyHdSc >> 16) & 0xFF);
    gDataBuffer[452] = (uint8_t)((CyHdSc >> 8) & 0xFF);
    gDataBuffer[453] = (uint8_t)((CyHdSc) & 0xFF);
...
int FSformat (char mode, long int serialNumber, char * volumeID)
{
    ...
    uint32_t    secCount, DataClusters, RootDirSectors;
    BootSec   BSec;
    DISK   d;
    DISK * disk = &d;
    uint16_t    j;
    ...
    switch (mode)
    {
        ...
        case 1:
            ...
            if ((disk->type == FAT12) || (disk->type == FAT16))
            {
                ...
                gDataBuffer[28] = (uint8_t)(disk->firsts & 0xFF);
                gDataBuffer[29] = (uint8_t)((disk->firsts / 0x100) & 0xFF);
                gDataBuffer[30] = (uint8_t)((disk->firsts / 0x10000) & 0xFF);
                gDataBuffer[31] = (uint8_t)((disk->firsts / 0x1000000) & 0xFF);
    
                // Total Sectors = same as sectors in the partition from MBR
                gDataBuffer[32] = (uint8_t)(secCount & 0xFF);
                gDataBuffer[33] = (uint8_t)((secCount / 0x100) & 0xFF);
                gDataBuffer[34] = (uint8_t)((secCount / 0x10000) & 0xFF);
                gDataBuffer[35] = (uint8_t)((secCount / 0x1000000) & 0xFF);
                ...
                gDataBuffer[39] = (uint8_t)(serialNumber & 0xFF);
                gDataBuffer[40] = (uint8_t)((serialNumber / 0x100) & 0xFF);
                gDataBuffer[41] = (uint8_t)((serialNumber / 0x10000) & 0xFF);
                gDataBuffer[42] = (uint8_t)((serialNumber / 0x1000000) & 0xFF);
            ...
            else //FAT32
            {
                ...
                gDataBuffer[67] = (uint8_t)(serialNumber & 0xFF);
                gDataBuffer[68] = (uint8_t)((serialNumber / 0x100) & 0xFF);
                gDataBuffer[69] = (uint8_t)((serialNumber / 0x10000) & 0xFF);
                gDataBuffer[70] = (uint8_t)((serialNumber / 0x1000000) & 0xFF);
...
/**********************************************************
  Function:
    uint8_t FAT_erase_cluster_chain (uint16_t cluster, DISK * dsk)
  Summary:
...
CETYPE CreateFileEntry(FILEOBJ fo, uint16_t *fHandle, uint8_t mode, bool createFirstCluster)
{
	...
	#if defined(SUPPORT_LFN)
	LFN_ENTRY *lfno;
	uint16_t   *templfnPtr = (uint16_t *)fo -> utf16LFNptr,*dest;
	uint16_t	tempString[MAX_UTF16_CHARS_IN_LFN_ENTRY];
	UINT16_VAL tempShift;
    bool    firstTime = TRUE;
    ...
    if(FindEmptyEntries(fo, fHandle) == FOUND)
    {
		...
		if(fo->utf16LFNlength)
		{
			...
			if(tempCalc1)
			{
				...
				while(tempCalc1)
				{
					tempString[(uint8_t)index++] = templfnPtr[fileNameLength - tempCalc1];
					tempCalc1--;
				}				 
				...
				{
					tempString[(uint8_t)index] = 0xFFFF;
				}				 
			}
			else
			{
			...
			while(numberOfFileEntries)
			{
        	    ...
        	    *fHandle = *fHandle + 1;
				...
				{
					if(reminder)
						dest = (uint16_t *)(fo -> utf16LFNptr + fileNameLength - reminder - 1);
					else
						dest = (uint16_t *)(fo -> utf16LFNptr + fileNameLength - MAX_UTF16_CHARS_IN_LFN_ENTRY - 1);
					firstTime = FALSE;
				}
			}
		}
...
uint8_t FindEmptyEntries(FILEOBJ fo, uint16_t *fHandle)
{
    ...
    uint8_t   amountfound,numberOfFileEntries;
    uint8_t   a = 0;
    uint16_t   bHandle = *fHandle;
    uint32_t b;
    DIRENTRY    dir;
...
bool Alias_LFN_Object(FILEOBJ fo)
{
	...
	templfnPtr = (uint16_t *)filePtr1.utf16LFNptr;
	...
	if(lfnIndex)
	{
		...
		for(;((index1 < fileNameLength) && (tempVariable < 3));index1++)
		{
			...
			index = (uint8_t)templfnPtr[index1];
	...
	for(index4 = 1;index4 < (uint32_t)10000000;index4++)
	{
	    filePtr1.cluster = 0;
	    filePtr1.ccls    = 0;
	    filePtr1.entry = 0;
		...
		{
			tempString[7] = index4 % (uint8_t)10 + '0';
			tempString[6] = (index4 % (uint8_t)100)/10 + '0';
...
void CacheTime (void)
{
    uint16_t    year, monthday, weekhour, minsec, c, result;
    ...
    // Put them in place.
    year        = (uint16_t)(d0 >> 24);
    monthday    = (uint16_t)(d0 >> 8);
    weekhour    = (uint16_t)((d0 & 0x0F) << 8);
    weekhour   |= (uint16_t)(t0 >> 24);
    minsec      = (uint16_t)(t0 >> 8);
...
/***********************************************
  ...
  Input:
    fo -  Pointer to file structure
  Return Values:
    uint32_t - Address of empty cluster
...
int FSfclose(FSFILE   *fo)
{
    ...
    #ifdef	SUPPORT_LFN
	SystemFree((unsigned char *)fo->utf16LFNptr);
    #endif
    SystemFree((unsigned char *)fo);
#else
...
/***************************************************************
  Function:
    int wFSrename (const uint16_t * fileName, FSFILE * fo)
  Summary:
...
/*********************************************************************
  Function:
    FSFILE * wFSfopen (const uint16_t * fileName, const char *mode)
  Summary:
...
FSFILE * FSfopen( const char * fileName, const char *mode )
{
...
#ifdef FS_DYNAMIC_MEM
    filePtr = (FILEOBJ) SystemMalloc(sizeof(FSFILE));
#else
    ...
    if( filePtr == NULL )
    {
        ...
        return NULL;      //no file structure slot available
			...
			filePtr -> utf16LFNptr = (uint16_t *)SystemMalloc(514);
		#else
    ...
    if( !FormatFileName(fileName, filePtr, 0) )
    {
        #ifdef FS_DYNAMIC_MEM
            #if defined(SUPPORT_LFN)
                SystemFree((unsigned char *)filePtr->utf16LFNptr);
            #endif
            SystemFree( (unsigned char *)filePtr );
        #else
...
int FSremove (const char * fileName)
{
    ...
    //Format the source string
	#if defined(SUPPORT_LFN)
		fo->utf16LFNptr = (uint16_t *)&tempArray[0];
	#endif
...
/*********************************************************************
  Function:
    int wFSremove (const uint16_t * fileName)
  Summary:
...
/***************************************************
  Function:
    uint8_t ReadWord(uint8_t * pBuffer, uint16_t index)
  Summary:
...
/****************************************************
  Function:
    uint8_t ReadDWord(uint8_t * pBuffer, uint16_t index)
  Summary:
...
size_t FSfwrite(const void *data_to_write, size_t size, size_t n, FSFILE *stream)
{
    ...
    uint8_t   *    src = (uint8_t *) data_to_write;
    DISK   *    dsk;                 // pointer to disk structure
    CETYPE      error = CE_GOOD;
    uint16_t        pos;
    uint32_t       l;                     // absolute lba of sector to load
    uint32_t       seek, filesize;
...
uint8_t FormatFileName( const char* fileName, FILEOBJ fptr, uint8_t mode)
{
	...
	#if defined(SUPPORT_LFN)
		uint16_t tempString[256];
		bool	AscciIndication = TRUE;
		count1 = 256;
	#else
		uint16_t	tempString[13];
		count1 = 12;
	#endif
	...
	if(utfModeFileName)
	{
		utf16Filename = (uint16_t *)fileName;
		fileNameLength = 0;
...
uint8_t FormatDirName (char * string,FILEOBJ fptr, uint8_t mode)
{
    ...
    if((NAME_LFN_TYPE == fileNameType) || (TRUE == supportLFN))
	{
        	...
        	return FALSE;
		#else
			fptr -> utf16LFNptr = (uint16_t *)string;
		
			if(utfModeFileName)
			{
				if(utf16Filename != (uint16_t *)string)
				{
			...
			{
				#ifdef FS_DYNAMIC_MEM
	    			uint16_t *tempAsciiLFN = (uint16_t *)SystemMalloc((temp + 1) * 2);
				#else
					uint16_t	tempAsciiLFN[temp + 1];
				#endif
				...
				#ifdef FS_DYNAMIC_MEM
	    			SystemFree((unsigned char *)tempAsciiLFN);
				#endif
				fptr -> utf16LFNptr[count1] = 0x0000;
			}
...
/****************************************************************************
  Function:
    uint16_t WriteFAT (DISK *dsk, uint32_t ccls, uint16_t value, uint8_t forceWrite)
  Summary:
...
{
    uint8_t i, q, c;
    uint32_t p, li, l, ClusterFailValue;
...
#ifdef SUPPORT_FAT32 // If FAT32 supported.
        case FAT32:
            p = (uint32_t)ccls *4;   // "p" is the position in "gFATBuffer" for corresponding cluster.
            q = 0;      // "q" not used for FAT32, only initialized to remove a warning
            break;
#endif
        case FAT12:
            p = (uint32_t) ccls * 3; // "p" is the position in "gFATBuffer" for corresponding cluster.
            q = p & 1;   // Odd or even?
            p >>= 1;
            break;
        case FAT16:
        default:
            p = (uint32_t) ccls *2;   // "p" is the position in "gFATBuffer" for corresponding cluster.
            q = 0;      // "q" not used for FAT16, only initialized to remove a warning
            break;
    }
...
/*************************************************************************
  Function:
    ...
    int chdirhelper (uint8_t mode, char * ramptr, char * romptr);
    // PIC18
    int chdirhelper (uint8_t mode, char * ramptr, const char * romptr);
  Summary:
...
int chdirhelper (uint8_t mode, char * ramptr, char * romptr)
{
	...
	#ifdef SUPPORT_LFN
		uint16_t *utf16path = (uint16_t *)ramptr;
	#endif
...
char * FSgetcwd (char * path, int numchars)
{
    ...
    uint16_t totalchars = (path == NULL) ? 10 : numchars;
    char * returnPointer;
    char * bufferEnd;
	...
	FSFILE cwdTemp;
	LFN_ENTRY *lfno;
	uint16_t *tempLFN = (uint16_t *)&tempDirectoryString[0];
	#endif
            ...
            (((unsigned char)entry->DIR_Name[0] == 0xE5) || (entry->DIR_Attr == ATTR_VOLUME) || (entry->DIR_Attr == ATTR_LONG_NAME))))
        {
	   	...
	   	else
	   	{
			...
			cwdptr->utf16LFNlength = tempCWD->utf16LFNlength;
	   		...
	   		tempCWD->utf16LFNptr = (uint16_t *)&tempDirectoryString[0];

	   		cwdptr->utf16LFNptr = (uint16_t *)&tempDirectoryString[0];
	...
	#if defined(SUPPORT_LFN)
    if (((tempCWD->name[0] == SYS_FS_CHAR) && (tempCWD->utf16LFNlength == 0x0000)) ||
		((tempCWD->utf16LFNlength != 0x0000) && (tempCWD->utf16LFNptr[0] == (uint16_t)SYS_FS_CHAR)) || (numchars == 0x02))
	#else
    if ((tempCWD->name[0] == SYS_FS_CHAR) || (numchars == 0x02))
	#endif
    {
        // Easy, our CWD is the root
        *returnPointer = SYS_FS_CHAR;
        *(returnPointer + 1) = 0;
    ...
    else
    {
        ...
        while (tempCWD->dirclus != FatRootDirClusterValue)
        {
            ...
            *(returnPointer + index++) = SYS_FS_CHAR;
            if (index == totalchars)
            {
                index = 0;
                bufferOverflow = TRUE;
...
/**************************************************************
  Function:
    char * wFSgetcwd (uint16_t * path, int numchars)
  Summary:
...

	{
...

   	{
...

		{
   		...
   		fo->utf16LFNptr = (uint16_t *)&tempDirectoryString[0];
...
/*************************************************************************
  Function:
    ...
    int mkdirhelper (uint8_t mode, char * ramptr, char * romptr)
    // PIC18
    int mkdirhelper (uint8_t mode, char * ramptr, const char * romptr)
  Summary:
...
// go with static allocation
...

	{
		...
		if(utfModeFileName)
		{
			utf16path = (uint16_t *)ramptr;
	        // Scan for too-long file names
	        while (1)
	        {
	            i = 0;
	            while((*utf16path != 0) && (*utf16path != '.')&& (*utf16path != SYS_FS_CHAR))
	            {
	                utf16path++;
	                i++;
	            }
	            ...
	            {
	                utf16path++;
	                while ((*utf16path != 0) && (*utf16path != SYS_FS_CHAR))
	                {
	                    utf16path++;
	                    j++;
	                }
		...
		#endif
		{
	        ...
	        {
	            i = 0;
	            while((*temppath != 0) && (*temppath != '.')&& (*temppath != SYS_FS_CHAR))
	            {
	                temppath++;
	                i++;
	            }
	            ...
	            {
	                temppath++;
	                while ((*temppath != 0) && (*temppath != SYS_FS_CHAR))
	                {
	                    temppath++;
	                    j++;
	                }
...
//				}
    ...
    while (1)
    {
		...
		if(utfModeFileName)
		{
			...
			uint16_t *tempPtr2;
			k = 0;
			tempPtr1 = (uint16_t *)&tempArray[0];
			tempPtr2 = (uint16_t *)&tempDirectoryString[0];
...
/***********************************************************************************
  Function:
    uint8_t writeDotEntries (DISK * disk, uint32_t dotAddress, uint32_t dotdotAddress)
  Summary:
...
uint8_t writeDotEntries (DISK * disk, uint32_t dotAddress, uint32_t dotdotAddress)
{
    ...
    entry.DIR_FstClusLO = (uint16_t)(dotAddress & 0x0000FFFF); // Lower 16 bit address

#ifdef SUPPORT_FAT32 // If FAT32 supported.
    entry.DIR_FstClusHI = (uint16_t)((dotAddress & 0x0FFF0000)>> 16); // Higher 16 bit address. FAT32 uses only 28 bits. Mask even higher nibble also.
#else // If FAT32 support not enabled
...
// Set dir date for uncontrolled clock source
    ...
    entry.DIR_FstClusLO = (uint16_t)(dotdotAddress & 0x0000FFFF); // Lower 16 bit address

#ifdef SUPPORT_FAT32 // If FAT32 supported.
    entry.DIR_FstClusHI = (uint16_t)((dotdotAddress & 0x0FFF0000)>> 16); // Higher 16 bit address. FAT32 uses only 28 bits. Mask even higher nibble also.
#else // If FAT32 support not enabled
    entry.DIR_FstClusHI = 0;
#endif
...
/**************************************************************************
  Function:
    int wFSrmdir (uint16_t * path, unsigned char rmsubdirs)
  Summary:
...
/************************************************************************************************
  Function:
    ...
    int rmdirhelper (uint8_t mode, char * ramptr, char * romptr, unsigned char rmsubdirs)
    // PIC18
    int rmdirhelper (uint8_t mode, char * ramptr, const char * romptr, unsigned char rmsubdirs)
  Summary:
...
int rmdirhelper (uint8_t mode, char * ramptr, char * romptr, unsigned char rmsubdirs)
{
    ...
    char Index, Index2;
    	...
    	FSFILE cwdTemp;
		UINT16_VAL tempShift;
		uint16_t *tempLFN = (uint16_t *)&tempArray[0];
		bool	forFirstTime;
	#else
    ...
    else
    {
        ...
        subDirDepth = 0;
		#if defined(SUPPORT_LFN)
		tempCWD-> utf16LFNptr = (uint16_t *)&tempArray[0];
		fo-> utf16LFNptr = (uint16_t *)&tempArray[0];
		#endif
            ...
            if (entry->DIR_Name[0] != 0)
            {
                ...
                else if (((unsigned char)entry->DIR_Name[0] != 0xE5) && (entry->DIR_Attr != ATTR_VOLUME) && (entry->DIR_Attr != ATTR_LONG_NAME))
                {
                    if ((entry->DIR_Attr & ATTR_DIRECTORY) == ATTR_DIRECTORY)
                    {
						...
						#endif
						{
                    	    for (Index = 0; (Index < DIR_NAMESIZE) && (entry->DIR_Name[(uint8_t)Index] != 0x20); Index++)
                    	    {
                    	        tempArray[(uint8_t)Index] = entry->DIR_Name[(uint8_t)Index];
                    	    }
                    	    if (entry->DIR_Extension[0] != 0x20)
                    	    {
                    	        tempArray[(uint8_t)Index++] = '.';
                    	        for (Index2 = 0; (Index2 < DIR_EXTENSION) && (entry->DIR_Extension[(uint8_t)Index2] != 0x20); Index2++)
                    	        {
                    	            tempArray[(uint8_t)Index++] = entry->DIR_Extension[(uint8_t)Index2];
                    	        }
                    	    }
                    	    tempArray[(uint8_t)Index] = 0;
							#ifdef SUPPORT_LFN
							utfModeFileName = 0;
							#endif
							Index = FSchdir(&tempArray[0]);
                    ...
                    else
                    {
						...
						#endif
						{
                    	    for (Index = 0; Index < 8; Index++)
                    	    {
                    	        fo->name[(uint8_t)Index] = entry->DIR_Name[(uint8_t)Index];
                    	    }

                    	    for (Index = 0; Index < 3; Index++)
                    	    {
                    	        fo->name[(uint8_t)Index + 8] = entry->DIR_Extension[(uint8_t)Index];
                    	    }
						}
	...
	if(utfModeFileName)
        ...
        Index3 = FSchdir ("..");
					...
					if(Index3 == 0)
					{
                	    ...
                	    for (Index = 0; Index < 3; Index++)
                	    {
                	        tempArray[(uint8_t)Index + 8] = entry->DIR_Extension[(uint8_t)Index];
                	    }
						#if defined(SUPPORT_LFN)
						cwdptr->utf16LFNlength = 0;
						#endif
					...
					else
					{
						...
						cwdptr->utf16LFNptr = (uint16_t *)&tempArray[0];
					}
					#endif
	...
	else
	{
		...
		#endif
			{
        		...
        		for (Index = 0; Index < 11; Index++)
        		{
            		tempArray[(uint8_t)Index] = cwdptr->name[(uint8_t)Index];
        		}			
			}
	}
...
/***********************************************************************************
  Function:
    int wFindFirst (const uint16_t * fileName, unsigned int attr, SearchRec * rec)
  Summary:
...
/**********************************************************************
  Function:
    ...
    // PIC18
    int FSfpritnf (FSFILE * fptr, const char * fmt, ...)
  Summary:
...
/**********************************************************************
  Function:
    ...
    int FSvfprintf (FSFILE * handle, const char * formatString, va_list ap)
    // PIC18
    int FSvfpritnf (auto FSFILE * handle, auto const char * formatString, auto va_list ap)
  Summary:
                ...
                case 'B':
                    ...
                    else if (size == _FMT_LONGLONG)
                    {
                        if (flags & _FLAG_SIGNED)
                            ...
                            larg = (uint64_t) va_arg (ap, uint64_t);
                        goto _do_integer_conversion;
                    }
                    /* fall trough */
                case 'p':
