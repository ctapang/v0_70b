...
#ifdef MEDIA_STORAGE_MDD_SPI_SDMMC
    #include "system/drivers/drv_spi_sd.h"
#endif

#ifdef MEDIA_STORAGE_MDD_INTERNAL_RAM
    #include "system/fs/int_ramdisk.h"
#endif

#ifdef MEDIA_STORAGE_MDD_CF_PMP
    #include    "system/drivers/cf_pmp.h"
#endif
#ifdef MEDIA_STORAGE_MDD_CF_IO
    #include    "system/drivers/cf_bit_transaction.h"
#endif
#ifdef MEDIA_STORAGE_MDD_USB_MSD
    #include    "usb/usb_host_msd_scsi.h"
#endif
#ifdef MEDIA_STORAGE_MDD_INTERNAL_FLASH
    #include    "system/drivers/int_flashdisk.h"
#endif
...
typedef struct
{
    ...
    char            name[FILE_NAME_SIZE_8P3];       // The short name of the file
		...
		uint16_t *utf16LFNptr;	        // Pointer to long file name in UTF16 format
		uint16_t utf16LFNlength;          // LFN length in terms of words excluding the NULL word at the last.
	#endif
...
#define MDD_INVALID_HANDLE NULL

typedef FSFILE  * MDDFS_HANDLE;
...
typedef struct
{
	...
	#ifdef SUPPORT_LFN
		bool			AsciiEncodingType;          // Ascii file name or Non-Ascii file name indicator
		uint16_t *utf16LFNfound;		    // Pointer to long file name found in UTF16 format
		uint16_t utf16LFNfoundLength;     // LFN Found length in terms of words including the NULL word at the last.
	#endif
...
/***************************************************************
  Function:
    int wFSrename (const rom uint16_t * fileName, FSFILE * fo)
  Summary:
...
char * wFSgetcwd (uint16_t * path, int numbchars);
#endif
