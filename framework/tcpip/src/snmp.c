...
static bool TCPIP_SNMP_OIDStringGetByAddr(OID_INFO* rec, uint8_t* oidString, uint8_t* len);
static void TCPIP_SNMP_MIBRecordRead(uint32_t h, OID_INFO* rec);
...
//    TCPIP_ARP_Resolve(remHostIpAddrPtr);
}
...
bool TCPIP_SNMP_Notify(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index)
{
	...
	if((packetStructLenOffset == 0)&&(pduStructLenOffset==0))
	{
            ...
            TCPIP_SNMP_DataCopyToProcessBuffer(SnmpStackDcptMemStubPtr->SNMPNotifyInfo.timestamp.v[3],snmpTrapPutData);
            TCPIP_SNMP_DataCopyToProcessBuffer(SnmpStackDcptMemStubPtr->SNMPNotifyInfo.timestamp.v[2],snmpTrapPutData);
            TCPIP_SNMP_DataCopyToProcessBuffer(SnmpStackDcptMemStubPtr->SNMPNotifyInfo.timestamp.v[1],snmpTrapPutData);
            TCPIP_SNMP_DataCopyToProcessBuffer(SnmpStackDcptMemStubPtr->SNMPNotifyInfo.timestamp.v[0],snmpTrapPutData);
            ...
            TCPIP_SNMP_DataCopyToProcessBuffer(0,snmpTrapPutData);
            if ( !TCPIP_SNMP_OIDStringFindByID(SnmpStackDcptMemStubPtr->SNMPNotifyInfo.trapIDVar, &rec, OIDValue, &OIDLen) )
            {
#if defined (HARMONY_FS)
                SYS_FS_FileClose(snmpFileDescrptr);
#else
	...
	if(SnmpStackDcptMemStubPtr->gGenericTrapNotification != ENTERPRISE_SPECIFIC)
	{
            ...
            if ( !TCPIP_SNMP_OIDStringFindByID(SnmpStackDcptMemStubPtr->SNMPNotifyInfo.agentIDVar, &rec, OIDValue, &OIDLen) )
            {
#if defined (HARMONY_FS)
                SYS_FS_FileClose(snmpFileDescrptr);
#else
            ...
            TCPIP_SNMP_DataCopyToProcessBuffer(ASN_OID,snmpTrapPutData);
            SYS_FS_FileRead((uint8_t*)&len,1,snmpFileDescrptr);
#else
            ...
            while( len-- )
            {
                uint8_t c;
#if defined (HARMONY_FS)
                SYS_FS_FileRead((uint8_t*)&c,1,snmpFileDescrptr);
#else
...
bool TCPIP_SNMP_Notify(SNMP_ID var, SNMP_VAL val, SNMP_INDEX index)
{
    ...
    if ( !TCPIP_SNMP_OIDStringFindByID(SnmpStackDcptMemStubPtr->SNMPNotifyInfo.agentIDVar,
    ...
    SYS_FS_FileSeek(snmpFileDescrptr, rec.hData, SYS_FS_SEEK_SET);
    TCPIP_SNMP_DataCopyToProcessBuffer(ASN_OID,snmpTrapPutData);
    SYS_FS_FileRead((uint_8*)&len,1,snmpFileDescrptr);
#else
    SYS_FS_lseek(snmpFileDescrptr, rec.hData, SEEK_SET);
    TCPIP_SNMP_DataCopyToProcessBuffer(ASN_OID,snmpTrapPutData);
	...
	uint8_t c;
#if defined (HARMONY_FS)
        SYS_FS_FileRead(&c,1,snmpFileDescrptr);
#else
...
static SNMP_ERR_STATUS TCPIP_SNMP_ProcessPDUHeader(PDU_INFO* pduDbPtr, char* community, uint8_t* len)
{
	...
	*snmpMsgBuf = TCPIP_SNMP_ReadByte(snmpDcpt[intfIdx].skt);
    ...
    retlen= TCPIP_SNMP_LengthIsValid(&tempLen.w[0]);
    if ( !retlen )
...
static bool TCPIP_SNMP_ProcessHeaderGetSet(PDU_INFO* pduDbPtr)
{
    ...
    if ( TCPIP_SNMP_VarDataTypeIsValidInteger(&tempData.Val) )
	...
	if((pduDbPtr->snmpVersion == (uint8_t)SNMP_V1 || pduDbPtr->snmpVersion == (uint8_t)SNMP_V2C /*|| pduDbPtr->snmpVersion == (uint8_t)SNMP_V3*/) &&(pduDbPtr->pduType != GET_BULK_REQUEST))
	{
	    ...
	    return TCPIP_SNMP_VarDataTypeIsValidInteger(&tempData.Val);
	}
    ...
    while(1)
    {
        switch(smSnmp)
        {
            ...
            case SM_RESPONSE_PDU_LEN_OFFSET:
                ...
                TCPIP_SNMP_DataCopyToProcessBuffer(pduDbPtr->requestID.v[3],snmpPutData); // Start MSB
                TCPIP_SNMP_DataCopyToProcessBuffer(pduDbPtr->requestID.v[2],snmpPutData);
                TCPIP_SNMP_DataCopyToProcessBuffer(pduDbPtr->requestID.v[1],snmpPutData);
                TCPIP_SNMP_DataCopyToProcessBuffer(pduDbPtr->requestID.v[0],snmpPutData);
		...
		case SM_VARSTRUCT_LEN_OFFSET:
                    ...
                    if(Getbulk_N==0u)
                    {
                        ...
                        if((pduDbPtr->snmpVersion == (uint8_t)SNMP_V3))
                        {
                            ...
                            TCPIP_SNMP_OIDStringFindByID(MICROCHIP,&rec, OIDValue, &agentIDLen);
...
#if defined (HARMONY_FS)
                            SYS_FS_FileRead(&len,1,snmpFileDescrptr);
#else
		...
		case SM_MAX_REPETITIONS:
                    ...
                    for(repeatCntr=0;repeatCntr<Getbulk_M;repeatCntr++)
                    {
                        ...
                        for(varBindCntr=0;varBindCntr<Getbulk_R;varBindCntr++)
                        {
                            ...
                            {
                                temp = TCPIP_SNMP_NextLeafGet(&OIDInfo);
                            }
...
uint8_t TCPIP_SNMP_ProcessGetNextVar(OID_INFO* rec,PDU_INFO* pduDbPtr)
{
    ...
    {
        if ( !TCPIP_SNMP_NextLeafGet(rec))
            return false;
    }

    // Get complete OID string from oid record.
    if ( !TCPIP_SNMP_OIDStringGetByAddr(rec, OIDValue, &OIDLen))
    {
        return false;
    }
    ...
    while(1)
    {
...
#if defined (HARMONY_FS)
        SYS_FS_FileRead(&dummyRead,1,snmpFileDescrptr);
        indexBytes = 0;
        SYS_FS_FileRead(&indexInfo.Val,1,snmpFileDescrptr);
        SYS_FS_FileRead(&idLen,1,snmpFileDescrptr);
        if(idLen == 1)
        {
            uint8_t temp;
            SYS_FS_FileRead((uint8_t*)&temp,1,snmpFileDescrptr);
            indexRec.id = temp & 0xFF;
        }
        else if(idLen == 2)
        {
            uint8_t temp[2];
            SYS_FS_FileRead(temp,2,snmpFileDescrptr);
            indexRec.id = 0;
        ...
        indexRec.dataType = 0;
        SYS_FS_FileRead((uint8_t*)&indexRec.dataType,1,snmpFileDescrptr);
#else
...
uint8_t TCPIP_SNMP_ProcessGetBulkVar(OID_INFO* rec, uint8_t* oidValuePtr, uint8_t* oidLenPtr,uint8_t* successor,PDU_INFO* pduDbPtr)
{
    ...
    */
    ...
    // Get complete OID string from oid record.
    if(!TCPIP_SNMP_OIDStringGetByAddr(rec, oidValuePtr, &OIDLen))
        return false;
    ...
    while(1)
    {
        ...
        {
            uint8_t temp;
    	    SYS_FS_FileRead(&temp,1,snmpFileDescrptr);
    	    indexRec.id = temp & 0xFF;
        }
        else if(idLen == 2)
        {
...
/****************************************************************************
  Function:
	uint8_t TCPIP_SNMP_OIDFindInMgmtInfoBase(PDU_INFO* pduDbPtr,uint8_t* oid, uint8_t oidLen, OID_INFO* rec)
...
uint8_t TCPIP_SNMP_OIDFindInMgmtInfoBase(PDU_INFO* pduDbPtr,uint8_t* oid, uint8_t oidLen, OID_INFO* rec)
{
    ...
    {
#if defined (HARMONY_FS)
        SYS_FS_FileSeek(snmpFileDescrptr, hNode, SYS_FS_SEEK_SET);    
        rec->hNode = SYS_FS_FileTell(snmpFileDescrptr); // hNode;
        SYS_FS_FileRead(&savedOID,1,snmpFileDescrptr);
        if(comapreOidWithSibling==(uint8_t)false)
        {
            tempSavedOID=savedOID;
            tempHNode=hNode;
        }
        SYS_FS_FileRead(&rec->nodeInfo.Val,1,snmpFileDescrptr);
        ...
        {
            SYS_FS_FileRead(&tempData.v[0],1,snmpFileDescrptr);
            SYS_FS_FileRead(&tempData.v[1],1,snmpFileDescrptr);
        ...
        if ( savedOID != *reqOidPtr )
        {
            ...
            {
#if defined (HARMONY_FS)
                SYS_FS_FileSeek(snmpFileDescrptr, tempData.Val, SYS_FS_SEEK_SET);
                hNode = SYS_FS_FileTell(snmpFileDescrptr);
#else
...
#if defined (HARMONY_FS)
                SYS_FS_FileRead((uint8_t*)&rec->dataType,1,snmpFileDescrptr);
                rec->hData = SYS_FS_FileTell(snmpFileDescrptr);
#else
...
/****************************************************************************
  Function:
	bool TCPIP_SNMP_NextLeafGet(OID_INFO* rec)
...
bool TCPIP_SNMP_NextLeafGet(OID_INFO* rec)
{
    ...
    if ( !rec->nodeInfo.Flags.bIsParent )
    {
           ...
           rec->nodeInfo.Flags.bIsDistantSibling )
        {
            ...
            SYS_FS_FileSeek(snmpFileDescrptr, rec->hSibling, SYS_FS_SEEK_SET);
#else
    ...
    while( 1 )
    {
...
#if defined (HARMONY_FS)
        rec->hNode = SYS_FS_FileTell(snmpFileDescrptr);

        // Read OID byte.
        SYS_FS_FileRead(&rec->oid,1,snmpFileDescrptr);
        SYS_FS_FileRead(&rec->nodeInfo.Val,1,snmpFileDescrptr);
        if ( rec->nodeInfo.Flags.bIsIDPresent )
        {
             SYS_FS_FileRead(&idLen,1,snmpFileDescrptr);
        ...
        if ( rec->nodeInfo.Flags.bIsSibling ||
             rec->nodeInfo.Flags.bIsDistantSibling )
        {
            SYS_FS_FileRead(&temp.v[0],1,snmpFileDescrptr);
            SYS_FS_FileRead(&temp.v[1],1,snmpFileDescrptr);
...
/****************************************************************************
  Function:
	bool TCPIP_SNMP_OIDStringFindByID(SNMP_ID id, OID_INFO* info, 
...
***************************************************************************/
bool TCPIP_SNMP_OIDStringFindByID(SNMP_ID id, OID_INFO* info, uint8_t* oidString, uint8_t* len)
{
    uint32_t hCurrent;
    ...
    while (1)
    {
        ...
        TCPIP_SNMP_MIBRecordRead(hCurrent, info);
        ...
        if ( !info->nodeInfo.Flags.bIsParent )
        {
            ...
            {
                if ( info->id == id )
                    return TCPIP_SNMP_OIDStringGetByAddr(info, oidString, len);
            }
            ...
            {
#if defined (HARMONY_FS)
                SYS_FS_FileSeek(snmpFileDescrptr, info->hSibling, SYS_FS_SEEK_SET);
#else
...
#if defined (HARMONY_FS)
        hCurrent = SYS_FS_FileTell(snmpFileDescrptr);
#else
...
/****************************************************************************
  Function:
	bool TCPIP_SNMP_OIDStringGetByAddr(OID_INFO* rec, uint8_t* oidString, uint8_t* len)
...
bool TCPIP_SNMP_OIDStringGetByAddr(OID_INFO* rec, uint8_t* oidString, uint8_t* len)
{
    ...
    while( 1 )
    {
        TCPIP_SNMP_MIBRecordRead(hCurrent, &currentMIB);
...
#if defined (HARMONY_FS)
                SYS_FS_FileSeek(snmpFileDescrptr, hNext, SYS_FS_SEEK_SET);
                hNext = SYS_FS_FileTell(snmpFileDescrptr);
#else
...
/****************************************************************************
  Function:
	void TCPIP_SNMP_MIBRecordRead(uint32_t h, OID_INFO* rec)
...
static void TCPIP_SNMP_MIBRecordRead(uint32_t h, OID_INFO* rec)
{
...
#if defined (HARMONY_FS)
    SYS_FS_FileSeek(snmpFileDescrptr, h, SEEK_SET);
    ...
    {
        SYS_FS_FileRead(&tempVal.v[0],1,snmpFileDescrptr);
        SYS_FS_FileRead(&tempVal.v[1],1,snmpFileDescrptr);
        rec->hSibling = tempVal.Val;
    }
    if ( nodeInfo.Flags.bIsParent )
    {
       rec->hChild = SYS_FS_FileTell(snmpFileDescrptr);
    }
    else
    {
...
uint8_t TCPIP_SNMP_ProcessGetVar(OID_INFO* rec, bool bAsOID,PDU_INFO* pduDbPtr)
{
    ...
    if ( rec->nodeInfo.Flags.bIsConstant )
    {
        ...
        SYS_FS_FileRead(&varLen,1,snmpFileDescrptr);
        temp = varLen;
        while( temp-- )
        {
            SYS_FS_FileRead(&c,1,snmpFileDescrptr);
...
uint16_t snmpPduLen;
	...
	while(snmpPduLen)
	{
		...
		if(!structureLen)
		  return false;
	//	if(!TCPIP_SNMP_SNMPStructureIsValid(&varBindLen))
		//	return false;
