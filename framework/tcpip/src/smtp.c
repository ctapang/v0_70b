...
bool TCPIP_SMTP_ClientInitialize(const TCPIP_STACK_MODULE_CTRL* const stackCtrl, const SMTP_CLIENT_MODULE_CONFIG* pSmtpConfig)
{
    return true;
}
...
void TCPIP_SMTP_ClientTask(void)
{
	...
	switch(TransportState)
	{
		...
		case TRANSPORT_BEGIN:
			...
			{
                TCPIP_DNS_Resolve((const char*)SMTPClient.Server, DNS_TYPE_A);
			}
			else
			{
                ...
                SMTPClient.Server++;
                TCPIP_DNS_Resolve((const char*)SMTPClient.Server, DNS_TYPE_MX);
			}
		...
		case TRANSPORT_SECURING_SOCKET:		
			...
			if(SMTPClient.UseSSL && !TCPIP_TCPSSL_ClientStart(MySocket,NULL))
				break;
		...
		case TRANSPORT_SOCKET_OBTAINED:
			...
			if(SMTPClient.UseSSL && TCPIP_TCPSSL_StillHandshaking(MySocket))
				break;
			#endif
...
/*****************************************************************************
  Function:
	void TCPIP_SMTP_MailSend(SMTP_POINTERS* smtpClientMessage)
