...
static void     _DHCPDiscard(UDP_SOCKET s, int len);
...
bool TCPIP_DHCP_Initialize(const TCPIP_STACK_MODULE_CTRL* const stackCtrl, const DHCP_MODULE_CONFIG* pDhcpConfig)
{
    ...
    {   // first time we're run
        ...
        TCPIP_Helper_SingleListInitialize(&dhcpRegisteredUsers);
    }
        ...
        switch(pClient->smState)
        {
            ...
            case SM_DHCP_GET_OFFER:
                ...
                if(TCPIP_UDP_GetIsReady(pClient->hDHCPSocket) < 250u)
                {
                    ...
                    if(SYS_TICK_Get() - pClient->dwTimer >= (dhcpTmo * SYS_TICK_TicksPerSecondGet()))
                    {
        ...
        switch(v)
        {
            ...
            case DHCP_ROUTER:
                ...
                else
                {
                    ...
                    // discard remaining/other router addresses
                    _DHCPDiscard(s, len - 4); 
                }
            ...
            case DHCP_DNS:
                ...
                else
                {
                    ...
                    // Discard any other DNS addresses offered
                    _DHCPDiscard(s, len);
            ...
            default:
                ...
                _DHCPDiscard(s, len);           // Ignore option values
                break;
        }
    }
...
// discards an amount of data from UDP socket s
static void _DHCPDiscard(UDP_SOCKET s, int len)
{
    int     discardLen;
    uint8_t discardBuffer[40];

    while(len)
    {
        if(len > sizeof(discardBuffer))
        {
            discardLen = sizeof(discardBuffer);
        }
        else
        {
            discardLen = len;
        }
        TCPIP_UDP_ArrayGet(s, discardBuffer, discardLen);
        len -= discardLen;
    }
...
static void _DHCPSend(DHCP_CLIENT_VARS* pClient, TCPIP_NET_IF* pNetIf, uint8_t messageType, bool bRenewing)
{
	...
	TCPIP_UDP_Put(s, BOOT_REQUEST);                       // op
	TCPIP_UDP_Put(s, BOOT_HW_TYPE);                       // htype
	TCPIP_UDP_Put(s, BOOT_LEN_OF_HW_TYPE);                // hlen
	TCPIP_UDP_Put(s, 0);                                  // hops
    if(newTransaction)
    {
        pClient->transactionID.Val = SYS_Rand();        // generate a new transaction ID
    }
	TCPIP_UDP_Put(s, pClient->transactionID.v[0]);      // xid[0]
	TCPIP_UDP_Put(s, pClient->transactionID.v[1]);      // xid[1]
	TCPIP_UDP_Put(s, pClient->transactionID.v[2]);      // xid[2]
	TCPIP_UDP_Put(s, pClient->transactionID.v[3]);      // xid[3]
	TCPIP_UDP_Put(s, 0);                                // secs[0]
	TCPIP_UDP_Put(s, 0);                                // secs[1]
	TCPIP_UDP_Put(s, 0);                                // flags[0] request unicast reply
	TCPIP_UDP_Put(s, 0);                                // flags[1]
	...
	// Load message type.
	TCPIP_UDP_Put(s, DHCP_MESSAGE_TYPE);
	TCPIP_UDP_Put(s, DHCP_MESSAGE_TYPE_LEN);
    ...
    {   // can use a server ID
        if((messageType == DHCP_REQUEST_MESSAGE) && !bRenewing)
        {
            ...
            TCPIP_UDP_Put(s, DHCP_SERVER_IDENTIFIER);
            TCPIP_UDP_Put(s, DHCP_SERVER_IDENTIFIER_LEN);
            TCPIP_UDP_Put(s, ((uint8_t*)(&pClient->dwServerID))[3]);
            TCPIP_UDP_Put(s, ((uint8_t*)(&pClient->dwServerID))[2]);
            TCPIP_UDP_Put(s, ((uint8_t*)(&pClient->dwServerID))[1]);
            TCPIP_UDP_Put(s, ((uint8_t*)(&pClient->dwServerID))[0]);
        }
    }
	...
	TCPIP_UDP_Put(s, DHCP_PARAM_REQUEST_LIST);
	TCPIP_UDP_Put(s, DHCP_PARAM_REQUEST_LIST_LEN);
	TCPIP_UDP_Put(s, DHCP_SUBNET_MASK);
	TCPIP_UDP_Put(s, DHCP_ROUTER);
	TCPIP_UDP_Put(s, DHCP_DNS);
	TCPIP_UDP_Put(s, DHCP_HOST_NAME);
	...
	{
		TCPIP_UDP_Put(s, DHCP_PARAM_REQUEST_IP_ADDRESS);
		TCPIP_UDP_Put(s, DHCP_PARAM_REQUEST_IP_ADDRESS_LEN);
	...
	while(TCPIP_UDP_TxCountGet(s) < 300u)
		TCPIP_UDP_Put(s, 0); 
