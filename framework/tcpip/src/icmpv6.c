...
typedef void (*icmpv6Callback) (TCPIP_NET_HANDLE hNetIf, uint8_t type, IPV6_ADDR * localIP, IPV6_ADDR * remoteIP, void * header);

static SINGLE_LIST      icmpv6RegisteredUsers = { 0 };
//
...
static void _ICMPV6_NotifyClients(TCPIP_NET_HANDLE hNetIf, uint8_t type, IPV6_ADDR * localIP, IPV6_ADDR * remoteIP, void * header);
                       ...
                       const void* icmpv6Data)
{
    ...
    if (icmpv6Init == false)
    {
        ...
        TCPIP_Helper_SingleListInitialize(&icmpv6RegisteredUsers);
        icmpv6Init = true;
    }
...
void TCPIP_ICMPV6_Deinitialize (const TCPIP_STACK_MODULE_CTRL* const stackInit)
{
    ...
    TCPIP_Notification_RemoveAll(&icmpv6RegisteredUsers, icmpv6MemH);
    icmpv6Init = false;
}
...
/*****************************************************************************
  ...
  Remarks:
    None
  ***************************************************************************/
IPV6_PACKET * TCPIP_ICMPV6_Open (TCPIP_NET_IF * pNetIf, IPV6_ADDR * localIP, IPV6_ADDR * remoteIP)
{
...
/*****************************************************************************
  ...
  Remarks:
    None
  ***************************************************************************/
IPV6_PACKET * TCPIP_ICMPV6_HeaderErrorPut (TCPIP_NET_IF * pNetIf, IPV6_ADDR * localIP, IPV6_ADDR * remoteIP, uint8_t code, uint8_t type, uint32_t additionalData)
{
...
/*****************************************************************************
  ...
  ***************************************************************************/
void TCPIP_ICMPV6_Process(TCPIP_NET_IF * pNetIf, TCPIP_MAC_PACKET* pRxPkt, IPV6_ADDR_STRUCT * localIPStruct, IPV6_ADDR * localIP, IPV6_ADDR * remoteIP, uint16_t dataLen, uint16_t headerLen, uint8_t hopLimit, uint8_t addrType)
{
        ...
        case ICMPV6_ERROR_PACKET_TOO_BIG:
            ...
            if (pseudoHeader.DestAddress.v[0] == 0xFF)
            {
                ...
                if (pIpv6Config->multicastMTU > h.header_Error.additionalData)
                {
                    ...
                    pIpv6Config->mtuIncreaseTimer = SYS_TICK_Get() + (SYS_TICK_ResolutionGet() * IPV6_MTU_INCREASE_TIMEOUT);
                }
            }
            else
            {
            ...
            if (icmpv6RegisteredUsers.head != NULL)
            {
...
static void _ICMPV6_NotifyClients(TCPIP_NET_HANDLE hNetIf, uint8_t type, IPV6_ADDR * localIP, IPV6_ADDR * remoteIP, void * header)
{
    ...
    for(dNode = (ICMPV6_LIST_NODE*)icmpv6RegisteredUsers.head; dNode != 0; dNode = dNode->next)
    {
