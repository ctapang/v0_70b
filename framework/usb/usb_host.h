...
/* Driver USB Host Module Index
  ...
  Remarks:
    ...
    USB_HOST_Open() routines to identify the host layer instance in use.
*/
...
/* USB Host Layer Event Handler Response None  
  ...
  Summary:
    USB Host Layer Event Handler Response Type None.

  Description:
    This is the definition of the USB Host Layer Event Handler Response
    Type None.
...
#define USB_HOST_ENDPOINT_TABLE_SIZE /*DOM-IGNORE-BEGIN*/32/*DOM-IGNORE_END*/
...
// *****************************************************************************
/* USB Host Class Driver Match by Class, Subclass and Protocl
  ...
  Description:
    ...
    applicaton must set the device member of the TPL table entry to this macro
...
/* USB Device Information Structure
  ...
  Summary:
    Defines the USB Device Information data strucutre.

  Description:
    This data type defines the USB Device information data structure. This data
    structure is passed to USB_HOST_DeviceInformationGet() function to obtain
    specific information about the device. The application can use this
    information to update its display or for other purposes. When processed by
    the USB_HOST_DeviceInformationGet() function, the stringDesciptor member of
    this data structure will contain a NULL terminated string descriptor string
    in UTF-16 format. The vid and pid members of the data structure will be
    populated with VID and PID of the attached device respectively.
...
typedef struct
{   
    /* VID of the attached device */
    uint16_t vid;

    /* PID of the attached device */
    uint16_t pid;

    /* String desciptor string in UTF-16 format. The string when
       returned by USB_HOST_DeviceInformationGet() function will be
       NULL terminated. */
...
typedef enum
{
     ...
     * information on the amount of current requested. */
    ...
    /* The host layer could not enumerate the attached device. This can happen
       when the device does not respond to control transfer. There is not event
...
/* USB Host Event Handler Function Pointer Type.
  ...
  Description:
    ...
    a USB Host Event handling function who's function signature (parameter and
...
USB_ERROR USB_HOST_DeviceInformationGet(USB_HOST_HANDLE hostHandle, uint8_t deviceAddress, USB_DEVICE_INFORMATION * deviceInfo); 
