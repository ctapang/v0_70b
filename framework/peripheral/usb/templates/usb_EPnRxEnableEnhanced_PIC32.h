        ...
        uint8_t endpoint, int direction, bool isControl, bool handshake)
{
    ...
    if(isControl)
    {
        *(epControlRegister + 1) = 0x10;    // Clear the EPCONDIS bit
        *(epControlRegister + 2) = 0x0D;    // Set the TX, RX and Handshake bit
        return;
    }
    
    /* Set the direction */
    *(epControlRegister + 2) = 0x8 >> direction;

    /* Based on whether handshake has to enabled or disabled
     * epControlRegister points to either the set or clr 
     * register */
    epControlRegister = (handshake) ? (epControlRegister + 2) : 
                                            (epControlRegister + 1);

    *epControlRegister = 0x1; 
...
PLIB_TEMPLATE void USB_EPnStallClear_PIC32( USB_MODULE_ID index , uint8_t endpoint )
{
    ...
    epControlRegister += (4 * endpoint);
    epControlRegister += 1; // This is endpoint clear register
    *epControlRegister = 0x2; // Bit 1 is the stall indicator bit
