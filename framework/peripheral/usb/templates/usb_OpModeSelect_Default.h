...
PLIB_TEMPLATE void USB_OperatingModeSelect_Default( USB_MODULE_ID index , USB_OPMODES opMode )
{
    switch ( opMode )
    {
        case USB_OPMODE_NONE:
            _SFR_BIT_CLEAR(_USB_DEVICE_ENABLE_VREG(index),_USB_DEVICE_ENABLE_POS(index) );
            _SFR_BIT_CLEAR(_USB_HOST_ENABLE_VREG(index),  _USB_HOST_ENABLE_POS(index)   );
            _SFR_BIT_CLEAR(_USB_OTG_ENABLE_VREG(index),   _USB_OTG_ENABLE_POS(index)    );

        case USB_OPMODE_DEVICE:
            _SFR_BIT_SET(  _USB_DEVICE_ENABLE_VREG(index),_USB_DEVICE_ENABLE_POS(index) );
            _SFR_BIT_CLEAR(_USB_HOST_ENABLE_VREG(index),  _USB_HOST_ENABLE_POS(index)   );
            _SFR_BIT_CLEAR(_USB_OTG_ENABLE_VREG(index),   _USB_OTG_ENABLE_POS(index)    );
            break;
        ...
        case USB_OPMODE_HOST:
            _SFR_BIT_CLEAR(_USB_DEVICE_ENABLE_VREG(index),_USB_DEVICE_ENABLE_POS(index) );
            _SFR_BIT_SET(  _USB_HOST_ENABLE_VREG(index),  _USB_HOST_ENABLE_POS(index)   );
            _SFR_BIT_CLEAR(_USB_OTG_ENABLE_VREG(index),   _USB_OTG_ENABLE_POS(index)    );
            break;
        ...
        case USB_OPMODE_OTG:
            _SFR_BIT_SET(  _USB_DEVICE_ENABLE_VREG(index),_USB_DEVICE_ENABLE_POS(index) );
            _SFR_BIT_CLEAR(_USB_HOST_ENABLE_VREG(index),  _USB_HOST_ENABLE_POS(index)   );
            _SFR_BIT_SET(  _USB_OTG_ENABLE_VREG(index),   _USB_OTG_ENABLE_POS(index)    );
            break;
