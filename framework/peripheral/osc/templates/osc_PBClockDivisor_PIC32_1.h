...
PLIB_TEMPLATE void OSC_PBClockDivisorSet_PIC32_1( OSC_MODULE_ID index , OSC_PERIPHERAL_BUS peripheralBusNumber , OSC_PB_CLOCK_DIV_TYPE peripheralBusClkDiv )
{
	...
	switch (peripheralBusClkDiv)
	{
    case 1:		_SFR_FIELD_WRITE(_OSC_OSC_PB_DIVISOR_VREG(index),
								 _OSC_OSC_PB_DIVISOR_MASK(index),
								 _OSC_OSC_PB_DIVISOR_POS(index) ,
								 0x0 );
				break;
	case 2:		_SFR_FIELD_WRITE(_OSC_OSC_PB_DIVISOR_VREG(index),
								 _OSC_OSC_PB_DIVISOR_MASK(index),
								 _OSC_OSC_PB_DIVISOR_POS(index) ,
								 0x1 );
				break;
	case 4:		_SFR_FIELD_WRITE(_OSC_OSC_PB_DIVISOR_VREG(index),
								 _OSC_OSC_PB_DIVISOR_MASK(index),
								 _OSC_OSC_PB_DIVISOR_POS(index) ,
								 0x2 );
				break;
	case 8:		_SFR_FIELD_WRITE(_OSC_OSC_PB_DIVISOR_VREG(index),
								 _OSC_OSC_PB_DIVISOR_MASK(index),
								 _OSC_OSC_PB_DIVISOR_POS(index) ,
								 0x3 );
				break;
	default: 	PLIB_ASSERT(false, "Given PB Divisor value is not supported for the selected device");							 
	}
