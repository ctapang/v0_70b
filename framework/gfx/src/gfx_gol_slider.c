/*******************************************************************************
  ...
  Summary:
    This implements the slider object of the GOL.
        ...
        )
{
    ...
    pSld->hdr.actionGet = SldTranslateMsg; // message function
    pSld->hdr.actionSet = GFX_GOL_ScrollBarMsgDefault; // default message function
...
uint16_t SldGetWidth(GFX_GOL_SCROLLBAR *pSld)
{
     ...
     */
    if (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_VERTICAL))
    {
        temp = pSld->hdr.right - pSld->hdr.left;
        if (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_SCROLLBAR))
        {
...
void GFX_GOL_ScrollBarMsgDefault(uint16_t translatedMsg, void *pObj, GFX_GOL_MESSAGE *pMsg)
{
    ...
    {
        if (!GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_FOCUSED))
        {
    ...
    if (pMsg->type == TYPE_TOUCHSCREEN)
    {
        if ((translatedMsg == GFX_GOL_SCROLLBAR_ACTION_DEC) || (translatedMsg == GFX_GOL_SCROLLBAR_ACTION_INC))
        {
            ...
            { // check if we need to redraw thumb
                ...
                GFX_GOL_ObjectStateSet(pSld, GFX_GOL_SLIDER_DRAW); // redraw the thumb only
            }
            else
                return;
        }
    ...
    { // for keyboard
        ...
        GFX_GOL_ObjectStateSet(pSld, GFX_GOL_SLIDER_DRAW_THUMB); // redraw the thumb only
    }
...
uint16_t SldTranslateMsg(void *pObj, GFX_GOL_MESSAGE *pMsg)
{
    ...
    if (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_DISABLED))
...
GFX_STATUS GFX_GOL_ScrollBarDraw(void *pObj)
{
    ...
    typedef enum
    {
        ...
        WAIT
    } GFX_GOL_SLIDER_DRAW_STATES;

    static GFX_COLOR colorTemp = 0;

    static GFX_GOL_SLIDER_DRAW_STATES state = GFX_GOL_SLIDER_STATE_IDLE;
    ...
    while (1)
    {
        switch (state)
        {
            ...
            case GFX_GOL_SLIDER_STATE_IDLE:
                if (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_HIDE))
                {
                ...
                else
                {
                    ...
                    midPoint = GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_VERTICAL) ? (pSld->hdr.left + pSld->hdr.right) >> 1 : (pSld->hdr.top + pSld->hdr.bottom) >> 1;
                    ...
                    if (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_DRAW))
                    { // draw the panel for the slider
                        ...
                        GFX_GOL_PanelParameterSet
                                ...
                                0,
                                colorTemp,
                                (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_SCROLLBAR)) ? pSld->hdr.pGolScheme->EmbossDkColor : pSld->hdr.pGolScheme->EmbossLtColor,
                                (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_SCROLLBAR)) ? pSld->hdr.pGolScheme->EmbossLtColor : pSld->hdr.pGolScheme->EmbossDkColor,
                                NULL,
            ...
            case GFX_GOL_SLIDER_STATE_HIDE:
                index = GFX_RectangleFillDraw(pSld->hdr.left, pSld->hdr.top, pSld->hdr.right, pSld->hdr.bottom);
            ...
            case GFX_GOL_SLIDER_STATE_CLEARTHUMB: // this removes the current thumb
                ...
                if (!GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_DRAW_THUMB))
                { // GFX_GOL_SLIDER_DRAW_THUMB is only set when
            ...
            case GFX_GOL_SLIDER_STATE_THUMBPANEL:
                ...
                if (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_SCROLLBAR) || !GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_DRAW_FOCUS))
                { // check if scroll bar focus is not used
                    state = WAIT; // go back to idle state
                    return (GFX_STATUS_FAILURE);
                }
            ...
            case GFX_GOL_SLIDER_STATE_FOCUS:
                ...
                { // do not draw focus when in scroll bar mode
                    GFX_LineTypeSet(GFX_GOL_FOCUS_LINE);
                    if (GFX_GOL_ObjectStateGet(pSld, GFX_GOL_SLIDER_FOCUSED))
                    {
                    ...
                    index =  GFX_RectangleDraw(
                            ...
                            pSld->hdr.bottom - GFX_GOL_EMBOSS_SIZE);
                    if(index == GFX_STATUS_FAILURE)
                    {
                        return (GFX_STATUS_FAILURE);
                    }

                    GFX_LineTypeSet(SOLID); // reset line type
                }

                state = WAIT; // set state to idle
