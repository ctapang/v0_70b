...
/* Interrupt Tasks Routine Pointer
  ...
  Summary:
    Pointer to an interrupt-handlng "Tasks" routine.
...
/*******************************************************************************
  ...
  Parameters:
    ...
    subpriority	- Subproirity (if supported)
...
/*******************************************************************************
  ...
  Summary:
    Registers an Interrupt "Tsks" Routine for the specified interrupt source
    or trap).
    
  Description:
    This function registers an Interrupt "Tasks" Routine for the specified interrupt
    source or trap).
  ...
  Remarks:
    This routine only generates executable code when a driver is configured to
    register dynamically its "Tasks" routine with the system interrupt
    service.  However, it should be called even if the ISR-to-source 
    association is defined statically at compile time to maintain
    source code compatibility.  A device driver normally registers its
    own ISR from it's initialization routine.
    
    In the example code, the macros MY_DRIVER_INTERRUPT_SOURCE would be defined
    in the appropriate configuration header, which would be included by
    the driver source file where the "DRV_MYDEV_Tasks" routine and the MyParam
    data would be defined.
    
    It is safe to call this routine without first calling 
    SYS_INT_DynamicDeregister, even if a previous ISR has been registered.  The
    effect will be that the new ISR supplants the old one.
*/
...
/*******************************************************************************
  ...
  Remarks:
    ...
    initialiazed, this routine does not need to be called.
*/
...
/*******************************************************************************
  ...
  Summary:
    Identifies if interrupts are currently enbled or disabled at the top level.
    
  Description:
    This function identifies if interrupts are enbled or disabled at the top
    level.
  ...
  Returns:
    ...
    - false	- If the interrupts are ccurrentlyl disabled
