...
/*******************************************************************************
  ...
  Remarks:
    ...
    significant size to all projects.  The most efficient use is to enabled it
    in individual libraries and build them seperately for debugging.
*/

#ifndef SYS_ASSERT

    #define SYS_ASSERT(test,message) if (!(test))     asm("break 1");
...
/* Unsupported Attribute Abstraction
  ...
  Example:
    ...
    Will not generate a compiler error if the interface is not defined for the
    device selected.
