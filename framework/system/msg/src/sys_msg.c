...
void SYS_MSB_MailboxClose( SYS_OBJ_HANDLE handleMailbox )
{
...
void SYS_MSB_MailboxReinit( SYS_OBJ_HANDLE handleMailbox,
                            SYS_MSG_RECEIVE_CALLBACK msgCallBackFunction )
{
    SYS_MSG_MAILBOX_OBJECT *hMailbox = (SYS_MSG_MAILBOX_OBJECT *)handleMailbox;
...
/* Function:
  ...
  Summary:
    Gets sample driver version in numerical format.
*/

unsigned int SYS_MSG_VersionGet ( void )
{
    return( ( _SYS_MSG_VERSION_MAJOR * 10000 ) +
            ( _SYS_MSG_VERSION_MINOR * 100 ) +
            ( _SYS_MSG_VERSION_PATCH ) );
}
...
/* Function:
  ...
  Summary:
    Gets sample driver version in string format.
*/

char * SYS_MSG_VersionStrGet ( void )
{
    return _SYS_MSG_VERSION_STR;
}
