...
int MPFS_Open ( uintptr_t handle, const char* filewithDisk, uint8_t mode )
{
    ...
    uint8_t c = 0, hMPFS;
    volatile const char* cFile = filewithDisk + 2;    // Take the file name without the disk number (ignore "0:", so increment 2)
...
int MPFS_Stat ( const char* filewithDisk, uintptr_t stat_str )
{
    ...
    const char *ptr, *ptr1;
    uint8_t c = 0;
    const char* cFile = filewithDisk + 2;    // Take the file name without the disk number (ignore "0:", so increment 2)
    unsigned char i;
