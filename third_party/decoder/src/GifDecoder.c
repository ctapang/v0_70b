...
{
       GFX_DRV_instance[command->instance].PrimitiveFunction.SetColor(pGifDec->awPalette[bData]);
       while(GFX_DRV_instance[command->instance].PrimitiveFunction.PixelPut(pGifDec->wCurrentX, pGifDec->wCurrentY)); //TODO: Make NonBlocking
       pGifDec->wCurrentX++;
...
BYTE GIF_bDecode(IMG_FILE *pFile)
{
	...
	static enum
	{
        ...
        static uint8_t decodestate = INITIAL;
